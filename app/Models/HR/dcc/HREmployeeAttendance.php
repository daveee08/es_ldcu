...
class HREmployeeAttendance extends Model
{
    ...
    public static function gethours($days,$employeeid){
        ...
        foreach($days as $day)
        {
            ...
            $timeoutam = collect($attrecords)->where('ttime','<=',$customtimesched->pmin)->where('tapstate','OUT')->last()->ttime ?? '';
            $timeinpm = collect($attrecords)->where('ttime','>=',$customtimesched->amout)->where('ttime','<=',$customtimesched->pmout)->where('tapstate','IN')->first()->ttime ?? '';
