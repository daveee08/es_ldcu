...
class ExpensesController extends Controller
{
    public function expenses()
    {
        ...
        return view('finance.expenses.expensesV2', compact('extend'));
    ...
    public function saveexpense(Request $request)
    {
        if($request->ajax())
        {
            ...
            if($voucherno != '')
            {
                ...
                $voucherno = sprintf('%06d', $expense->voucherno+1);
            }
    ...
    public function searchexpenses(Request $request)
    {
        ...
        ->select(
            ...
            DB::raw("CONCAT(teacher.firstname,' ', teacher.lastname) AS employeename")
        )
        ...
        $expenses = $expenses->orderBy('refnum')->get();
        ...
        if(count($expenses) > 0)
        {
            ...
            foreach($expenses as $expense)
            {
                ...
                {
                    $list .='
                        <tr class="" data-id="'.$expense->expenseid.'">
                            ...
                            <td>
                                <div class="row text-center">
                                    <div class="col-md-6" style="border-right: 1px solid #ddd">
                                        <a href="#"><i class="text-info fas fa-pencil-alt expense-tr" data-id="'.$expense->expenseid.'"></i></a>
                                    </div>
                                    <div class="col-md-6">
                                        <a href="#"><i class="text-danger far fa-trash-alt delete" data-id=""></i></a>
                                    </div>
                                </div>
                            </td>
                        </tr>
                ...
                {
                    $list .='
                        <tr class="text-success" data-id="'.$expense->expenseid.'">
                           <td>'.$expense->refnum.'</td>
                            <td>'.$expense->employeename.'</td>
                            <td>'.$expense->department.'</td>
                            <td>'.$expense->remarks.'</td>
                ...
                {
                    $list .='
                        <tr class="text-danger" data-id="'.$expense->expenseid.'">
                           <td>'.$expense->refnum.'</td>
                            <td>'.$expense->employeename.'</td>
                            <td>'.$expense->department.'</td>
                            <td>'.$expense->remarks.'</td>
            ...
            $data = array(
                'list' => $list,
                'gtotal' => '<td colspan="4" class="text-right text-bold">TOTAL:</td>
                <td class="text-right"><span class="text-success text-bold">'.number_format($total, 2).'</span></td>
                <td colspan="4"></td>
                '
            );
    ...
    public function searchexpenses_backup(Request $request)
    {
        if($request->ajax())
        {
            $filter = $request->get('filter');
    ...
    public function company_load(Request $request)
    {
        ...
        if(isset($request->type) && $request->type == 'teacher'){
            $teachers = db::table('teacher')
            ->where('deleted', 0)
            ->orderBy('lastname','desc')
            ->get();

            foreach($teachers as $comp)
            {
                $list .= '
                    <option value="'.$comp->id.'">
                        '.strtoUpper($comp->lastname.', '.$comp->firstname).'
                    </option>
                ';
            }
    ...
    public function expenses_items(Request $request)
    {
        ...
        $items = db::table('items')
            ->select('id', 'description', 'itemcode')
            ->where('isexpense', 1)
            ->where('deleted', 0)
            ->get();
    ...
    public function expenses_print(Request $request)
    {
        ...
        {
            $status = 'POSTED' . ' - ' . date_format(date_create($expense->approveddatetime), 'm/d/Y h:i A');
        }
        else{
            $status = $expense->status;
        }
        ...
        $pdf = PDF::loadview('finance.expenses.reports.pdf_voucher', compact('expense', 'supplier', 'details', 'jedetails', 'bankname', 'paytitle', 'status'))
    ...
    public function cv_print(Request $request)
    {
        $datefrom = $request->get('datefrom');
        $dateto = $request->get('dateto');
        $paytitle = $request->get('paytype');

        $costcenters = db::table('expense_jedetails')
            ->select('acc_costcenters.description as costcenter', 'headerid')
            ->join('acc_coa', 'expense_jedetails.glid', '=', 'acc_coa.id')
            ->join('acc_costcenters', DB::raw("acc_coa.code"), 'LIKE', DB::raw("CONCAT(acc_costcenters.acc_code, '%')"))
            ->where('expense_jedetails.deleted', 0)
            ->groupBy('headerid')
            ->get();
        ...
        $expenses = db::table('expense')
            ->select(db::raw('expense.id as headerid, voucherno, transdate, companyname AS payee, items.`description` AS itemname, expense.remarks, SUM(expensedetail.`total`) AS amount'))
            ->join('expensedetail', 'expense.id', '=', 'expensedetail.headerid')
            ->join('items', 'expensedetail.itemid', '=', 'items.id')
            ->join('expense_company', 'expense.requestedbyid', '=', 'expense_company.id')
            ->where('expense.deleted', 0)
            ->where('expensedetail.deleted', 0)
            ->where('paytype', $paytitle)
            ->whereBetween('expense.transdate', [$datefrom . ' 00:00', $dateto . ' 23:59'])
            ->groupBy('expense.id')
            ->get();
        ...
        foreach($expenses as $expense)
        {
            $getcostcenter = $costcenters->where('headerid', $expense->headerid)->first();
            $cc = '';

            if($getcostcenter)
            {
                $cc = $getcostcenter->costcenter;
            }
            // dd($cc);
            array_push($expenselist, (object)[
                'voucherno' => $expense->voucherno,
        ...
        $expensesummary = db::table('expense')
            ->select(db::raw('items.description as itemname, SUM(expensedetail.`total`) AS amount'))
            ->join('expensedetail', 'expense.id', '=', 'expensedetail.headerid')
            ->join('items', 'expensedetail.itemid', '=', 'items.id')
            ->join('expense_company', 'expense.requestedbyid', '=', 'expense_company.id')
            ->where('expense.deleted', 0)
            ...
            ->where('paytype', $paytitle)
            ->whereBetween('expense.transdate', [$datefrom . ' 00:00', $dateto . ' 23:59'])
            ->groupBy('expensedetail.itemid')
            ->get();



        $pdf = PDF::loadview('finance.expenses.reports.pdf_voucher_report', compact('expenses', 'paytitle', 'datefrom', 'dateto', 'expensesummary'))
            ->setPaper('Legal', 'portrait');
        return $pdf->stream('Cash/Check Voucher Report.pdf');
    }
