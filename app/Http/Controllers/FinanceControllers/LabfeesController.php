...
class LabfeesController extends Controller
{
    ...
    public function labfeesubject_append(Request $request)
    {
        $subj = $request->get('subj');

        DB::table('labfee_subject')
            ->insert([
                'prospectusid' => $subj,
                'createdby' => auth()->user()->id,
                'createddatetime' => FinanceModel::getServerDateTime()
    ...
    public function labfeesubject_get(Request $request)
    {
        $subjects = DB::table('college_prospectus')
            ->where('deleted', 0)
            ->where('labunits', '!=', 0.0)
            ->groupBy('subjCode', 'subjDesc')
            ->get();

        $labfee_subject = DB::table('labfee_subject')
            ->join('college_prospectus', 'labfee_subject.prospectusid', '=', 'college_prospectus.id')
            ->select('college_prospectus.*', 'labfee_subject.prospectusid as labfeeid')
            ->get();

        $subject = $subjects->merge($labfee_subject);
        return $subject;
    ...
    public function labfee_search(Request $request)
    {
        ...
        $labfees = db::table('labfees')
            ...
            ->leftJoin('college_prospectus', 'labfees.subjid', '=', 'college_prospectus.id');
        if ($abbrv && strtolower($abbrv) != 'apmc') {
            $labfees = $labfees->where('syid', $syid)
                ->where('semid', $semid);
        }
        // ->where('syid', $syid)
        // ->where('semid', $semid)
        $labfees = $labfees->where(function ($q) use ($filter) {
            ...
            foreach ($labfee_setup as $setup) {
                ...
                $list .= '
                    <div id="' . $labfeeid . '" data-id="0" class="row mt-2 labfee_item">
                        ...
                        <div class="col-md-1">
                            <button class="btn btn-danger labfee_savesetup"  data-sort="' . $labfeeid . '"><i class="fas fa-trash-alt"></i></button>
                        </div>
                    </div>
                ';
            }
