...
class FinanceController extends Controller
{
	...
	public static function fixPaymentNo($id)
	{
		$paymentdetails = db::table('paymentsetupdetail')
			->where('paymentid', $id)
			->where('deleted', 0)
			->orderBy('duedate')
			->get();
	...
	public function saveFC(Request $request)
	{
		if ($request->ajax()) {
			...
			if (($levelid == 15 || $levelid == 14) || ($levelid >= 17 && $levelid <= 25)) {
				$semid = $request->get('semid');
			}

			$existingRecord = DB::table('tuitionheader')
				->where('description', $desc)
				->where('syid', $syid)
				->where('levelid', $levelid)
				->where('semid', $semid)
				->where('deleted', 0)
				->when($headerid, function ($query, $headerid) {
					return $query->where('id', '!=', $headerid);
				})
				->exists();

			if ($existingRecord) {
				return response()->json(['error' => 'Description already exists.'], 400);
			}

			if ($headerid > 0) {
				DB::table('tuitionheader')
					->where('id', $headerid)
					->update([
						'description' => $desc,
						'syid' => $syid,
			...
			return response()->json(['success' => 'Fees and Collection successfully saved.']);
		}
	}
	...
	public function getReceivableItems()
	{
		$receivableItems = FinanceModel::receivableitems();
		return response()->json($receivableItems);
	}
	...
	public function getStudLedger(Request $request)
	{
		...
		$info = db::table('studinfo')
			->select('sid', 'lastname', 'firstname', 'middlename', 'grantee', 'levelid')
			->where('id', $studid)
			->first();

		$studname = $info->lastname . ', ' . $info->firstname . ' ' . $info->middlename;
				...
				if (strpos($led->particulars, 'ADJ:') !== false) {
					$list .= '
							<tr data-id="' . $led->id . '">
					          ...
					          <td>
					          	' . $led->particulars . '
					          	<span class="text-sm text-danger adj_delete" style="cursor:pointer" data-studid="' . $studid . '" data-id="' . $led->ornum . '">
					          		<i class="far fa-trash-alt"></i>
					          	</span>
					          	<span class="text-sm text-info adj_view" style="cursor:pointer" data-studid="' . $studid . '" data-id="' . $led->ornum . '"><i class="far fa-eye"></i></span>
					          </td>
	...
	// 				$adjustmentClassIds = $adjustment->adjustmentitemsclassid; // Get class IDs from adjustment
	// 				$totalAdjustmentAmount = collect($adjustmentClassIds)->sum(function ($classid) use ($directPayments) {
	// 					return $directPayments[$classid] ?? 0; // Sum only existing values
	// 				});
	...
	public function getStudLedgerV2(Request $request)
	{
		...
		if ($enrolled) {
			...
			if ($enrolled) {
				...
				if ($enrolled) {
					...
					$enrolled = db::table('tesda_enrolledstud')
						->where('studid', $studid)
						// ->where('syid', $syid)
						->where(function ($q) use ($semid) {
							if ($semid == 3) {
		...
		$getLedgerQuery = DB::table('studledger')
			...
			->where(function ($query) {
				$query->where(function ($q) {
					...
					// ->orWhere('particulars', 'LIKE', 'Balance forwarded from%');
					// ->orWhere('particulars', 'LIKE', '%OLD ACCOUNTS FORWARDED FROM SY%')
					->orWhere('particulars', 'LIKE', '%OLD ACCOUNTS FORWARDED%');
			})
			->where(function ($query) {
				$query->where('particulars', 'NOT LIKE', 'ADJ:%');
			})
		...
		foreach ($getLedger as $lg) {
			if (strpos($lg->particulars, 'OLD ACCOUNTS FORWARDED FROM') !== false) {
				// Get payment schedule details with positive balance
				$history = DB::table('studpayscheddetail')
					->where('studid', $lg->studid)
					->where('balance', '>', 0)
					->where('deleted', 0)
					->where('particulars', 'NOT LIKE', '%OLD ACCOUNTS FORWARDED%')
					->orderBy('createddatetime', 'asc')
					->get();

				// Calculate merged balance for classid = 1
				$mergedBalance = $history->where('classid', 1)->sum('balance');

				// Get unique records by classid
				$history = $history->unique('classid');

				// Update balance for classid = 1 with the merged balance
				$history->each(function ($item) use ($mergedBalance) {
					if ($item->classid == 1) {
						$item->balance = $mergedBalance;
					}
				});

				// Assign the processed history to the ledger item
				$lg->history = $history ?? null;
			}
		}
		// dd($getLedger);
		...
		$finalLedger = $getLedger->merge($getDebitAdditional);
		// $finalLedger = collect($finalLedger)->where('particulars', 'LABORATORY FEE(PH 3)')->values();
		...
		$labfee_class = DB::table('labfee_setup')
			->where('deleted', 0)
			->where(function ($q) use ($levelid, $semid) {
				if ($levelid == 14 || $levelid == 15) {
		...
		foreach ($stud_discounts as $discount) {
			...
			$currentAmount = isset($directPayments[$classid]) ? $directPayments[$classid] : 0;

			// Add the discount amount
			$directPayments[$classid] = $currentAmount + $discamount;
		}
		// Step 4: Merge payments correctly (sum instead of overwrite)
		$mergedPayments = collect($directPayments)
			->union($adjustmentPayments)
		...
		// 		$ledger->particular_items = $tuitiondetails;
		// 	} if($ledger->classid == 4){
		// 		$lab = DB::table('studpayscheddetail')
		// 			->where('studpayscheddetail.studid', $studid)
		// 			->where('studpayscheddetail.syid', $syid)
		// 			->where(function($q) use($levelid, $semid) {
		// 				if ($levelid == 14 || $levelid == 15) {
		...
		// 			->where('studpayscheddetail.deleted', 0)
		// 			->where('studpayscheddetail.classid', $ledger->classid)
		// 			->where('studpayscheddetail.particulars', $ledger->particulars)
		// 			->first();

		// 		$ledger->totalpaid = $lab->amountpay ?? 0;
		...
		foreach ($finalLedger as $ledger) {
			$ledger->particular_items_count = 0;
			$ledger->particular_items = [];

			if ($ledger->classid != 15) {
				$ledger->totalpaid = $mergedPayments[$ledger->classid] ?? 0;
			}

			if ($labfee_class) {
				if ($ledger->classid == $labfee_class->classid) {
					$lab = DB::table('studpayscheddetail')
						->where('studpayscheddetail.studid', $studid)
						->where('studpayscheddetail.syid', $syid)
						->where('studpayscheddetail.particulars', $ledger->particulars)
			...
			if ($ledger->classid == 1) {
				...
				$tuitiondetails = $tuitiondetails->map(function ($item) {
					if (!empty($item->duedate)) {
						$month = strtoupper(date('F', strtotime($item->duedate)));
						$item->tuition_month = "TUITION $month";
					} else {
						$item->tuition_month = $item->particulars;
					}
					return $item;
				});
				...
				$ledger->particular_items_count = count($tuitiondetails);
				$ledger->particular_items = $tuitiondetails;
			} else if ($ledger->classid == 15) {
				$oldaccounts = DB::table('studpayscheddetail')
					->where('studpayscheddetail.studid', $studid)
					->where('studpayscheddetail.syid', $syid)
					->where(function ($q) use ($levelid, $semid) {
						if ($levelid == 14 || $levelid == 15) {
				...
				$ledger->totalpaid = $oldaccounts->amountpay ?? 0;

			}
			// else {

			// 	$studledgeritemized = DB::table('studledgeritemized')
			// 		->select('studledgeritemized.*', 'items.description')
			// 		->join('items', 'studledgeritemized.itemid', 'items.id')
			// 		->where('studledgeritemized.studid', $studid)
			// 		->where('studledgeritemized.syid', $syid)
			// 		->where(function($q) use($levelid, $semid){
			...
			// 	if (count($studledgeritemized) == 0 && isset($ledger->adjustmentitemsclassid) ) {
			// 		$studledgeritemized = DB::table('studledgeritemized')
			// 			->select('studledgeritemized.*', 'items.description')
			// 			->join('items', 'studledgeritemized.itemid', 'items.id')
			// 			->where('studledgeritemized.studid', $studid)
			...
			// 			->where('items.deleted', 0)
			// 			->where('studledgeritemized.deleted', 0)
			// 			->whereIn('studledgeritemized.classificationid', $ledger->adjustmentitemsclassid)
			// 			->get();
			// 	}
			// 	if($ledger->totalpaid == 0){ // ako ni gituyo kay example naay changes sa fees classification dayun naa nay mga item nga gibayaran atong daan nga classid pero giusab same lang ang mga item
			// 		$ledger->totalpaid = collect($studledgeritemized)->sum('totalamount');
			// 	}
	...
	public function history_list(Request $request)
	{
		...
		$notrecievable = DB::table('items')
			->where('isreceivable', 0)
			// ->where('classcode', 4)
			->where('deleted', 0)
			->whereRaw("itemcode NOT REGEXP '[A-Za-z]'") // This excludes itemcodes with any letters
			->get();

		foreach ($getLedger as $ledger) {
			$or = DB::table('chrngtrans')
				->where('id', $ledger->transid)
				->first();

			if ($or) {

				$or_details = DB::table('chrngcashtrans')
					->where('transno', $or->transno)
					->where('deleted', 0)
					->orderBy('kind', 'desc')
					->get();

				$desc = collect($or_details)->pluck('particulars');

				$notrecievable_items = collect($notrecievable)->whereIn('description', $desc)->count();

				$ledger->cashier_items_count = $or_details->where('itemprice', 0.00)->count();
				$ledger->notrecievable_count = $notrecievable_items;
			} else {
				$ledger->cashier_items_count = 0;
				$ledger->notrecievable_count = 0;
			}
	...
	public function view_receipts(Request $request)
	{
		...
		if ($or) {
			...
			$totalitemprice = $or_details->sum('itemprice');
			$totalamount = $or_details->sum('amount');
		}

		return response()->json([
			'or_details' => $or_details,
			'total_amount' => $totalamount,
			'total_itemprice' => $totalitemprice
		]);
	}
					...
					$lists .= '
						<div class="col-md-3">
	            			<div class="card tui-card" data-id="' . $tui->id . '" style="cursor: pointer;">
	              				...
	              				<div class="card-body">
	                				<div>LEVEL: <span class="text-bold">' . strtoupper($tui->levelname) . '</span></div>
					...
					$lists .= '
						<div class="col-md-4">
	            			<div class="card tui-card" data-id="' . $tui->id . '" style="cursor: pointer;">
	              				...
	              				<div class="card-body">
	                				<div>LEVEL: <span class="text-bold">' . $tui->levelname . '</span></div>
	              				</div>
	            			</div>
	          			</div>
					';
	...
	public function onlinepaymentlist(Request $request)
	{
		...
		$lists = db::table('onlinepayments')
			...
			->select(db::raw('onlinepayments.*, concat(lastname, ", ", firstname) as fname, middlename, levelname, refnum, paymenttype.description, amount, semester, sydesc, levelid, paymentdate'))
			...
			// ->where('isapproved', 0)
			->where('syid', $syid)
			->where(function ($q) use ($filter_status) {
				if ($filter_status != 'all') {
					$q->where('isapproved', $filter_status);
				}
			})
		...
		foreach ($lists as $list) {
			...
			if ($list->isapproved == 0) {
				$status = 'PENDING';
				$color = 'text-default';
			} elseif ($list->isapproved == 1) {
				$status = 'APPROVED';
				$color = 'text-primary';
			} elseif ($list->isapproved == 2) {
				$status = 'DISAPPROVED';
				$color = 'text-danger';
			} elseif ($list->isapproved == 5) {
				$status = 'COMPLETED';
				$color = 'text-success';
			}

			if ($action != 'print') {
				$sysem = $list->sydesc . ' - ' . $list->semester;
				...
				$payments .= '
                    <tr style="cursor: pointer;" data-id="' . $list->id . '">
                        ...
                        <td class="' . $color . '">' . $list->levelname . '</td>
                        <td style="width: 30%; text-align: center;">' . $list->description . '</td>
						<td class="' . $color . '">' . $list->refnum . '</td>
		...
		if ($action != 'print') {
			...
			$pdf = PDF::loadView('/finance/reports/pdf/pdf_olreport', $data)->setPaper('legal', 'portrait');
	...
	public function loadTerminal(Request $request)
	{
		if ($request->ajax()) {
			...
			foreach ($terminals as $terminal) {
				$list .= '
					<tr id="' . $terminal->id . '">
			            <td style="vertical-align:middle" class="text-lg">' . $terminal->description . '</td>
			            <td style="vertical-align:middle" class="text-lg">' . $terminal->name . '</td>
			        	<td>
			        		<button class="btn btn-sm btn-outline-secondary btn-sm btn-block oclear" data-id="' . $terminal->id . '">Clear</button>
			        		<button class="btn btn-sm btn-outline-danger btn-sm btn-block oremove" data-id="' . $terminal->id . '">Remove</button>
			        	</td>
			        </tr>
				';
			}
	...
	public function printFC(Request $request, $headid)
	{
		// Fetch tuition header info with all required columns
		$headinfo = DB::table('tuitionheader')
			->join('gradelevel', 'tuitionheader.levelid', '=', 'gradelevel.id')
			->leftJoin('sh_strand', 'tuitionheader.strandid', '=', 'sh_strand.id')
			->leftJoin('college_courses', 'tuitionheader.courseid', '=', 'college_courses.id')
			->where('tuitionheader.id', $headid)
			->select(
				'tuitionheader.*', // Select all columns from tuitionheader
				'gradelevel.levelname', // Level Name
				'sh_strand.strandname', // Strand Name
				'college_courses.courseabrv as courseabrv' // Course Name
			)
			->first();

		// Handle null case
		if (!$headinfo) {
			abort(404, 'Tuition header not found.');
		}
		...
		$data = [
			...
			'courseid' => $headinfo->courseid ?? '',
			'paymentplan' => $paymentplan ?? '',
			'levelname' => $headinfo->levelname ?? 'N/A',
			'strandname' => $headinfo->strandname ?? '',
			'courseabrv' => $headinfo->courseabrv ?? ''
		];
		...
		$details = DB::table('tuitiondetail')
			->join('itemclassification', 'tuitiondetail.classificationid', '=', 'itemclassification.id')
			->leftJoin('paymentsetup', 'tuitiondetail.pschemeid', '=', 'paymentsetup.id')
			->select(
				'tuitiondetail.id',
				'tuitiondetail.headerid',
				'itemclassification.description as classdesc',
				'paymentsetup.paymentdesc',
				'tuitiondetail.amount'
			)
			->where('tuitiondetail.headerid', $headid)
			->where('tuitiondetail.deleted', 0)
			->get();

		// Fetch and structure tuition items
		$output = [];
		foreach ($details as $detail) {
			$items = DB::table('tuitionitems')
				->join('items', 'tuitionitems.itemid', '=', 'items.id')
				->select('tuitionitems.id', 'items.description', 'tuitionitems.amount')
				->where('tuitionitems.tuitiondetailid', $detail->id)
				->where('tuitionitems.deleted', 0)
				->get();

			$output[$detail->classdesc] = $items;
		}

		// Load PDF view
		$pdf = PDF::loadView('finance.statementofaccount.schoollfees', [
			'details' => $details,
			'feesData' => $output,
			'information' => $data
		]);

		return $pdf->stream('schoolfees.pdf');
	}
	...
	public function deletecolFCdetail(Request $request)
	{
		...
		if (Hash::check($pword, auth()->user()->password)) {
			// Ensure we keep $detailid unchanged
			$updateDetail = DB::table('tuitiondetail')
				->where('id', $detailid)
				->update([
					'deleted' => 1,
					'deletedby' => auth()->user()->id,
			...
			// Use the original $detailid here
			$updateItems = DB::table('tuitionitems')
				->where('tuitiondetailid', $detailid)
				->update([
					'deleted' => 1,
					'deletedby' => auth()->user()->id,
	...
	public function searchOLReceipt(Request $request)
	{
		if ($request->ajax()) {
			...
			foreach ($olpay as $pay) {
				...
				$list .= '
					<tr data-id="' . $pay->id . '" data-src="' . $url . '/' . $pay->picurl . '">
			...
			foreach ($olpay as $pay) {
				...
				$list .= '
					<tr data-id="' . $pay->id . '" data-src="' . $url . '/' . $pay->picurl . '">
						...
						<td class="' . $statClass . ' ol-item">' . $status . '</td>
						<td>
							<a href="' . $url . '/' . $pay->picurl . '" download="' . $url . '/' . $pay->id . '.jpg" class="btn btn-secondary dl"><i class="fas fa-download"></i></a>
							</td>
					</tr>
				';
			}
	...
	public function searchadj(Request $request)
	{
		...
		$desc = $request->get('desc');
		$cbocreditdebit = $request->get('cbocreditdebit');

		$datearray = [$datefrom . ' 00:00', $dateto . ' 23:59'];

		$query = DB::table('adjustments')
			->where('adjustments.syid', $syid)
			->where('adjustments.semid', $semid)
			->whereBetween('adjustments.createddatetime', $datearray)
			->where('adjustments.deleted', 0)
			->join('adjustmentdetails', 'adjustments.id', '=', 'adjustmentdetails.headerid')
			->join('studinfo', 'studinfo.id', '=', 'adjustmentdetails.studid')
			->join('gradelevel', 'adjustments.levelid', '=', 'gradelevel.id');

		// Apply level filter if provided
		if (!empty($level)) {
			$query->where('gradelevel.levelname', $level);
		}

		if (!empty($cbocreditdebit)) {
			if ($cbocreditdebit == '1') {
				$query->where('isdebit', 1);
			} else {
				$query->where('iscredit', 1);
			}
		}
		...
		if (!empty($desc)) {
			...
			$query->whereIn('adjustments.description', $desc);
		}
		...
		$adjustments = $query
			->select(
				'adjustments.*',
				'gradelevel.levelname',
				DB::raw('CONCAT(studinfo.lastname, ", ", studinfo.firstname, " ", studinfo.middlename) AS studname')
			)
			->orderBy('adjustments.id', 'DESC')
			->get();

		// Get unique descriptions for dropdown
		$uniqueDescriptions = DB::table('adjustments')
			->where('adjustments.syid', $syid)
			->where('adjustments.semid', $semid)
			->whereBetween('adjustments.createddatetime', $datearray)
			->where('adjustments.deleted', 0)
			->where(function ($query) use ($cbocreditdebit) {
				if (!empty($cbocreditdebit)) {
					if ($cbocreditdebit == '1') {
						$query->where('isdebit', 1);
					} else {
			...
			->select('adjustments.description')
			->distinct()
			->orderBy('adjustments.description')
			->pluck('description')
			->toArray();
		...
		foreach ($adjustments as $adj) {
			$acc = $adj->isdebit == 1 ? 'DEBIT' : 'CREDIT';
			switch ($adj->adjstatus) {
				case 'APPROVED':
			...
			$list .= '
				<tr data-id="' . $adj->id . '" class="' . $statusClass . '">
					<td>' . $adj->studname . '</td>
					<td class="cbolevel">' . $adj->levelname . '</td>
					<td class="cbodesc">' . $adj->description . '</td>
		...
		return response()->json([
			'list' => $list,
			'desc' => $uniqueDescriptions,
		]);
	}

	public function exportAdj(Request $request)
	{
		$syid = $request->get('syid');
		$semid = $request->get('semid');
		$datefrom = $request->get('datefrom');
		...
		$datearray = array($datefrom . ' 00:00', $dateto . ' 23:59');
		...
		if (!empty($desc)) {
			...
			if (is_array($descArray)) {
				$query->whereIn('adjustments.description', $descArray);
			}
		}
		...
		$adjustments = $query
			...
			->orderBy('gradelevel.sortid', 'ASC')
			->get();
		// dd($adjustments, $request->all(), $desc);
		...
		$spreadsheet->getProperties()
			->setCreator("Mater Dei Academy of Tagaytay")
			->setTitle("Adjustments List");

		// Main title (like in your image)
		...
		$sheet->setCellValue('A2', 'LIST OF SCHOLARS From: ' . $datefrom . ' To: ' . $dateto);
		...
		// Header row (now with No. column)
		$headerRow = 4;
		$headers = ['No.', 'NAME', 'LEVEL', 'DESCRIPTION', 'AMOUNT', 'TYPE'];
		...
		$headerStyle = [
			...
			'fill' => [
				...
				'startColor' => ['rgb' => '4472C4'], // Dark blue like in your image
			],
			'borders' => [
				'allBorders' => [
		...
		foreach ($adjustments as $adj) {
			...
			$data[] = [
				...
				$adj->levelname,
				$adj->description,
				$adj->amount, // Don't format here - we'll apply number format later
				$acc,
			];
		}
		...
		// Apply general style to all data cells
		$sheet->getStyle('A' . ($headerRow + 1) . ':F' . ($currentRow + count($data)))->applyFromArray($dataStyle);

		// Center align all columns except NAME (column B)
		$centerStyle = [
			'alignment' => [
				'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER,
			],
		...
		// Center align columns: A (No.), C (LEVEL), D (DESCRIPTION), E (AMOUNT), F (TYPE)
		$sheet->getStyle('A' . ($headerRow + 1) . ':A' . ($currentRow + count($data)))->applyFromArray($centerStyle);
		$sheet->getStyle('C' . ($headerRow + 1) . ':F' . ($currentRow + count($data)))->applyFromArray($centerStyle);

		// Format amount column (column E)
		$sheet->getStyle('E' . ($headerRow + 1) . ':E' . ($currentRow + count($data)))
			->getNumberFormat()
			->setFormatCode('#,##0.00');
		...
		$filename = 'adjustments_' . date('YmdHis') . '.xlsx';
	...
	public function cashtranssearch(Request $request)
	{
		if ($request->ajax()) {
			...
			$trans_mode = $request->get('trans_mode');
			...
			if ($gradelevel == 0) {
				$gradelevelids = DB::table('gradelevel')
					->where('deleted', 0)
					->when($acadprog != 0, function ($query) use ($acadprog) {
						return $query->where('acadprogid', $acadprog);
					})
					->pluck('id') // Get only the IDs
					->toArray();  // Convert collection to array
			} else {
				$gradelevelids = [(int) $gradelevel]; // Convert single value into an array
			}
			...
			$transactions = DB::table('chrngtrans')
				->select(
					...
					'chrngtrans.studid',
					'chrngtrans.particulars',
					'chrngtrans.terminalno',
					'chrngtrans.transby',
					'chrngtrans.studname',
					'chrngtrans.posted',
					'chrngtrans.sid',
				...
				->where(function ($query) use ($search) {
					$query->where('ornum', 'like', '%' . $search . '%')
						->orWhere('studname', 'like', '%' . $search . '%');
				})
				->whereBetween('transdate', [$from, $to])
				// ->where(function ($q) use ($gradelevel, $acadprog) {
				// 	if ($acadprog == 3 && $gradelevel == 0) { 
				// 		// If acadprog is 3 (elementary) and gradelevel is 0, get grades 1 to 6
				// 		$q->whereBetween('studinfo.levelid', [1, 6]);
				// 	} elseif ($gradelevel !== '' && $gradelevel != 0) { 
				// 		// Otherwise, filter by selected grade level
				// 		$q->where('studinfo.levelid', '=', $gradelevel);
				// 	}
				// })
				->whereIn('studinfo.levelid', $gradelevelids)
				// ->where(function ($q) use ($acadprog) {
				// 	if ($acadprog !== '' && $acadprog != 0) { 
				// 		$q->where('studinfo.acadprogid', '=', $acadprog);
				// 	}
				// })
				->where(function ($q) use ($trans_mode) {
					$q->where('isonlinepay', $trans_mode);
				})
				->where(function ($q) use ($terminalid) {
					if ($terminalid > 0) {
						$q->where('terminalno', $terminalid);
			...
			foreach ($transactions as $trans) {
				...
				if ($trans->posted == 0 && $trans->cancelled == 0) {
					...
					<tr>
						...
						<td>' . $trans->ornum . '</td>
						<td>' . strtoupper($trans->studname) . '</td>
						<td>' . strtoupper($trans->levelname) . '</td>
						<td class="text-center">' . number_format($trans->amountpaid, 2) . '</td>
						<td></td>
						<td>' . strtoupper($trans->name) . '</td>
						<td>' . $trans->description . '</td>
						<td>
					...
					<tr>
						...
						<td class="text-danger"><del>' . strtoupper($trans->studname) . '</del></td>
						<td>' . strtoupper($trans->levelname) . '</td>
						<td class="text-center text-danger"><del>' . number_format($trans->amountpaid, 2) . '</del></td>
	...
	public function printcashtrans($terminalid, $dtfrom, $dtto, $filter, $paytype)
	{
		...
		$transactions = db::table('chrngtrans')
			->select('chrngtrans.id', 'ornum', 'transdate', 'totalamount', 'amountpaid', 'studid', 'particulars', 'terminalno', 'transby', 'studname', 'posted', 'sid', 'glevel', 'name', 'cancelled', 'cancelledremarks', 'paymenttype.description')
			->join('users', 'chrngtrans.transby', '=', 'users.id')
			->join('paymenttype', 'chrngtrans.paytype', '=', 'paymenttype.description')
	...
	public function transviewdetail(Request $request)
	{
		...
		// 	->select('chrngtrans.id', 'sid', 'studname', 'glevel', 'ornum', 'items', 'transdate', db::raw('SUM(amount) as amount'), db::raw('SUM(amountpaid) as amountpaid'), db::raw('SUM(qty) as qty'), db::raw('AVG(itemprice) as itemprice'), 'cancelled')
		// 	->leftJoin('chrngtransdetail', 'chrngtrans.id', '=', 'chrngtransdetail.chrngtransid')
		...
		$chrngtrans = db::table('chrngtrans')
			...
			->select(
				...
				'chrngtrans.ornum',
				'chrngtrans.transdate',
				'chrngcashtrans.itemprice',
				'cancelled',
			)
			// ->groupBy('chrngcashtrans.transno')
			->get();
		// return $chrngtrans;
		if (count($chrngtrans) > 0) {
			foreach ($chrngtrans as $trans) {
				...
				$totalamount += $trans->itemprice;

				$list .= '
	        			<tr>
	        				<td>' . $trans->particulars . '</td>
	        				<td class="text-right">' . number_format($trans->itemprice, 2) . '</td>
	        			</tr>
	        		';
			}
	...
	// public function printpaysched($id)
	// {
	// 	// if($request->ajax())
	// 	// {
	// 		$tuitionid = $id;
	...
	// 		$pdf = PDF::loadView('/finance/printcashtrans', $tuitionheader)->setPaper('legal','portrait');
	// 		$pdf->getDomPDF()->set_option("enable_php", true);
	// 		return $pdf->stream('cashiertransactions.pdf');
	// 		// return $test_data;
	// 	// }
	...
	public function printpaysched($id)
	{
		...
		$sydescription = request('sydescription');
		$grantedescription = request('grantedescription');

		$paymentsched = DB::table('paymentsched')
			->get();

		// Group by 'duedate' (month), sort by sortid
		$grouped = collect($paymentsched)
			->groupBy('duedate')
			->sortBy(function ($group) {
				return $group->first()->sortid;
			});

		// Transform the grouped collection to desired format
		$formatted = $grouped->map(function ($items, $month) {
			return [
				'month' => strtolower($month),
				'data' => $items->values(), // reset keys
			];
		})->values(); // reset keys for the outer array

		// Now $formatted is a collection of arrays with month and data keys

		// For debugging or API response, you can return JSON:
		// return response()->json($formatted);

		// Or pass it to your view for PDF:
		$paymentsched = $formatted;
		$pdf = PDF::loadView('finance/reports/pdf_printfees', compact('paymentsched', 'description', 'sydescription', 'grantedescription'))
			...
			foreach ($chrngtrans as $trans) {
				...
				$ledgeritemized = db::select(
					...
					WHERE `studid` = ?
						...
						AND `deleted` = 0
						AND `totalamount` < itemamount',
					[$trans->studid, $trans->syid, $trans->semid, $trans->classid]
				);
	...
	public function ledgeradj_loadadjinfo(Request $request)
	{
		...
		$class = '<option value="0">CLASSIFICATION</option>';
		foreach ($scheddetail as $sched) {
			$class .= '<option value="' . $sched->classid . '">' . $sched->particulars . '</option>';
		}
		...
		return response()->json([
			...
			'class' => $class,
			'granteeid' => $stud->granteeid,
			'mol' => $stud->mol,
			'acadprogid' => $stud->acadprogid,
			'studid' => $stud->studid,
			'levelid' => $stud->levelid,
		]);
	}
	...
	public function ledgeradj_debitsave(Request $request)
	{
		try {
			...
			$student = DB::table('studinfo')->where('id', $data['studid'])->first();
			$data['levelid'] = $student ? $student->levelid : null;

			$data['isdebit'] = 1;
			$data['createdby'] = auth()->user()->id;
			$data['createddatetime'] = FinanceModel::getServerDateTime();
			$data['adjstatus'] = 'APPROVED';

			unset($data['studid']);
			$adjid = DB::table('adjustments')->insertGetId($data);
			$data['studid'] = $request->studid;

			$adjdetailid = DB::table('adjustmentdetails')->insertGetId([
				'headerid' => $adjid,
				'studid' => $data['studid'],
				'createdby' => auth()->user()->id,
				'createddatetime' => FinanceModel::getServerDateTime()
			...
			DB::table('adjustments')->where('id', $adjid)->update([
				'refnum' => 'ADJ' . date('Y') . sprintf('%05d', $adjid),
				'updatedby' => auth()->user()->id,
				'updateddatetime' => FinanceModel::getServerDateTime()
			]);
			...
			DB::table('studledger')->insert([
				...
				'particulars' => 'ADJ: ' . $data['description'],
				'amount' => $data['amount'],
				'ornum' => $adjdetailid,
				'pschemeid' => $data['mop'],
				'createdby' => auth()->user()->id,
			...
			if (!empty($items)) {
				DB::table('adjustmentitems')
					->whereIn('id', $items)
					->update(['detailid' => $adjdetailid]);

				$itemized = DB::table('adjustmentitems')->whereIn('id', $items)->get();

				foreach ($itemized as $item) {
					$existingItem = DB::table('studledgeritemized')
						->where('studid', $data['studid'])
						->where('itemid', $item->itemid)
						->where('classificationid', $item->classid)
						->where('syid', $data['syid'])
						->where('deleted', 0)
						->first();

					if ($existingItem) {
						DB::table('studledgeritemized')->where('id', $existingItem->id)->update([
							'itemamount' => $existingItem->itemamount + $item->amount
						]);
					} else {
	...
	public function ledgeradj_debitsave_backup(Request $request)
	{
		try {
			// return $request->all();
			$studid = $request->get('studid');
			...
			$semid = $request->get('semid');
			$mol = $request->get('mol');
			$granteeid = $request->get('granteeid');
			$acadprogid = $request->get('acadprogid');
			$items = $request->has('items') ? $request->get('items') : [];

			$student = DB::table('studinfo')->where('id', $studid)->first();
			$levelid = $student ? $student->levelid : null;
			...
			$adjdetailid = db::table('adjustmentdetails')
				->insertGetId([
					'headerid' => $adjid,
					'studid' => $studid,
					'createdby' => auth()->user()->id,
			...
			//-----------------studledgeritemized--------------//
			if ($items != '') {
				$itemized = db::table('adjustmentitems')
					->whereIn('id', $items)
					->get();

				foreach ($itemized as $i) {
					db::table('adjustmentitems')
						->where('id', $i->id)
						->update([
							'detailid' => $adjdetailid
						]);

					$checkitemized = db::table('studledgeritemized')
						->where('studid', $studid)
						->where('itemid', $i->itemid)
						->where('classificationid', $i->classid)
						->where('syid', $syid)
						->where(function ($q) use ($levelid, $semid) {
							if ($levelid == 14 || $levelid == 15) {
								if (db::table('schoolinfo')->first()->shssetup == 0) {
					...
					if ($checkitemized == 0) {
						...
						db::table('studledgeritemized')
							->where('id', $checkitemized->id)
							->update([
								'itemamount' => $checkitemized->itemamount + $i->amount
							]);
					}
				}
			} else {
			...
			$_adjitems = db::table('adjustmentitems')
				->select(db::raw('classid, sum(amount) as amount, description'))
				->join('itemclassification', 'adjustmentitems.classid', '=', 'itemclassification.id')
				->where('detailid', $adjdetailid)
				->where('adjustmentitems.deleted', 0)
				->groupBy('classid')
				->get();

			if (count($_adjitems) > 0) {
				foreach ($_adjitems as $_item) {
					$schedid = db::table('studpayscheddetail')
						->insertGetId([
							'studid' => $studid,
							'syid' => $syid,
							...
							'particulars' => $_item->description,
							'amount' => $_item->amount,
							'balance' => $_item->amount,
							'createdby' => auth()->user()->id,
							'createddatetime' => FinanceModel::getServerDateTime()
						]);
				}
	...
	public function adjitems_append(Request $request)
	{
		$studid = $request->get('studid');
		$itemid = $request->get('itemid');
		$amount = $request->get('amount');
		$adjtype = $request->get('adjtype');
		$curamount = str_replace(',', '', $request->get('curamount'));

		if ($curamount == '') {
			$curamount = 0;
		}

		$btnclass = ($adjtype == 'credit') ? 'adjitem_removeitem_credit' : 'adjitem_removeitem';

		$item = DB::table('items')
			->select(DB::raw('items.id, items.description AS itemdesc, itemclassification.description AS classdesc, classid'))
		...
		$adjitemid = DB::table('adjustmentitems')->insertGetId([
			'itemid' => $itemid,
			'classid' => $classid,
			'amount' => $amount,
			'createddatetime' => FinanceModel::getServerDateTime()
	...
	// 		$check = db::table('adjustments')
	// 			->where('description', $adjDesc)
	// 			->where('classid', $classid)
	// 			->where('syid', $syid)
	// 			->where('semid', $semid)
	// 			->where('levelid', $levelid)
	// 			->where('createdby', auth()->user()->id)
	// 			->where('createddatetime', '>=', Carbon::now()->startOfDay())
	// 			->where('createddatetime', '<=', Carbon::now()->endOfDay())
	// 			->first();

	// 	if($check != null){
	// 		return response()->json([
	// 			['status' => 2, 'message' => 'Already Exist']
	// 		]);
	// 	}
	// 	else{
	// 		$adjid = db::table('adjustments')
	...
	public function ledgeradj_delete(Request $request)
	{
		...
		$remainingDetails = DB::table('adjustmentdetails')
			->where('headerid', $dataid)
			->where('deleted', 0)
			->count();
	...
	public function ledgeroa_delete(Request $request)
	{
		...
		$ledger = db::table('studledger')
			->select('id', 'particulars', 'classid', 'createddatetime', 'syid', 'semid', 'amount')
			->where('id', $dataid)
			->first();
		...
		db::table('studledger')
			->where('classid', $oaclassid)
			// ->where('payment', $ledger->amount)
			->where('createddatetime', $ledger->createddatetime)
			->where('studid', $studid)
			->where('deleted', 0)
			->update([
				'deleted' => 1,
	...
	public function ledger_studyload(Request $request)
	{
		...
		$droppedSubjects = DB::table('college_loadsubject')
			...
			->join('users', 'college_loadsubject.deletedby', '=', 'users.id')
		...
		foreach ($droppedSubjects as $dropped) {
			...
			$droppedList .= '
					<tr class="text-danger"> <!-- Red text for dropped subjects -->
						...
						<td>' . $dropped->subjDesc . ' (Dropped) <br><small class="text-muted">Dropped by: ' . $dropped->droppedby . '</small></td>
	...
	public function readytoenroll_load(Request $request)
	{
		...
		$data = FinanceModel::readytoenroll($syid, $semid);
	...
	public function processpayment(Request $request)
	{
		...
		// Fetch payment data
		$info = $this->paydata2($dataid);

		$getStudledger = DB::table('studledger')
			...
			->whereRaw("particulars NOT LIKE '%ADJ%'")
			->select(
				'itemclassification.id as classificationid',
				'studledger.particulars',
				'studledger.amount',
				'studledger.classid',
				'itemclassification.description as classification'
			)
			->get();


		// $getStudledgerADJ = DB::table('studledger')
		// 	->join('itemclassification', 'studledger.classid', '=', 'itemclassification.id')
		// 	->join('adjustments', 'studledger.ornum', '=', 'adjustments.id')
		// 	->where('studledger.studid', $studid)
		// 	->where('studledger.syid', $syid)
		// 	->where('studledger.deleted', 0)
		// 	->where('studledger.payment', '<=', 0)
		// 	->whereRaw("particulars LIKE '%ADJ%'")
		// 	->select(
		// 		'itemclassification.id as classificationid',
		// 		'studledger.particulars',
		// 		'studledger.amount',
		...
		$groupedData = [];
		foreach ($getStudledger as $ledger) {

			$items = DB::table('studledgeritemized')
				->where('studledgeritemized.deleted', 0)
				->join('items', 'studledgeritemized.itemid', '=', 'items.id')
				->where('studledgeritemized.studid', $studid)
				->where('studledgeritemized.classificationid', $ledger->classificationid)
				->select(
					'items.id as item_id',
					'items.description as item_description',
					'studledgeritemized.totalamount',
					'studledgeritemized.classificationid',
					'studledgeritemized.id as itemizedid',
					'studledgeritemized.itemamount'
				)
				->get();
			...
			foreach ($items as $item) {
				// $paidAmount = DB::table('onlinepaymentlogs')
				// 	->where('studid', $studid)
				// 	->where('studledgerid', $item->itemizedid)
				// 	->sum('totalamount');

				// $item->remaining_amount = max(0, $item->totalamount - $paidAmount);
				$item->remaining_amount = $item->itemamount - $item->totalamount;

				// $monthlyitems = DB::table('studpayscheddetail')
				// 	->where('studpayscheddetail.deleted', 0)
				// 	->where('studid', $studid)
				// 	->where('classid', $item->classificationid)
				// 	->where('studpayscheddetail.syid',$syid)
				// 	->get();

				// if(count($monthlyitems) > 0) {
				// 	$item->ismonthly = 1;
				// 	$item->itemmonthly = $monthlyitems;
				// }else{
				// 	$item->ismonthly = 0;
				// 	$item->itemmonthly = [];
				// }
			...
			if (!isset($groupedData[$ledger->classificationid])) {
				$groupedData[$ledger->classificationid] = [
					'classificationid' => $ledger->classificationid,
					'classification' => $ledger->classification,
					'details' => []
				];
			}

			$groupedData[$ledger->classificationid]['details'][] = [
			...
			// }
			$getStudledgerADJ = DB::table('studledger')
				->where('studledger.classid', $ledger->classificationid)
				->join('itemclassification', 'studledger.classid', '=', 'itemclassification.id')
				->join('adjustments', 'studledger.ornum', '=', 'adjustments.id')
				->where('studledger.studid', $studid)
				->where('studledger.syid', $syid)
				...
				->select(
					...
					'itemclassification.description as classification',
					'adjustments.description',
					'adjustments.id as adjID'

				)
				->get();
			// dd($getStudledgerADJ);

			if (count($getStudledgerADJ) > 0) {
				foreach ($getStudledgerADJ as $adj) {
					// dd($adj);
					// if (!isset($groupedData[$adj->classificationid])) {
					// 	$groupedData[$adj->classificationid] = [
					// 		'classificationid' => $adj->classificationid,
					...
					$paysched = DB::table('studpayscheddetail')
						->where('tuitiondetailid', $adj->adjID)
						->first();
					// dd($paysched);

					$paysched->remaining_amount = $paysched->amount - $paysched->amountpay;
					// dd($paysched);

					$groupedData[$adj->classificationid]['details'][] = [
						'particulars' => $adj->particulars,
						'amount' => $adj->amount,
						'totalamount' => $paysched->amount,
						'classid' => $adj->classid,
						'id' => $adj->adjID,
						'remaining_amount' => $paysched->remaining_amount,
						'isadj' => 1,
						'items' => []
					];
				}
		...
		$paymentLogs = DB::table('onlinepaymentlogs')
			->join('items', 'onlinepaymentlogs.item_id', '=', 'items.id')
			->where('onlinepaymentlogs.studid', $studid)
			->where('onlinepaymentlogs.onlinepayment_id', $dataid)
			->select(
				'onlinepaymentlogs.item_id',
				'items.description',
				'onlinepaymentlogs.totalamount',
				'onlinepaymentlogs.remaining'
			)
			->get();
		...
		return response()->json([
			'info' => $info,
			'ledger' => array_values($groupedData),
	...
	public function onlinepayment(Request $request)
	{
		...
		$refnum = preg_replace('/[^0-9]/', '', $data['refnum']);
		$paymentDate = $data['paymentDate'];
		$paymentNo = $data['paymentNo'];
		$syid = FinanceModel::getSYID();
		$semid = FinanceModel::getSemID();
		$remainingAmount = floatval($data['remainingAmount'] ?? 0);
		$totalAmount = floatval($request->get('totalAmount') ?? 0);
		$paymentprocess = [];
		$totalAmountPaid = 0;
		...
		if (!isset($data['items']) || !is_array($data['items']) || empty($data['items'])) {
			return response()->json([
				'status' => 'error',
				'message' => 'No items found for payment processing.'
			], 400);
		}
		...
		$desc = collect($data['items'])->pluck('classid')->unique();
		...
		$desc_string = $itemclassification->pluck('description')->implode('/');

		// Get student information
		$studinfo = DB::table('studinfo')
			->select('sid', 'levelname', 'sectionname', 'studstatus', 'levelid', 'lastname', 'firstname', 'middlename')
			->join('gradelevel', 'studinfo.levelid', '=', 'gradelevel.id')
			->where('studinfo.id', $studid)
			->first();

		if ($studinfo) {
			$studinfo->fullname = $studinfo->lastname . ', ' . $studinfo->firstname;

			if (!empty($studinfo->middlename)) {
				$studinfo->fullname .= ' ' . $studinfo->middlename;
			}
			...
			// Determine academic program
			if ($levelid == 14 || $levelid == 15) {
				$acadprogid = 5;
			} elseif ($levelid >= 17 && $levelid <= 25) {
				$acadprogid = 6;
			} elseif ($levelid == 2 || $levelid == 3 || $levelid == 4) {
				$acadprogid = 2;
			} else {
				$acadprogid = 3;
			}
		...
		// Insert into 'chrngtrans' table
		$chrngtransid = DB::table('chrngtrans')
			->insertGetId([
				'ornum' => $refnum,
				'transdate' => $transdate,  // Ensure it's a valid date/time format
		...
		// Insert into 'studledger' if transaction is successful
		if (!empty($chrngtransid)) {

			foreach ($data['items'] as $item) {
				$classid = $item['classid'];
				$amount = floatval($item['amount']);
				$itemizedid = $item['itemizedid'] ?? null;
				$isadj = $item['isadj'];
				...
				$studitemized = DB::table('studledgeritemized')
					->where('id', $item['payschedid'])
					->where('deleted', 0)
					->first();
				...
				$cashtrans = DB::table('chrngcashtrans')
					->insertGetId([
						'transno' => $tnum,
						'payscheddetailid' => $item['payschedid'],
						'particulars' => $item['desc'],
						'itemprice' => $studitemized->itemamount,
						'qty' => 1,
						'amount' => $item['amount'],
						'duedate' => $duedate,
						'deleted' => 0,
				...
				DB::table('onlinepaymentlogs')->insert([
					'studid' => $studid,
					'studledgerid' => $itemizedid, // Now it is properly set
					'onlinepayment_id' => $paymentNo,
					'item_id' => $item['itemid'],
					'totalamount' => $amount,
		...
		// 	$paymentId = DB::table('studpaysched')->insertGetId([
		// 		'studid'       => $studid,
		// 		'semid'        => $semid,
		// 		'syid'         => $syid,
		// 		'classid'      => $classid,
		...
		// 	$paymentprocess[] = $paymentId;
		// 	$totalAmountPaid += $amount;

		// 	if($isadj == 0 || !$isadj){
		// 		// Update ledger itemized balance
		// 		$ledgerItem = DB::table('studledgeritemized')
		// 			->where('studid', $studid)
		// 			->where('semid', $semid)
		// 			->where('syid', $syid)
		// 			->where('classificationid', $classid)
		// 			->where('id', $itemizedid)
		// 			->select('id', 'totalamount', 'itemamount')
		// 			->first();

		// 		if ($ledgerItem) {
		// 			// Update total amount
		// 			if(($ledgerItem->totalamount + $amount) > $ledgerItem->itemamount ) {
		// 				return response()->json([
		// 					'status' => 'error',
		// 					'message' => 'Total amount exceeds item amount.',
		// 				], 400);
		// 			}
		// 			$newTotalAmount = max(0, $ledgerItem->totalamount + $amount);
		// 			DB::table('studledgeritemized')
		// 				->where('id', $ledgerItem->id)
		...
		// 	// Insert into online payment logs
		// 	DB::table('onlinepaymentlogs')->insert([
		// 		'studid'          => $studid,
		// 		'studledgerid'    => $itemizedid, // Now it is properly set
		// 		'onlinepayment_id'=> $paymentNo,
		...
		// 	// 	'semid' =>  FinanceModel::getSemID(),
		// 	// 	'syid' => $syid,
		// 	// 	'particulars' => DB::table('itemclassification')->where('id', $classid)->value('description'),
		// 	// 	'payment' => $amount,
		// 	// 	'ornum' => $refnum,
		// 	// 	'paytype' => $paymentType,
		...
		// 	return response()->json([
		// 		'status' => 'error',
		// 		'message' => 'No valid payment amounts found.',
		// 	], 400);
		// }
		...
		// $lastTrans = DB::table('chrngtrans')->orderBy('transno', 'desc')->first();
		// if ($lastTrans) {
		// 	$transno = sprintf('%05d', intval($lastTrans->transno) + 1);
		// } else {
		// 	$transno = '00001';
		// }


		// // $this->v2_performpay($transno, $studid, $refnum, $payor, $paymentType);


		// return response()->json([
		// 	'status' => 'success',
		// 	'message' => 'Payments successfully processed.',
		// 	'payment_ids' => $paymentprocess
		// ]);
		return response()->json([
			'status' => 'success',
			'message' => 'Payments successfully processed.'
	...
	public function printOnlinePayment(Request $request, $paymentID, $studid)
	{
		$schoolInfo = DB::table('schoolinfo')->select('schoolinfo.*')->first();

		$cashier = auth()->user();

		$cashierDetails = DB::table('users')
			->where('id', $cashier->id)
		...
		$student = DB::table('studinfo')
			...
			->leftJoin('sections', function ($join) {
				$join->on('studinfo.sectionid', '=', 'sections.id')
					->whereRaw('studinfo.levelid <= 16');
			})
			->leftJoin('college_sections', function ($join) {
				$join->on('studinfo.sectionid', '=', 'college_sections.id')
					->whereRaw('studinfo.levelid >= 17');
			})
			->where('studinfo.id', $studid)
			->select(
				'studinfo.*',
				'gradelevel.levelname',
				DB::raw("COALESCE(sections.sectionname, college_sections.sectionDesc) AS section")
			)
			->first();

		// Fetch payment logs
		$paymentLogs = DB::table('onlinepaymentlogs')
			->join('onlinepayments', 'onlinepaymentlogs.onlinepayment_id', '=', 'onlinepayments.id')
			->join('paymenttype', 'onlinepayments.paymentType', '=', 'paymenttype.id')
			->join('items', 'onlinepaymentlogs.item_id', '=', 'items.id')
			->where('onlinepaymentlogs.onlinepayment_id', $paymentID)
			->where('onlinepaymentlogs.studid', $studid)
			->select(
				'paymenttype.description',
		...
		$totalAmount = $paymentLogs->sum('totalamount');

		// Convert number to words
		$formatter = new NumberFormatter('en', NumberFormatter::SPELLOUT);
		$amountInWords = ucwords($formatter->format($totalAmount)) . ' Pesos Only';

		$pdf = PDF::loadView('finance.onlinepaymentreceipt', compact('student', 'paymentLogs', 'schoolInfo', 'totalAmount', 'amountInWords', 'cashierDetails'))
			->setPaper('letter', 'landscape')
			->setOptions(['dpi' => 150, 'defaultFont' => 'sans-serif']);

		return $pdf->stream('onlinepaymentreceipt_' . $paymentID . '.pdf');
	}

	public function ledgeradj_view(Request $request)
	{
		$dataid = $request->get('dataid');
		...
		$adj = DB::table('adjustments')
			->select(DB::raw('adjustments.id, adjustmentdetails.id AS detailid, description AS particulars, amount, adjustments.createddatetime, iscredit, classid'))
			->join('adjustmentdetails', 'adjustments.id', '=', 'adjustmentdetails.headerid')
			->where('adjustmentdetails.headerid', $dataid)
			->where('adjustmentdetails.deleted', 0)
			->first();

		if ($adj) {
			$particulars = $adj->particulars;
			$detailid = $adj->detailid;
			$amount = $adj->amount;
			$adjdate = date('m-d-Y', strtotime($adj->createddatetime));

			$class = DB::table('itemclassification')->where('id', $adj->classid)->first();
			$header_classname = $class ? $class->description : 'Unknown';

			$type = $adj->iscredit == 1 ? 'CREDIT' : 'DEBIT';

			$adjitems = DB::table('adjustmentitems')
				->select(DB::raw('adjustmentitems.id, detailid, itemid, adjustmentitems.classid, adjustmentitems.amount, items.description AS itemname, itemclassification.description AS classname'))
				->join('items', 'adjustmentitems.itemid', '=', 'items.id')
				->join('itemclassification', 'adjustmentitems.classid', '=', 'itemclassification.id')
		...
		return response()->json([
			...
			'detailid' => $detailid,
			'headeramount' => number_format($amount, 2),
			'date' => $adjdate,
			'isitemized' => $isitemized,
			'type' => $type,
			'adjdate' => $adjdate,
			'totalitemamount' => number_format($totalitemamount, 2),
			'adjitems' => $itemarray
		]);
	}
