...
class PurchasingController extends Controller
{
    ...
    public static function stockcard($id, $invoice)
    {
        ...
        foreach ($data as $item) {
            ...
            $existingOnhand = DB::table('items')
                ->where('id', $item->itemid)
                ->value('qty');


            $remarks = 'Receiving' . ' - Invoice No:' . $invoice;

            // Insert data into stockard table
            DB::table('stock_card')->insert([
                'initial_onhand' => $existingOnhand ?? 0,
                'onhand' => ($existingOnhand + $item->receivedqty) ?? 0,
                'stock_in' => $item->receivedqty ?? 0,
                'itemid' => $item->itemid,
                'remarks' => $remarks,
            ...
            DB::table('items')
                ->where('id', $item->itemid)
                ->increment('qty', $item->receivedqty);
    ...
    public function receiving_post(Request $request)
    {
        ...
        if (isset($chkstatus->invoiceno)) {

            $invoice = $chkstatus->invoiceno;
    ...
    public function receiving_removeje(Request $request)
    {
        $jeid = $request->get('id');
