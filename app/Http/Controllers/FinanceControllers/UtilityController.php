...
class UtilityController extends Controller
{
	...
	public static function resetpayment_v3(Request $request)
	{
			...
			if ($einfo) {
				...
				if ($einfo) {
					...
					if ($einfo) {
						...
						$levelid = 26; // 26 ang techvoc kay walay gradelevel, naay chance ma change ni please check grade level look for tesda
						$enrollid = $einfo->id;
					} else {
						return 'notenrolled';
					}
		...
		//         }
		//     })
		// 	// ->where('particulars', 'not like', '%OLD ACCOUNTS FORWARDED TO%')
		// 	->where('classid', '!=',$balclassid)
		//     ->update([
		//         'deleted' => 1,
		//         'deleteddatetime' => FinanceModel::getServerDateTime()
		...
		foreach ($studledger as $ledger) {
			if ($ledger->classid != $balclassid) {
				...
				if (strpos($ledger->particulars, 'ADJ:') !== false) {
					db::table('studledger')
						->where('id', $ledger->id)
						->update([
							'deleted' => 1,
		...
		FinanceUtilityModel::resetv3_generatefees($studid, $levelid, $enrollid, $syid, $semid, $feesid,$cor);
		...
		//Generate Discounts
		FinanceUtilityModel::resetv3_generatediscounts($studid, $levelid, $syid, $semid, 0);
	...
	public static function u_viewtuitiondetails(Request $request, $feesid = null)
	{
		if ($request != null) {
			$feesid = $request->get('feesid');
		}
