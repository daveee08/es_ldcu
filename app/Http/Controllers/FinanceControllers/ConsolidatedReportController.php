...
class ConsolidatedReportController extends Controller
{
    ...
    {
        $terminals = Db::table('chrngterminals')->get();
        return view('finance.reports.consolidatedreport.consolidatedschoolfees')->with('terminals',$terminals);
    }
  ...
  // public function generate_v2(Request $request)
    ...
    //         //tuition
    //         $transtui = db::table('chrngtrans')
    //             ->select(db::raw('chrngtrans.ornum, "" as itemcode, itemclassification.id as itemid, itemclassification.description, SUM(chrngcashtrans.amount) AS amount, gradelevel.acadprogid as acadprogid, classcode, progid '))
    //             ->join('chrngcashtrans', 'chrngtrans.transno', '=', 'chrngcashtrans.transno')
    //             ->join('itemclassification', 'chrngcashtrans.classid', '=', 'itemclassification.id')
    //             ->Join('studinfo', 'chrngtrans.studid', '=', 'studinfo.id')
    //             ->join('gradelevel', 'studinfo.levelid', '=', 'gradelevel.id')
    ...
    //             ->groupBy('acadprogid', 'itemclassification.id')
    //             ->having('amount', '>', 0)
    //             ->orderBy('classcode', 'ASC')
    //             ->orderBy('itemclassification.id', 'ASC')
    //             ->get();
    ...
    //         $transmisc = db::table('chrngtrans')
    //             ->select(db::raw('chrngtrans.ornum, items.id AS itemid, itemcode, items.description, SUM(chrngcashtrans.amount) AS amount, gradelevel.acadprogid as acadprogid, classcode, progid'))
    //             ->join('chrngcashtrans', 'chrngtrans.transno', '=', 'chrngcashtrans.transno')
    //             ->join('items', 'chrngcashtrans.itemid', '=', 'items.id')
    //             ->Join('studinfo', 'chrngtrans.studid', '=', 'studinfo.id')
    //             ->join('gradelevel', 'studinfo.levelid', '=', 'gradelevel.id')
    ...
    //         // old
    //         $oldsetup = db::table('balforwardsetup')->first();
    //         $oldclassid = $oldsetup->classid;
    //         $olditemid = $oldsetup->itemid;

    //         $transold = db::table('chrngtrans')
    //         ->select(db::raw('chrngtrans.ornum, "" as itemcode, itemclassification.id AS itemid, itemclassification.description, SUM(chrngcashtrans.amount) AS amount, gradelevel.acadprogid as acadprogid, classcode, progid'))
    //             ->join('chrngcashtrans', 'chrngtrans.transno', '=', 'chrngcashtrans.transno')
    //             // ->join('items', 'chrngcashtrans.classid', '=', 'items.classid')
    //             // ->join('items', function($q) use($olditemid){
    //             //     $q->on('chrngcashtrans.classid', '=', 'items.classid')
    ...
    public function generate_schoolfees(Request $request)
    {
        // Get and parse date range
        $dates = explode(' - ', $request->get('selecteddaterange'));
        $datefrom = date('Y-m-d', strtotime($dates[0]));
        $dateto = date('Y-m-d', strtotime($dates[1]));
    
        // Initialize totals
        $totals = [
            'col' => 0, 'shs' => 0, 'hs' => 0, 'gs' => 0, 'ps' => 0, 'gen' => 0, 'grand' => 0
        ];
        $list = '';
        ...
        $classcodes = DB::table('itemclassification')->where('deleted', 0)->get();
    
        foreach ($classcodes as $class) {
            // Class header
            $list .= '<tr><td colspan="9" class="text-bold">'.$class->description.'</td></tr>';
    
            // Initialize class totals
            $classTotals = [
                'col' => 0, 'shs' => 0, 'hs' => 0, 'gs' => 0, 'ps' => 0, 'gen' => 0
            ];
    
            // Get items for this classification
            ...
            foreach ($items as $item) {
                ...
                $transactions = DB::table('chrngtrans')
                    ->select(DB::raw('gradelevel.acadprogid, SUM(chrngcashtrans.amount) as amount'))
                    ->join('chrngcashtrans', 'chrngtrans.transno', '=', 'chrngcashtrans.transno')
                    ->join('itemclassification', 'chrngcashtrans.classid', '=', 'itemclassification.id')
                    ->join('studinfo', 'chrngtrans.studid', '=', 'studinfo.id')
                    ->join('gradelevel', 'studinfo.levelid', '=', 'gradelevel.id')
                    ...
                    ->where('chrngcashtrans.deleted', 0)
                    ->where('gradelevel.deleted', 0)
                    ->where('chrngcashtrans.classid', $class->id)
                    ->where('itemclassification.id', $item->id)
                    ->where('itemclassification.deleted', 0)
                    ->groupBy('acadprogid')
                    ->having('amount', '>', 0)
                    ->get();
                ...
                // Calculate amounts by program
                $amounts = [
                    'col' => 0, 'shs' => 0, 'hs' => 0, 'gs' => 0, 'ps' => 0
                ];
    
                foreach ($transactions as $trans) {
                    switch ($trans->acadprogid) {
                        case 6: $amounts['col'] = $trans->amount; break;
                        case 5: $amounts['shs'] = $trans->amount; break;
                        case 4: $amounts['hs'] = $trans->amount; break;
                        case 3: $amounts['gs'] = $trans->amount; break;
                        case 2: $amounts['ps'] = $trans->amount; break;
                    }
                }
    
                $gen = array_sum($amounts);
                $itemTotal = $gen;
    
                // Add to class totals
                foreach ($amounts as $key => $value) {
                    $classTotals[$key] += $value;
                }
                $classTotals['gen'] += $gen;
                ...
                $list .= '
                    <tr>
                        ...
                        <td class="text-right">'.number_format($amounts['col'], 2).'</td>
                        <td class="text-right">'.number_format($amounts['shs'], 2).'</td>
                        <td class="text-right">'.number_format($amounts['hs'], 2).'</td>
                        <td class="text-right">'.number_format($amounts['gs'], 2).'</td>
                        <td class="text-right">'.number_format($amounts['ps'], 2).'</td>
                        <td class="text-right">'.number_format($gen, 2).'</td>
                        <td class="text-right">'.number_format($itemTotal, 2).'</td>
                    </tr>
                ';
            ...
            $list .= '
                <tr>
                    <td colspan="2" class="text-bold text-right">TOTAL ' . strtoupper($class->description) . ':</td>
                    <td class="text-bold text-right">' . number_format($classTotals['col'], 2) . '</td>
                    <td class="text-bold text-right">' . number_format($classTotals['shs'], 2) . '</td>
                    <td class="text-bold text-right">' . number_format($classTotals['hs'], 2) . '</td>
                    <td class="text-bold text-right">' . number_format($classTotals['gs'], 2) . '</td>
                    <td class="text-bold text-right">' . number_format($classTotals['ps'], 2) . '</td>
                    <td class="text-bold text-right">' . number_format($classTotals['gen'], 2) . '</td>
                    <td class="text-bold text-right">' . number_format($classTotals['gen'], 2) . '</td>
                </tr>
        ...
        if (!$request->has('exporttype')) {
            return view('finance.reports.consolidatedreport.filtertable', [
                ...
                'gentotalcol' => $totals['col'],
                'gentotalshs' => $totals['shs'],
                'gentotalhs' => $totals['hs'],
                'gentotalgs' => $totals['gs'],
                'gentotalps' => $totals['ps'],
                'gentotalgen' => $totals['gen'],
                'gentotal' => $totals['grand'],
                'rangeOR' => $rangeOR
            ]);
        } 
    }
    ...
    public function generate_v2(Request $request)
    {
            ...
            foreach($classcodes as $class)
            {
                ...
                {      
                    ...
                    if($itemid == $trx->itemid || $itemid == 0)
                    {
                        ...
                        if($trx->acadprogid == 2)
                        {
                            $ps += $trx->amount;
                            $totalps += $trx->amount;
                            $gentotalps += $trx->amount;
                        }
                        elseif($trx->acadprogid == 3)
                        {
                            $gs += $trx->amount;
                            $totalgs += $trx->amount;
                            $gentotalgs += $trx->amount;
                        ...
                        $trxitemcode = $trx->itemcode;
                        $trxdescription = $trx->description;
                        $totalperincome = $totalcol + $totalshs + $totalhs + $totalgs  + $totalps + $totalgen;
                    }
                    else
                    {
                        $list .='
                            <tr>
                                ...
                                <td class="text-right">'.number_format($hs, 2).'</td>
                                <td class="text-right">'.number_format($gs, 2).'</td>
                                <td class="text-right">'.number_format($ps, 2).'</td>
                                <td class="text-right">'.number_format($gen, 2).'</td>
                                <td class="text-right">'.number_format($itemtotal, 2).'</td>
                            </tr>
                        ';
                ...
                $list .='
                    <tr>
                        ...
                        <td class="text-bold text-right">' . number_format($totalhs, 2) . '</td>
                        <td class="text-bold text-right">' . number_format($totalgs, 2) . '</td>
                        <td class="text-bold text-right">' . number_format($totalps, 2) . '</td>
                        <td class="text-bold text-right">' . number_format($totalgen, 2) . '</td>
                        <td class="text-bold text-right">' . number_format($totalperincome, 2) . '</td>
                    </tr>
                ';
        ...
        $gentotal = $gentotalcol + $gentotalshs + $gentotalhs + $gentotalgs + $gentotalps + $gentotalgen;
