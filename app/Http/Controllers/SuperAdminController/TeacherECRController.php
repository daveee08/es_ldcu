...
class TeacherECRController extends \App\Http\Controllers\Controller
{
		...
		public static function get_students(
            ...
            if($levelid != 14 && $levelid != 15 ){
                  ...
                  $student = DB::table('student_specsubj')
                                    ...
                                    ->join('enrolledstud',function($join) use($syid){
                                          ...
                                          $join->where('enrolledstud.studstatus','!=',0);
                  ...
                  $student = collect($temp_students)->sortBy(function($item) {
                        ...
                        return str_replace('Ñ', 'N',$gsort.'-'.$item->studentname);
                  })->values();
                  ...
                  $student = DB::table('student_specsubj')
                                      ...
                                      ->join('sh_enrolledstud',function($join) use($syid,$semid){
                                          ...
                                          $join->where('sh_enrolledstud.studstatus','!=',0);
            ...
            if($quarter != null && $quarter != ""){
                  $student_quarter = DB::table('student_quarter')
                                    ->where('syid',$syid)
                                    ->whereIn('studid',collect($student)->whereNotIn('studstatus',[1,2,4])->pluck('studid'))
                                    ->get();

                  $not_included = array();
                  foreach($student_quarter as $item){
                        $quarter_text  = 'quarter'.$quarter;
                        if($item->$quarter_text == 0){
                              array_push($not_included,$item->studid);
                        }
                  }

                  $student = collect($student)->whereNotIn('studid',$not_included)->values();
            }
      ...
      function download_ecr(Request $request){
            // return $request->all();
            ...
            if($schoolinfo->abbreviation == 'SJCHSSI'){
                  if($syid < 4){
                      return \App\Http\Controllers\SuperAdminController\TeacherECRController_1::download_ecr($request);
                  }
            }

            if($schoolinfo->abbreviation == 'HCHS CP'){
                  if($syid >= 3){
                      return \App\Http\Controllers\SuperAdminController\TeacherECR\HCHSCPDownloadUpload::download_ecr($request);
                  }
            }

            if($schoolinfo->abbreviation == 'APMC'){
                  if($syid >= 3){
                        return \App\Http\Controllers\SuperAdminController\TeacherECR\APMCTeacherECRController::download_ecr($request);
                  }
            }
			...
			if(count($subjinfo) > 1){
                if($strandid != "" && $strandid != null){
					$subjinfo = collect($subjinfo)->where('strandid',$strandid)->values();
				}
            }
            

			$checkteachergradesetup = DB::table('teacher_subjectcomponentdetail')
										->where('syid',$syid)
										->where('subjid',$subjid)
										->where('sectionid',$sectionid)
										->where('teacher_subjectcomponentdetail.deleted',0)
										 ->join('subject_gradessetup',function($join){
											$join->on('teacher_subjectcomponentdetail.subjcomid','=','subject_gradessetup.id');
			...
			if(isset($checkteachergradesetup)){
				$subjinfo[0]->qa = $checkteachergradesetup->qa;
				$subjinfo[0]->pt = $checkteachergradesetup->pt;
				$subjinfo[0]->ww = $checkteachergradesetup->ww;
				$subjinfo[0]->comp4 = $checkteachergradesetup->comp4;
			}
		...
		// return $setup;
            ...
            if($setup == 2){
                  ...
                  if(strtoupper($schoolinfo->abbreviation) == 'HCHS'){
                       ...
                       // if($levelinfo->acadprogid == 5){
                           //$spreadsheet = $reader->load("ECR/ECR_MUL_QA/ECR-SHS.xlsx");
                        //}else{
                            //$spreadsheet = $reader->load("ECR/ECR_MUL_QA/ECR.xlsx");
                        //}
                        // return $levelinfo->acadprogid;
                        if($levelinfo->acadprogid == 5){
                              $spreadsheet = $reader->load("ECR/ECR_HPS_VALIDATION/ECR-SHS.xlsx");
                        }else{
                              $spreadsheet = $reader->load("ECR/ECR_HPS_VALIDATION/ECR.xlsx");

                              // Loop through all worksheets to protect each one and lock all cells
                              foreach ($spreadsheet->getAllSheets() as $sheet) {
                                    // Step 1: Enable sheet protection
                                    $sheet->getProtection()->setSheet(true); // Protect the entire sheet
                                    
                                    // Step 2: Optionally, set a password to further protect the sheet
                                    // $sheet->getProtection()->setPassword('admin'); // Optional: Add a password for protection
                                    
                                    // All cells are locked by default, so no need to explicitly lock each cell.
                                    // If you have previously unlocked any cells, you can ensure they are locked by:
                  ...
                  if($levelinfo->acadprogid == 5){
                        $spreadsheet = $reader->load("ECR/DEFAULTORIG/ECR-SHS.xlsx");
                  }else{
                        $spreadsheet = $reader->load("ECR/DEFAULTORIG/ECR.xlsx");
                  }
            }
            else if($setup == 4){
                  // if($levelinfo->acadprogid == 5){
                  ...
                  // }else{
                        ...
                        // $spreadsheet = $reader->load("ECR/ECR_SHJMS/ECR.xlsx");
                  // }
                  $spreadsheet = $reader->load("ECR/ECR_SHJMS/ECR-SHJMS.xlsx");
            }
            ...
            for($x = 0 ; $x < 4; $x++){
                  ...
                  if($applicable){
                              ...
                              elseif($setup == 4){
                                    ...
                                    try{
                                          ...
                                          if(count( $check_gradedates) > 0){
                                                $sheet->setCellValue('C'.$count,$check_gradedates[0]->dww1 != null && $check_gradedates[0]->dww1 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dww1)->isoFormat('MM/DD/YYYY') : '');
                                                $sheet->setCellValue('D'.$count,$check_gradedates[0]->dww2 != null && $check_gradedates[0]->dww2 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dww2)->isoFormat('MM/DD/YYYY') : '');
                                                $sheet->setCellValue('E'.$count,$check_gradedates[0]->dww3 != null && $check_gradedates[0]->dww3 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dww3)->isoFormat('MM/DD/YYYY') : '');
                                                $sheet->setCellValue('F'.$count,$check_gradedates[0]->dww4 != null && $check_gradedates[0]->dww4 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dww4)->isoFormat('MM/DD/YYYY') : '');
                                                $sheet->setCellValue('G'.$count,$check_gradedates[0]->dww5 != null && $check_gradedates[0]->dww5 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dww5)->isoFormat('MM/DD/YYYY') : '');
                                                $sheet->setCellValue('H'.$count,$check_gradedates[0]->dww6 != null && $check_gradedates[0]->dww6 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dww6)->isoFormat('MM/DD/YYYY') : '');
                                                $sheet->setCellValue('I'.$count,$check_gradedates[0]->dww7 != null && $check_gradedates[0]->dww7 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dww7)->isoFormat('MM/DD/YYYY') : '');
                                                $sheet->setCellValue('J'.$count,$check_gradedates[0]->dww8 != null && $check_gradedates[0]->dww8 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dww8)->isoFormat('MM/DD/YYYY') : '');
      
                                                $sheet->setCellValue('M'.$count,$check_gradedates[0]->dpt1 != null && $check_gradedates[0]->dpt1 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dpt1)->isoFormat('MM/DD/YYYY') : '');
                                                $sheet->setCellValue('N'.$count,$check_gradedates[0]->dpt2 != null && $check_gradedates[0]->dpt2 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dpt2)->isoFormat('MM/DD/YYYY') : '');
                                                $sheet->setCellValue('O'.$count,$check_gradedates[0]->dpt3 != null && $check_gradedates[0]->dpt3 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dpt3)->isoFormat('MM/DD/YYYY') : '');
                                                $sheet->setCellValue('P'.$count,$check_gradedates[0]->dpt4 != null && $check_gradedates[0]->dpt4 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dpt4)->isoFormat('MM/DD/YYYY') : '');
      
                                                $sheet->setCellValue('S'.$count,$check_gradedates[0]->dqa1 != null && $check_gradedates[0]->dqa1 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dqa1)->isoFormat('MM/DD/YYYY') : '');
                                                $sheet->setCellValue('T'.$count,$check_gradedates[0]->dqa2 != null && $check_gradedates[0]->dqa2 != '0000-00-00' ? \Carbon\Carbon::create($check_gradedates[0]->dqa2)->isoFormat('MM/DD/YYYY') : '');
                                          }else{
                                                DB::table('grades_date')
                                                      ->insert([
                                                            'headerid' => $check_header[0]->id,
                                    ...
                                    // Set cell values and unprotect the cells
                                    $sheet->setCellValue('F'.$count,$check_header[0]->wwhr1 == 0 ? "": $check_header[0]->wwhr1);
                                    $sheet->getStyle('F'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('G'.$count,$check_header[0]->wwhr2 == 0 ? "": $check_header[0]->wwhr2);
                                    $sheet->getStyle('G'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('H'.$count,$check_header[0]->wwhr3 == 0 ? "": $check_header[0]->wwhr3);
                                    $sheet->getStyle('H'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('I'.$count,$check_header[0]->wwhr4 == 0 ? "": $check_header[0]->wwhr4);
                                    $sheet->getStyle('I'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('J'.$count,$check_header[0]->wwhr5 == 0 ? "": $check_header[0]->wwhr5);
                                    $sheet->getStyle('J'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('K'.$count,$check_header[0]->wwhr6 == 0 ? "": $check_header[0]->wwhr6);
                                    $sheet->getStyle('K'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('L'.$count,$check_header[0]->wwhr7 == 0 ? "": $check_header[0]->wwhr7);
                                    $sheet->getStyle('L'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('M'.$count,$check_header[0]->wwhr8 == 0 ? "": $check_header[0]->wwhr8);
                                    $sheet->getStyle('M'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('N'.$count,$check_header[0]->wwhr9 == 0 ? "": $check_header[0]->wwhr9);
                                    $sheet->getStyle('N'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('O'.$count,$check_header[0]->wwhr0 == 0 ? "": $check_header[0]->wwhr0);
                                    $sheet->getStyle('O'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    // Set cell values and unprotect for 'pt' columns
                                    $sheet->setCellValue('S'.$count,$check_header[0]->pthr1 == 0 ? "": $check_header[0]->pthr1);
                                    $sheet->getStyle('S'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('T'.$count,$check_header[0]->pthr2 == 0 ? "": $check_header[0]->pthr2);
                                    $sheet->getStyle('T'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('U'.$count,$check_header[0]->pthr3 == 0 ? "": $check_header[0]->pthr3);
                                    $sheet->getStyle('U'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('V'.$count,$check_header[0]->pthr4 == 0 ? "": $check_header[0]->pthr4);
                                    $sheet->getStyle('V'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('W'.$count,$check_header[0]->pthr5 == 0 ? "": $check_header[0]->pthr5);
                                    $sheet->getStyle('W'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('X'.$count,$check_header[0]->pthr6 == 0 ? "": $check_header[0]->pthr6);
                                    $sheet->getStyle('X'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('Y'.$count,$check_header[0]->pthr7 == 0 ? "": $check_header[0]->pthr7);
                                    $sheet->getStyle('Y'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('Z'.$count,$check_header[0]->pthr8 == 0 ? "": $check_header[0]->pthr8);
                                    $sheet->getStyle('Z'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('AA'.$count,$check_header[0]->pthr9 == 0 ? "": $check_header[0]->pthr9);
                                    $sheet->getStyle('AA'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    
                                    $sheet->setCellValue('AB'.$count,$check_header[0]->pthr0 == 0 ? "": $check_header[0]->pthr0);
                                    $sheet->getStyle('AB'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                        ...
                        if($setup == 2){
                              ...
                              $sheet->setCellValue('L10', $subjinfo[0]->ww / 100 );
                              $sheet->setCellValue('R10', $subjinfo[0]->pt / 100 );
                              $sheet->setCellValue('V10', $subjinfo[0]->qa / 100 );
                              ...
                              // $sheet->setCellValue('B79', 'Teacher: '. $adviser);
                        }
                        else{

                              // $sheet->getStyle('A11:AL11')->getProtection()->setLocked(
                              //       \PhpOffice\PhpSpreadsheet\Style\Protection::PROTECTION_UNPROTECTED
                              //   );
                                

                              $sheet->setCellValue('G5', $schoolinfo->schoolname);
                              $sheet->getCell('G5')->getStyle()->getProtection()->setLocked(
                                    \PhpOffice\PhpSpreadsheet\Style\Protection::PROTECTION_UNPROTECTED
                              );
                              $sheet->setCellValue('X5', $schoolinfo->schoolid);
                              $sheet->getCell('X5')->getStyle()->getProtection()->setLocked(
                                    \PhpOffice\PhpSpreadsheet\Style\Protection::PROTECTION_UNPROTECTED
                              );
                              $sheet->setCellValue('AG5', $sy->sydesc);
                              $sheet->getCell('AG5')->getStyle()->getProtection()->setLocked(
                                    \PhpOffice\PhpSpreadsheet\Style\Protection::PROTECTION_UNPROTECTED
                              );
                              $sheet->setCellValue('K7', $levelinfo->levelname.' '.$section->sectionname);
                              $sheet->getCell('K7')->getStyle()->getProtection()->setLocked(
                                    \PhpOffice\PhpSpreadsheet\Style\Protection::PROTECTION_UNPROTECTED
                              );
      
                              $sheet->setCellValue('G4', $schoolinfo->regiontext);
                              $sheet->getCell('G4')->getStyle()->getProtection()->setLocked(
                                    \PhpOffice\PhpSpreadsheet\Style\Protection::PROTECTION_UNPROTECTED
                              );
                              $sheet->setCellValue('O4', $schoolinfo->divisiontext);
                              $sheet->getCell('O4')->getStyle()->getProtection()->setLocked(
                                    \PhpOffice\PhpSpreadsheet\Style\Protection::PROTECTION_UNPROTECTED
                              );
                              $sheet->setCellValue('X4', $schoolinfo->districttext);
                              $sheet->getCell('X4')->getStyle()->getProtection()->setLocked(
                                    \PhpOffice\PhpSpreadsheet\Style\Protection::PROTECTION_UNPROTECTED
                              );
                        ...
                        foreach($student as $item){
                              ...
                              $sheet->getStyle('F'.$count.':'.'O'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                              $sheet->getStyle('S'.$count.':'.'AB'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);

                              // // Protect the specified cells
                              // $sheet->getStyle('P'.$count)->getProtection()->setLocked(Protection::PROTECTION_PROTECTED);
                              // $sheet->getStyle('Q'.$count)->getProtection()->setLocked(Protection::PROTECTION_PROTECTED);
                              // $sheet->getStyle('R'.$count)->getProtection()->setLocked(Protection::PROTECTION_PROTECTED);
                              // $sheet->getStyle('AC'.$count)->getProtection()->setLocked(Protection::PROTECTION_PROTECTED);
                              // $sheet->getStyle('AD'.$count)->getProtection()->setLocked(Protection::PROTECTION_PROTECTED);
                              // $sheet->getStyle('AE'.$count)->getProtection()->setLocked(Protection::PROTECTION_PROTECTED);
                              // $sheet->getStyle('AI'.$count)->getProtection()->setLocked(Protection::PROTECTION_PROTECTED);
                              // $sheet->getStyle('AJ'.$count)->getProtection()->setLocked(Protection::PROTECTION_PROTECTED);
                              ...
                              if(isset($check_detail->id)){
                                    ...
                                    if($setup == 2){
                                          ...
                                          $sheet->getStyle('C'.$count.':T'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    }
                                    else{
                                          ...
                                          // $sheet->getStyle('F'.$count.':AG'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
                                    }
                              }
                              ...
                              $sheet->setCellValue('AZ'.$count,$item->studid);
                        ...
                        if($setup == 2){ //spct
                              ...
                              for($removeRow = $count ; $removeRow <= ( $count + $remaining_rows ); $removeRow++){
                                    $sheet->removeRow($count);
                              }

                              $sheet->setCellValue('B79', $adviser);
                              ...
                              if($setup == 3){
                                    $sheet->getStyle('A'.$count.':'.'AL150')->applyFromArray($no_border);
                                    $sheet->getStyle('A'.$count.':'.'AL'.$count)->applyFromArray($thick_top);
                                    $sheet->getPageSetup()->setPrintArea('A1:AL'.($count-1));
                              }else if($setup == 1){
            ...
            if($setup==4){
                  ...
                  // $sheet->getCell('C6')->getStyle()->getProtection()->setLocked(
                  //             \PhpOffice\PhpSpreadsheet\Style\Protection::PROTECTION_UNPROTECTED
                  //       );
                  ...
                  // Define all the cell values and unprotect each one
                  $valuesToSet = [
                        'G6' => $schoolinfo->schoolname,
                        'C6' => null, // This will be unprotected but no value set
                        'W6' => $schoolinfo->schoolid,
                        'K8' => $levelinfo->levelname.' '.$section->sectionname,
                        'G5' => $schoolinfo->regiontext,
                        'O5' => $schoolinfo->divisiontext,
                        'W5' => $schoolinfo->districttext,
                  ];

                  // Additional values based on condition
                  if ($levelinfo->acadprogid == 5) {
                        $valuesToSet += [
                              'Q9' => $subjinfo[0]->subjdesc,
                              'I9' => $adviser,
                              'Y8' => $sy->sydesc,
                        ];
                  } else {
                        $valuesToSet += [
                              'W9' => $subjinfo[0]->subjdesc,
                              'K9' => $adviser,
                              'W8' => $sy->sydesc,
                        ];
                  }

                  // Loop through each cell to set the value and unprotect it
                  foreach ($valuesToSet as $cell => $value) {
                        if ($value !== null) {
            ...
            foreach($student as $item){
                  ...
                  if($male == 0 && strtoupper($item->gender) == 'MALE'){
                   ...
                   $sheet->getStyle('F'.$count.':'.'AB'.$count)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
            ...
            // // Loop through all worksheets and collect their names
            // foreach ($spreadsheet->getAllSheets() as $sheet) {
      ...
      public static function upload_ecr(Request $request){
            ...
            if($schoolinfo->abbreviation == 'APMC'){
                  if($syid >= 3){
                        return \App\Http\Controllers\SuperAdminController\TeacherECR\APMCTeacherECRController::upload_ecr($request);
                  }
            }
            ...
            if($check[0]->submitted == 0){
                  if($setup == 2){
                        ...
                        $start_row = 9;
                        // return $data[$start_row][2];
                        // return collect(self::validateDate('1/2/2012', 'n/j/Y'));
                        DB::table('grades_date')
                              ->where('headerid',$headerid)
                              ->take(1)
                              ->update([
                                    'dww1'=>self::validateDate($data[$start_row][2]) ? \Carbon\Carbon::create($data[$start_row][2]) : '',
                                    'dww2'=>self::validateDate($data[$start_row][3]) ? \Carbon\Carbon::create($data[$start_row][3]) : '',
                                    'dww3'=>self::validateDate($data[$start_row][4]) ? \Carbon\Carbon::create($data[$start_row][4]) : '',
                                    'dww4'=>self::validateDate($data[$start_row][5]) ? \Carbon\Carbon::create($data[$start_row][5]) : '',
                                    'dww5'=>self::validateDate($data[$start_row][6]) ? \Carbon\Carbon::create($data[$start_row][6]) : '',
                                    'dww6'=>self::validateDate($data[$start_row][7]) ? \Carbon\Carbon::create($data[$start_row][7]) : '',
                                    'dww7'=>self::validateDate($data[$start_row][8]) ? \Carbon\Carbon::create($data[$start_row][8]) : '',
                                    'dww8'=>self::validateDate($data[$start_row][9]) ? \Carbon\Carbon::create($data[$start_row][9]) : '',
                                    'dpt1'=>self::validateDate($data[$start_row][12]) ? \Carbon\Carbon::create($data[$start_row][12]) : '',
                                    'dpt2'=>self::validateDate($data[$start_row][13]) ? \Carbon\Carbon::create($data[$start_row][13]) : '',
                                    'dpt3'=>self::validateDate($data[$start_row][14]) ? \Carbon\Carbon::create($data[$start_row][14]) : '',
                                    'dpt4'=>self::validateDate($data[$start_row][15]) ? \Carbon\Carbon::create($data[$start_row][15]) : '',
                                    'dqa1'=>self::validateDate($data[$start_row][18]) ? \Carbon\Carbon::create($data[$start_row][18]) : '',
                                    'dqa2'=>self::validateDate($data[$start_row][19]) ? \Carbon\Carbon::create($data[$start_row][19]) : '',
                                    'updatedby'=>auth()->user()->id,
                                    'updateddatetime'=>\Carbon\Carbon::now('Asia/Manila')
                              ]);
                        ...
                        DB::table('grades')
                                ...
                                ->take(1)
                                ->update([
                                      'wwhr1'=>(float)$data[$start_row][2],
                                      'wwhr2'=>(float)$data[$start_row][3],
                                      'wwhr3'=>(float)$data[$start_row][4],
                                      'wwhr4'=>(float)$data[$start_row][5],
                                      'wwhr5'=>(float)$data[$start_row][6],
                                      'wwhr6'=>(float)$data[$start_row][7],
                                      'wwhr7'=>(float)$data[$start_row][8],
                                      'wwhr8'=>(float)$data[$start_row][9],
                                      'wwhrtotal'=>(float)$data[$start_row][10],
                                      'pthr1'=>(float)$data[$start_row][12],
                                      'pthr2'=>(float)$data[$start_row][13],
                                      'pthr3'=>(float)$data[$start_row][14],
                                      'pthr4'=>(float)$data[$start_row][15],
                                      'pthrtotal'=>(float)$data[$start_row][16],
                                      'qahr1'=>(float)$data[$start_row][18],
                                      'qahr2'=>(float)$data[$start_row][19],
                                      'qahrtotal'=>(float)$data[$start_row][20],
                                      'uploadedby'=>auth()->user()->id,
                                      'uploadeddatetime'=>\Carbon\Carbon::now('Asia/Manila')
                                ]);
                                    ...
                                    if($schoolinfo->snr == 'spct'){
                                          ...
                                          if(count($check_detail) == 0){
                                                DB::table('gradesdetail')
                                                      ->insert([
                                                            'ig'=>0,
                                                            'qg'=>(float)$data[$start_row][22],

                                                            'ww1'=>(float)$data[$start_row][2],
                                                            'ww2'=>(float)$data[$start_row][3],
                                                            'ww3'=>(float)$data[$start_row][4],
                                                            'ww4'=>(float)$data[$start_row][5],
                                                            'ww5'=>(float)$data[$start_row][6],
                                                            'ww6'=>(float)$data[$start_row][7],
                                                            'ww7'=>(float)$data[$start_row][8],
                                                            'ww9'=>(float)$data[$start_row][9],
                                                            'wwps'=>0,
                                                            'wwtotal'=>(float)$data[$start_row][10],
                                                            'wwws'=>(float)$data[$start_row][11],

                                                            'pt1'=>(float)$data[$start_row][12],
                                                            'pt2'=>(float)$data[$start_row][13],
                                                            'pt3'=>(float)$data[$start_row][14],
                                                            'pt4'=>(float)$data[$start_row][15],

                                                            'pttotal'=>(float)$data[$start_row][16],
                                                            'ptws'=>(float)$data[$start_row][17],
                                                            'ptps'=>0,

                                                            'qa1'=>(float)$data[$start_row][18],
                                                            'qa2'=>(float)$data[$start_row][19],

                                                            'qaps'=>0,
                                                            'qatotal'=>(float)$data[$start_row][20],
                                                            'qaws'=>(float)$data[$start_row][21]
                                                      ]);
                                          }else{
      ...
      public static function view_ecr(Request $request){
			...
			if(isset($checkteachergradesetup)){
				 $gradesetup = array((object)[
                        'ww'=>$checkteachergradesetup->ww,
                        'pt'=>$checkteachergradesetup->pt,
                        'qa'=>$checkteachergradesetup->qa,
                        'comp4'=>$checkteachergradesetup->comp4,
                  ]);
			}
            ...
            $student_quarter = DB::table('student_quarter')
                              ->where('syid',$syid)
                              ->whereIn('studid',collect($enrolledstud)->whereNotIn('studstatus',[1,2,4])->pluck('studid'))
                              ->get();
            ...
            $enrolledstud = collect($enrolledstud)->whereNotIn('studid',$not_included)->values();
            // dd($enrolledstud);
            $grades = array();
            ...
            try{
                  ...
                  $gradedates = DB::table('grades_date')
                                    ->where('headerid',$check_header[0]->id)
									->take(1)
                                    ->get();
            ...
            if($schoolinfo->snr == 'apmc'){
                  $view = 'superadmin.pages.teacher.gradeview_apmc';
            }
            ...
            try{
                  ...
                  if(count($exclude) == 0){
                        ...
                        if($check_refid->refid != 0){
                              ...
                              if($check_refid->refid == 22 || $check_refid->refid == 27){
                                    DB::table('grades')
                                          ->where('id',$header)
                                          ->take(1)
                                          ->update([
