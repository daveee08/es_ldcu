...
class StudentInformationController extends \App\Http\Controllers\Controller
{
      ...
      public static function all_student($syid = null, $semid = null){
            ...
            if(auth()->user()->type == 17){
                  ...
                  $teacherid = DB::table('teacher')->where('tid',auth()->user()->email)->select('id')->first();
                  if($teacherid){
                  ...
                  if( Session::get('currentPortal') == 2){
                        ...
                        $academicprogram = DB::table('teacheracadprog')
                                          ->where(function($query) use($teacherid){
                                                if($teacherid &&  Session::get('currentPortal') != 6 ){
                  ...
                  foreach($academicprogram as $item){
                        ...
                        if(Session::get('currentPortal') == 3 || Session::get('currentPortal') == 6){
                              ...
                              $students_list = DB::table('studinfo')
                                                ...
                                                ->leftJoin('college_courses', function($join) use($acad){
                                                      $join->on('studinfo.courseid','=','college_courses.id');
                                                      $join->where('college_courses.deleted', 0);
                                                })
                                                ->where('studinfo.deleted',0)
