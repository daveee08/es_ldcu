...
class StudentPregistration extends \App\Http\Controllers\Controller
{
      ...
      public static function age_listing(Request $request)
      {
            ...
            foreach ($schoolyears as $schoolyear) {
                  ...
                  $all_students = collect(array())->values();

                  $jhs_students = DB::table('enrolledstud')
                        ->join('studinfo', function ($join) {
                              $join->on('enrolledstud.studid', '=', 'studinfo.id');
                              $join->where('studinfo.deleted', 0);
                        })
                        ->whereIn('enrolledstud.studstatus', [1, 2, 4])
                        ->where('enrolledstud.syid', $schoolyear->id)
                  ...
                  foreach ($jhs_students as $jhs_student) {
                        $birthDate = $jhs_student->dob; // Your birthdate
                  ...
                  // $all_students = $all_students->merge($jhs_students);

                  $shs_students = DB::table('sh_enrolledstud')
                        ->join('studinfo', function ($join) {
                              $join->on('sh_enrolledstud.studid', '=', 'studinfo.id');
                              $join->where('studinfo.deleted', 0);
                        })
                        ->whereIn('sh_enrolledstud.studstatus', [1, 2, 4])
                        ->where('sh_enrolledstud.syid', $schoolyear->id)
                  ...
                  foreach ($shs_students as $shs_student) {
                        $birthDate = $shs_student->dob; // Your birthdate
                        $currentYear = explode("-", $schoolyear->sydesc)[0]; // Current Year
                        $birthYear = date('Y', strtotime($birthDate)); // Extracted Birth Year using strtotime and date() function
                        $age = $currentYear - $birthYear; // Current year minus birthyear
                        $shs_student->age = $age;
                  ...
                  // $all_students = $all_students->merge($shs_students);
                  ...
                  $college_students = DB::table('college_enrolledstud')
                        ...
                        ->where('college_enrolledstud.syid', $schoolyear->id)
                  ...
                  foreach ($college_students as $college_student) {
                        $birthDate = $college_student->dob; // Your birthdate
                        $currentYear = explode("-", $schoolyear->sydesc)[0]; // Current Year
                        $birthYear = date('Y', strtotime($birthDate)); // Extracted Birth Year using strtotime and date() function
                        $age = $currentYear - $birthYear; // Current year minus birthyear
                        $college_student->age = $age;
                  }

                  $schoolyear->jhs_students = $jhs_students;
                  $schoolyear->shs_students = $shs_students;
                  $schoolyear->college_students = $college_students;

            }

            $pdf = PDF::loadView('superadmin.pages.printable.agelisting', compact('schoolyears'));
            $pdf->getDomPDF()->set_option("enable_php", true)->set_option("DOMPDF_ENABLE_CSS_FLOAT", true);
            return $pdf->stream('Age Listing.pdf');
      ...
      public function getSections(Request $request)
      {
            $sections = DB::table('sections')
                  ->where('deleted', 0)
                  ->where('levelid', $request->gradelevel)
                  ->select('id', 'sectionname as sectionDesc')
                  ->get();
      ...
      public static function preenrolledstudents(Request $request)
      {
            ...
            //       if($studstat != 0 && $studstat != ''){
            //             $chngtrans = $chngtrans->whereIn('chrngtrans.studid',$student_array);
            //       }else if($studstat == 0 && $studstat != ''){
            //             $chngtrans = $chngtrans->whereNotIn('chrngtrans.studid',$student_array);
            //       }

            //       $chngtrans = $chngtrans->select(
            //                               'transno',
            //                               'syid',
            //                               'studid',
            //                               'semid'
            ...
            if ($studstat != 0 && $studstat != '') {
                  ...
                  if ($fillevelid >= 17 && $fillevelid <= 21) {
                        $all_students = $all_students->join('college_enrolledstud', 'studinfo.id', '=', 'college_enrolledstud.studid')
                        ->where('college_enrolledstud.syid', $syid)
                        ->where('college_enrolledstud.semid', $semid)
                        ->when($fillevelid != 0 && $fillevelid != '', function ($q) use ($fillevelid) {
                              return $q->where('college_enrolledstud.yearLevel', $fillevelid);
                        })
                        ->select('studinfo.*', 'gradelevel.*', 'college_enrolledstud.yearLevel as gradelevelid');

                  } else if ($fillevelid == 14 || $fillevelid == 15) {
                        $all_students = $all_students->join('sh_enrolledstud', 'studinfo.id', '=', 'sh_enrolledstud.studid')
                        ->where('sh_enrolledstud.syid', $syid)
                        ->where('sh_enrolledstud.semid', $semid)
                        ->when($fillevelid != 0 && $fillevelid != '', function ($q) use ($fillevelid) {
                              return $q->where('sh_enrolledstud.levelid', $fillevelid);
                        })
                        ->select('studinfo.*', 'gradelevel.*', 'sh_enrolledstud.levelid as gradelevelid');

                  } else if($fillevelid >= 13 || $fillevelid == 16) {
                        $all_students = $all_students->join('enrolledstud', 'studinfo.id', '=', 'enrolledstud.studid')
                        ->where('enrolledstud.syid', $syid)
                        ->when($fillevelid != 0 && $fillevelid != '', function ($q) use ($fillevelid) {
                              return $q->where('enrolledstud.levelid', $fillevelid);
                        })
                        ->select('studinfo.*', 'gradelevel.*', 'enrolledstud.levelid as gradelevelid');
                  }
            ...
            // // Count total students (with all conditions applied)
            // $student_count = $all_students->count();

            // // Fetch paginated results (using Laravel's paginate for accuracy)
            // $all_students = $all_students
            //       ->orderBy('lastname') // Sort by last name first
            //       ->orderBy('firstname') // Then first name
            //       ->orderBy('studid') // Finally, enforce uniqueness
            //       ->groupBy('studinfo.id')
            //       ->select([
            //             'contactno',
            //             'ismothernum',
            ...
            //       ])
            //       ->groupBy('studinfo.id')
            //       ->paginate($request->get('length')); // Use Laravel paginate() instead of skip/take
            ...
            if ($school_info->abbreviation == 'BCT') {
                  $payment_info = db::table('chrngtrans')
                        ...
                        ->where('classid', 106)
                        ->get();
            } else if ($school_info->abbreviation == 'VNBC') {


                  $gchngtrans = DB::table('chrngtransdetail')
            ...
            foreach ($all_students as $item) {
                  ...
                  $item->lastenrolledcourse = DB::table('college_enrolledstud')->where('studid', $item->studid)->orderBy('createddatetime', 'desc')->value('courseid');

                  // $lastGrade1 = DB::table('enrolledstud')->where('studid', $item->studid)->orderBy('createddatetime', 'desc')->value('levelid');
                  // $lastGrade2 = DB::table('sh_enrolledstud')->where('studid', $item->studid)->orderBy('createddatetime', 'desc')->value('levelid');
                  // $lastGrade3 = DB::table('college_enrolledstud')->where('studid', $item->studid)->orderBy('createddatetime', 'desc')->value('yearLevel');

                  // if ($lastGrade1 != null) {
                  //       $item->lastgrade = DB::table('gradelevel')->where('id', $lastGrade1)->value('levelname');
                  // } elseif ($lastGrade2 != null) {
                  //       $item->lastgrade = DB::table('gradelevel')->where('id', $lastGrade2)->value('levelname');
                  // } elseif ($lastGrade3 != null) {
                  //       $item->lastgrade = DB::table('gradelevel')->where('id', $lastGrade3)->value('levelname');
                  // } else {
                  //       $item->lastgrade = 'No records Found';
                  // }

                  $currentLevel = $item->levelid;
                  $lowerLevel = DB::table('enrolledstud')
                        ->select('levelid', 'createddatetime')
                        ...
                        ->unionAll(
                              DB::table('college_enrolledstud')
                                    ->selectRaw('yearLevel as levelid, createddatetime')
                                    ->where('studid', $item->studid)
                                    ->where('yearLevel', '<', $currentLevel)
                        )
                        ->orderBy('createddatetime', 'desc')
                        ->first();

                  // If a lower level exists, get its name; otherwise, fallback to the current level's name
                  if ($lowerLevel) {
                        $item->lastgrade = DB::table('gradelevel')
                              ->where('id', $lowerLevel->levelid)
                              ->value('levelname') ?? 'No records Found';
                  } else {
                        // Fallback to current level's name
                        $item->lastgrade = DB::table('gradelevel')
                  ...
                  if (($item->levelid >= 17 && $item->levelid <= 21)) {
                        ...
                        if ($schoolinfo->shssetup == 1) {
                              ...
                              if ($school_info->abbreviation == 'VNBC') {
                                    $temp_chrngrans = collect($chngtrans)
                                          ->where('studid', $item->studid)
                                          ->where('semid', $semid)
                                          ->values();
      ...
      public static function enrollment_history(Request $request)
      {
            ...
            $history_records = DB::table('enrollment_history')
                  ...
                  ->select(
                        ...
                        'enrollment_history.studstatus'
                  )
                  ->orderBy('enrollment_history.created_at', 'DESC')
                  ->get();

            // Track unique combinations of studstatus and created_at date (without time)
            $uniqueKeys = [];
            foreach ($history_records as $item) {
                  $dateWithoutTime = \Carbon\Carbon::create($item->dateenrolled)->format('Y-m-d');
                  $key = $item->studstatus . '-' . $dateWithoutTime;
      ...
      public static function documents_list(Request $request)
      {
            ...
            $syid = $request->get('syid');
            $studid = $request->get('studid');
            $studinfo = DB::table('studinfo')->where('id', $studid)->select('sid', 'levelid')->first();


            $sid = $studinfo->sid;

            // $levelid = DB::table('studinfo')->where('id',$studid)->select('levelid')->first()->levelid;
            ...
            $request->request->add(['sid' => $studinfo->sid]);

            $documents = \App\Http\Controllers\SuperAdminController\SubmittedRequirements::uploadeddocs($request);
            // return $documents;

            $tempdocs = array();
            foreach ($documents as $document) {
                  foreach ($document->uploaded as $uploadeditem) {
                        array_push($tempdocs, $uploadeditem);
                  }
            }
            ...
            try {
                  ...
                  if ($acadprogid == 5) {
                        ...
                        DB::table('enrolledstud')->insert([
                              'studmol' => $mol,
                              'ghssemid' => empty($semid) || $semid == 0 ? 1 : $semid,
                              'remarks' => $remarks,
                              'grantee' => $grantee,
                              'studid' => $studid,
                              'syid' => $syid,
                  ...
                  if ($schoolinfo->schoolid == '410400') {
                        if ($parent_contact != null) {
                              DB::table('smsbunker')
                                    ->insert([
                                          'message' => 'Welcome to CENTERPHIL BACOLOD. CONGRATULATIONS! ' . $studinfo->firstname . ' you are officially enrolled for S.Y ' . $sydesc . '.',
                        ...
                        if ($student_contact != null) {
                              DB::table('smsbunker')
                                    ->insert([
                                          'message' => 'Welcome to CENTERPHIL BACOLOD. CONGRATULATIONS! You student' . $studinfo->firstname . ' is now officially enrolled for S.Y ' . $sydesc . '.',
                  ...
                  $tuitionheader = db::table('tuitionheader')
                        ...
                        ->where('tuitionheader.grantee', $grantee)
                        ->where(function ($q) use ($levelid, $semid, $courseid, $strandid) {
                              if ($levelid == 14 || $levelid == 15) {
                                    if ($semid == 3) {
                                          $q->where('tuitionheader.semid', $semid);
                                          $q->where('tuitionheader.strandid', $strandid);
                                    } else {
                                          if (FinanceModel::shssetup() == 0) {
                                                $q->where('semid', $semid);
                                                $q->where('strandid', $strandid);
                                    ...
                                    $q->where('tuitionheader.semid', $semid);
                                    $q->where('tuitionheader.courseid', $courseid);
                        ...
                        $tuitionheader = db::table('tuitionheader')
                              ...
                              ->where(function ($q) use ($levelid, $semid, $courseid, $strandid) {
                                    if ($levelid == 14 || $levelid == 15) {
                                          if ($semid == 3) {
                                                $q->where('tuitionheader.strandid', $strandid);
                                                $q->where('tuitionheader.semid', null);
                                          } else {
                                                if (FinanceModel::shssetup() == 0) {
                                                      $q->where('semid', $semid);
                                                      $q->where('semid', null);
                                          ...
                                          $q->where('tuitionheader.semid', $semid);
                                          $q->where('tuitionheader.courseid', null);
                  ...
                  $request->merge(['feesid' => $tuitionheader->first()->id]);
      ...
      public static function update_student_information(Request $request)
      {
            try {
                  ...
                  $checklrn = DB::table('studinfo')
                        ->where(function ($query) use ($request) {
                              if ($request->get('lrn') != null) {
                                    $query->where('lrn', $request->get('lrn'));
                              }
                        })
                  ...
                  if ($checklrn > 0 && $request->get('lrn') != null) {
                        // dd($checklrn, $request->get('lrn'));
                        return 'LRN Exists';
                  }
