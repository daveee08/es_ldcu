    ...
    public static function getbracket_standard($employeeid = null, $rate = null, $particularid = null, $whattoget=null)
    {
        ...
        if($particularinfo)
        {
            ...
            if($particularinfo->brackettype == 1) //Pag-Ibig
            {
                ...
                if($getratebracket):
                    // return collect($getratebracket);
                    $amount = number_format($rate*($getratebracket->eescrate/100),2);
                endif;
            }
                ...
                if(count($deductiontypes)>0)
                {
                    foreach($deductiontypes as $eachstandard)
                    {
                        ...
                        if($employee->amount > 0 )
                        {
                            ...
                            {
                                $defaultamount = self::getbracket_standard($employee->id, $employee->amount, $eachstandard->id,'amount');
                            }else{
                            ...
                            if($info)
                            {
                                ...
                                if($eachstandard->brackettype > 0)
                                {
                                    if($info->eesamount != self::getbracket_standard($employee->id, $employee->amount, $eachstandard->id,'amount'))
                                    {
                                        $automatic = 0;
                                        $standardstatus = 1;
                                    }else{
                                ...
                                if($eachstandard->brackettype > 0)
                                {
                                    $getdeductionamount = self::getbracket_standard($employee->id, $employee->amount, $eachstandard->id,'amount');
                                }else{
                                    $getdeductionamount = $eachstandard->amount;;
                                }
                                DB::table('employee_deductionstandard')
                                    ->insert([
                        ...
                        array_push($contributions, (object)array(
                            ...
                            'cont_amount'   => $getdeductionamount == null ? '0.00' : $getdeductionamount,
