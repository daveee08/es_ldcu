...
class HRPayrollV3Controller extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('hr.payroll.v3.indexv3')
    ...
    public function getsalaryinfo(Request $request){
        ...
        // get basic salary info sa employee
        ...
        if($basicsalaryinfo)
        {
            ...
            //this will get the dates between the "date from" and "date to"
            ...
            elseif(strtolower($basicsalaryinfo->salarytype) == 'daily')
            {
                ...
                $basicsalaryinfo->amountperhour = $basicsalaryinfo->amountperday/$basicsalaryinfo->hoursperday;
                $basicsalaryinfo->amount = ($basicsalaryinfo->amountperday*count($dates));
            }
        ...
        $workingdays = collect($attendance)->count();
        $dayspresent = collect($attendance)->where('totalworkinghours','>',0)->count();
        $daysabsent = collect($attendance)->where('totalworkinghours',0)->count();
        $lateminutes = collect($attendance)->where('totalworkinghours','>',0)->sum('latehours')*60;
        $undertimeminutes = collect($attendance)->where('totalworkinghours','>',0)->sum('undertimehours')*60;
        $totalworkedhours = collect($attendance)->where('totalworkinghours','>',0)->sum('totalworkinghours');
        
        
        // compute late deduction
        $totalamountdeductlate = 0;
        ...
        if(count($attendance)>0 && count($tardiness_computations)>0) //get the am and pm lates (converted to minutes)
        {
            foreach($attendance as $eachatt)
            {
              ...
              $totalamountdeductlate += $eachatt->amountdeduct ?? 0;
            }

        }
        $totalamountdeductabsent = 0;
        // compute absent deduction
        if ($basicsalaryinfo) {
            if ($basicsalaryinfo->amountperday) {
                $totalamountdeductabsent =  $daysabsent * $basicsalaryinfo->amountperday;
            }
        ...
        if(count($standardallowances)>0){
            foreach($standardallowances as $allowancetype){
                $eachallowance = \App\Models\HR\HRAllowances::getstandardallowances($request->get('employeeid'), $payrollperiod, $allowancetype->empallowanceid); //get the information of the allowance, this will also check if it is already paid for the whole month or just by 15th
                    ...
                    if(strtolower(date('D',strtotime($leave->ldate))) == 'mon')
                    {
                        if($basicsalaryinfo->mondays == 1 && $getpay->withpay == 1)
                        {
                            ...
                            $leave->amount = ($basicsalaryinfo->saturdayhours*$basicsalaryinfo->amount);
                                ...
                                $leave->amount = $basicsalaryinfo->amountperday;
                            }
                        }
                    } 
        ...
        if(count($otherdeductions)>0){
            ...
            {   
                // gi check niya dri if naa nay nag exist nga payment
                ...
                ->join('hr_payrollv2','hr_payrollv2history.payrollid','=','hr_payrollv2.id')
                ->where('hr_payrollv2history.employeeid', $request->get('employeeid'))
                ...
                // return collect($paiddeductions);

                // if walay sulod ang $paiddeductions
                            ...
                            if(collect($monthlypayroll)->where('payrollid', $payrollperiod->id)->first()->released == 1)
                            {
                                ...
                                if($deductinfo)
                                {
                                    ...
                                    if($eachotherdeduction->term == 0)                                    
                                    {
                                        ...
                                        $eachotherdeduction->amounttopay = $eachotherdeduction->totalamount > 0 ? ($eachotherdeduction->totalamount/$eachotherdeduction->term) : 0;
                                        $eachotherdeduction->totalamount = $eachotherdeduction->amount > 0 ? ($eachotherdeduction->amount/$eachotherdeduction->term): 0;
                                    }
                                    $eachotherdeduction->lock        = 1;
                                    // return  $eachotherdeduction->totalamount;
                                }else{
                                    if($deductinfo)
                                    {
                                        $eachotherdeduction->amounttopay = $eachotherdeduction->amount;
        ...
        $amounttobededuct = $totalamountdeductabsent + $totalamountdeductlate;

        // return collect($basicsalaryinfo);
        $netsalary = $basicsalaryinfo->amount - $amounttobededuct;
        ...
        // return collect($amounttobededuct);
        return view('hr.payroll.v3.getsalaryinfo')
            ->with('basicsalaryinfo',$basicsalaryinfo)
            ->with('payrollperiod',$payrollperiod)
            ->with('attendance',$attendance)
            ->with('employeeinfo',$employeeinfo)
            ->with('dayspresent',$dayspresent)
            ->with('daysabsent',$daysabsent)
            ->with('lateminutes',$lateminutes)
            ->with('totalworkedhours',$totalworkedhours)
            ->with('totalamountdeductlate',$totalamountdeductlate)
            ->with('totalamountdeductabsent',$totalamountdeductabsent ?? '' )
            ->with('workingdays',$workingdays)
            ->with('undertimeminutes',$undertimeminutes)
            ...
            ->with('netsalary',$netsalary);
            
    }

    // public function getsalaryinfobackup(Request $request)
    // {
    //     // return $request->all();
    //     $basicsalaryinfo = DB::table('employee_basicsalaryinfo')
    //         ->select('employee_basicsalaryinfo.*','employee_basistype.type as salarytype','employee_basistype.type as ratetype')
    ...
    //         {
    //             //get the attendance (per day) information including the late details
    ...
    //     if($basicsalaryinfo)
    //     {
    //         $basicsalaryinfo->amountperday = $basicsalaryinfo->amountperday;
    ...
    { 
        $employeeid = $request->get('employeeid');
        $payrollid = $request->get('payrollid');
        $basicpay = $request->get('basicpay');
        $dailyrate = $request->get('dailyrate');
        $workingdays = $request->get('workingdays');
        $basicsalaryamount = $request->get('basicsalaryamount');
        $basicsalarytype = $request->get('basicsalarytype');
        $presentdays = $request->get('presentdays');
        ...
        $totalearnings = $request->get('totalearnings');
        $totaldeductions = $request->get('totaldeductions');
        $netsalary = $request->get('netsalary');
        ...
        if($checkhistoryifexists){
            ...
            $headerid = DB::table('hr_payrollv2history')
                ->update([
                    ...
                    'presentdays'           => $request->get('presentdays'),
                    'absentdays'            => $request->get('absentdays'),
                    'basicsalaryamount'     => str_replace( ',', '', $request->get('basicsalaryamount')),
    ...
    // public function configurationbackup(Request $request)
    // {            
    //     // return $request->all();
    //     $employeeid = $request->get('employeeid');
    //     $payrollid = $request->get('payrollid');
