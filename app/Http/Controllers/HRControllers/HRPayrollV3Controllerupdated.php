...
class HRPayrollV3Controller extends Controller
{
    ...
    //    return $request->all();
        ...
        // Now you can access the calculated totalotherdeductionpaid for each $otherdeduction
        // foreach ($otherdeductions as $otherdeduction) {
        //     echo "Deduction Description: " . $otherdeduction->description . "\n";
        //     echo "Total Deduction Paid: " . $otherdeduction->totalotherdeductionpaid . "\n";
        //     echo "\n";
        // }
        // return collect($payrollperiod);
            ...
            foreach($otherdeductions as $eachotherdeduction)
            {
                ...
                if($paiddeductions == null || $paiddeductions == 0)
                {
                        ...
                        if($payrollinfo)
                        {
                            ...
                            if($payrollinfo->id == $monthlypayroll[0]->id)
                            {
                                ...
                                if($deductinfo)
                                {
                                    ...
                                    $eachotherdeduction->amounttopay = ($eachotherdeduction->amount > 0 && $eachotherdeduction->term > 0) ? ($eachotherdeduction->amount/$eachotherdeduction->term) : $eachotherdeduction->amount;
                                    $eachotherdeduction->totalamount = $eachotherdeduction->amounttopay;
                                }
                        ...
                        if($deductinfo)
                        {
                            if(collect($monthlypayroll)->where('payrollid', $payrollperiod->id)->first()->released == 1)
                            {
                                ...
                                if($deductinfo)
                                {
                                    ...
                                    $eachotherdeduction->lock        = 1;
                                    $eachotherdeduction->paidstatus = $deductinfo->paidstatus;
                                    // $eachotherdeduction->totalotherdeductionpaid =  $totalotherdeductionpaid;
                                }
                                ...
                                if($deductinfo)
                                {
                                    ...
                                    if ($eachotherdeduction->term != 0) {
                                        ...
                                        $eachotherdeduction->amounttopay = ($eachotherdeduction->totalamount);
                                        $eachotherdeduction->totalamount = ($eachotherdeduction->amount);
                                        $eachotherdeduction->lock        = 1;
                                    }
                    ...
                    if($eachotherdeduction->amount)
                    {
                        ...
                        if ($eachotherdeduction->deductionstatus == 1) {
                            $eachotherdeduction->amounttopay = $eachotherdeduction->amount;
                            $eachotherdeduction->totalamount = $eachotherdeduction->amount;
                            $eachotherdeduction->paidstatus = 1;
                            $eachotherdeduction->paymenttype = 1;
                        }
                        // return count($monthlypayroll);
                        if(count($monthlypayroll) == 0)
                        {
                            if($eachotherdeduction->term == 0)
                            {
                        ...
                        elseif(count($monthlypayroll) == 1)
                        {
                            ...
                            if($payrollinfo)
                            {
                                ...
                                if(date('Y-m', strtotime($monthlypayroll[0]->datefrom)) == date('Y-m', strtotime($payrollperiod->datefrom)))
                                {
                                    ...
                                    // return $monthlypayroll[0]->datefrom;
                                    // return 'masaya';
                                    if($deductinfo)
                                    {
                                        if($deductinfo->paymenttype == 1)
                                        ...
                                        if($deductinfo)
                                        {
                                            ...
                                            $eachotherdeduction->amounttopay = $eachotherdeduction->amounttobededuct;
                                            $eachotherdeduction->totalamount = $eachotherdeduction->amounttobededuct;
                                        }
        ...
        if ($employeecustomsched) {
            ...
            //         if ($att->status != 1 && $att->holiday != 1) {
            //             foreach ($leavedetails as $leavedetail) {
            //                 if ($att->date == $leavedetail->ldate) {
            //                     $att->amtimein = $employeecustomsched->amin;
            //                     $att->amtimeout = $employeecustomsched->amout;
            //                     $att->pmtimein = $employeecustomsched->pmin;
            //                     $att->pmtimeout = $employeecustomsched->pmout;
            ...
            //                 }
            //             }
            //         // } else if ($att->status == 1 && $att->holiday != 1) {
            //         } else {
            //             // return 'd';
            //             if ($att->date == "2024-01-29") {
            //                 return $att->date;

            //             }
            //             foreach ($leavedetails as $leavedetail) {
            //                 if ($leavedetail->halfday == 1 && $att->date == $leavedetail->ldate) {
            //                     return $leavedetail;
            ...
            //                     $att->totalworkinghours = floor(($att->totalworkinghours + $att->lateamhours) * 100) / 100;
            ...
            //                 } else if($leavedetail->halfday == 2 && $att->date == $leavedetail->ldate){
            //                     $att->pmtimein = $employeecustomsched->pmin;
            //                     $att->pmtimeout = $employeecustomsched->pmout;
            //                     $att->timeinpm = $employeecustomsched->pmin;
            //                     $att->timeoutpm = $employeecustomsched->pmout;
            ...
            //                 } else if($leavedetail->halfday === 0 && $att->date == $leavedetail->ldate){
            //                     $att->amtimein = $employeecustomsched->amin;
            //                     $att->amtimeout = $employeecustomsched->amout;
            //                     $att->pmtimein = $employeecustomsched->pmin;
            //                     $att->pmtimeout = $employeecustomsched->pmout;
        ...
        if ($activedaterange) {
            ...
            foreach ($activeemployeetardy as $tardy) {
                ...
                } else if ($tardy->type == 'Parttime Late' || $tardy->type == 'Parttime Undertime' || $tardy->type == 'Parttime Absent') {
        ...
        if ($request->get('action') == 'modaldetails') {
            $data = [
                ...
                'collegeregularload', $clregulardata ?? null,
                'collegeoverloadload', $cloverloaddata ?? null,
                'collegeparttimeload', $clparttimedata ?? null,
                'sumregularload', $sumClRegular ?? 0,
                'sumoverloadload', $sumClOverload ?? 0,
                'sumparttimeload', $sumClPartTime ?? 0,
                'regulardload', $regulardload ?? 0,
                'overload', $overload ?? 0,
                'emergencyload', $emergencyload ?? 0,
                'peroras', $peroras
            ];
    ...
    {   
        if($request->get('action') == 'delete')
        {
            ...
            DB::table('hr_payrollv2additionalearndeduct')
                ->where('id', $request->get('additionalid'))
                ->where('employeeid', $request->get('employeeid'))
                ->update([
                    'deleted'           => 1,
                    'deletedby'         => auth()->user()->id,
        ...
        if($request->get('action') == 'update')
        {
            try{
                ...
                if($checkifexists)
                {
                    ...
                    // return $paiddeduction_all;
                    // $paiddeductions_all = DB::table('hr_payrollv2history')
                    //     ->select(DB::raw('SUM(`amountpaid`) as amountpaid'),
                    //             'hr_payrollv2history.employeeid',
                    //             'hr_payrollv2historydetail.particularid',
                    //             'hr_payrollv2history.payrollid'
                    //     )
                    //     ->leftjoin('hr_payrollv2historydetail','hr_payrollv2history.id','=','hr_payrollv2historydetail.headerid')
                    //     ->leftjoin('hr_payrollv2','hr_payrollv2history.payrollid','=','hr_payrollv2.id')
                    //     // ->where('hr_payrollv2history.employeeid', $request->get('employeeid'))
                        ...
                        // return collect($basicsalaryinfo);
                            ...
                            if ($basicsalaryinfo && isset($basicsalaryinfo->attendancebased) && $basicsalaryinfo->attendancebased == 1) {
                                ...
                                if ($regulardload > 0) {
                                    ...
                                    $regulardloadtardyamount = $regulardload * round($basicsalaryinfo->amountperhour / 60, 2);
                                } 
                                if ($overload > 0) {
                                    $overloadtardyamount = $overload * $perminute;
                                }
                                if ($emergencyload > 0) {
                                    $emergencyloadtardyamount = $emergencyload * $perminute;
                                }
                                ...
                                if (!empty($dates)) {
                                    ...
                                    if ($employeecustomsched) {
                                        ...
                                        foreach ($attendance as $attendancefinal) {
                                            ...
                                            if ($attendancefinal->date = "2024-04-03") {
                                                $amlate = floatval($attendancefinal->lateamminutes);
                                                $pmlate = floatval($attendancefinal->latepmminutes);
                                                $amlatehours = floatval($attendancefinal->lateamhours);
                                                $pmlatehours = floatval($attendancefinal->latepmhours);
                                                ...
                                                if ($attendancefinal->amin == null && $attendancefinal->amout != null && $attendancefinal->pmout == null) {
                                                    $attendancefinal->lateamminutes = 0;
                            ...
                            // $otherdeductions = collect($otherdeductions_all)->where('employeeid', $employeeinfo->id)->where('startdate','<=', $payrollperiod->datefrom)->where('startdate','<=', $payrollperiod->dateto)->values();
                            ...
                            if(count($otherdeductions)>0){
                                foreach($otherdeductions as $eachotherdeduction)
                                {
                                    ...
                                    // if ($formattedStartDate == $payrollperiod->dateto) {
                                        ...
                                        // $paiddeductions = collect($paiddeductions_all)->where('employeeid', $employeeinfo->id)->where('particularid', $eachotherdeduction->id)->where('payrollid','<=',$payrollperiod->id)->first()->amountpaid ? $paiddeductions->amountpaid : 0 ;
                                        
                                        // return collect($eachotherdeduction);
                                        
                                        $paiddeductionItem = collect($paiddeductions_all)
                                            ->where('employeeid', $employeeinfo->id)
                                            ->where('particularid', $eachotherdeduction->id)
                                            ->where('payrollid', '<=', $payrollperiod->id)
                                            ->first();

                                        // return collect($paiddeductionItem);
                                    
                                        // $paiddeductions = $paiddeductionItem ? $paiddeductionItem->map(function ($item) {
                                        //     return $item ?? 0;
                                        // }) : 0 ;
                                        if ($paiddeductionItem) {
                                            // Check if it's a string or an array
                                            $paiddeductions = is_string($paiddeductionItem->amountpaid) ? (float)$paiddeductionItem->amountpaid : $paiddeductionItem->amountpaid;
                                        } else {
                                            $paiddeductions = 0;
                                        }
                                        ...
                                        if($paiddeductions == null || $paiddeductions == 0)
                                        {
                                            ...
                                            if(count($monthlypayroll) == 0)
                                            {
                                                if ($eachotherdeduction->description == 'PERAA') {
                                                    ...
                                                    $eachotherdeduction->amounttopay = $eachotherdeduction->amount / 2;
                                                    $eachotherdeduction->totalamount = $eachotherdeduction->amount;
                                                    $eachotherdeduction->paidstatus = 1;
                                                    $eachotherdeduction->paymenttype = 1;
                                                }
                                            ...
                                            {          
                                                ...
                                                if($payrollinfo)
                                                {
                                                    if($payrollinfo->id == $monthlypayroll[0]->id)
                                                    {
                                                        ...
                                                        $verytotalamount = 0;
                                                        $amounttopaidis = 0;
                                                        $verytotalamountdivide = 0;
                                                        foreach ($otherdeductions as $other) {
                                                            if ($other->id == $eachotherdeduction->id) {
                                                                $verytotalamount = $other->amount;
                                                            }
                                                        }

                                                        $deductinfo = collect($deductinfo_all)->where('headerid', $payrollinfo->id)->where('particulartype', 2)->where('particularid',$eachotherdeduction->id)->first();
            
            
                                                        // return $verytotalamount;
                                                        // return collect($deductinfo);
                                                        if($deductinfo)
                                                        {

                                                            if ($deductinfo->paymenttype == 1) {
                                                                $amounttopaidis = $verytotalamount / 2;
                                                                $verytotalamountdivide = $verytotalamount / 2;
                                                            } else if($deductinfo->paymenttype == 0){
                                                                $amounttopaidis = $verytotalamount;
                                                                $verytotalamountdivide = $verytotalamount / 2;

                                                            }
                                                            // return $verytotalamountdivide;
                                                            
                                                            if ($deductinfo->amountpaid == $verytotalamountdivide) {
                                                                $eachotherdeduction->paymenttype = 1;
                                                            } else if($deductinfo->amountpaid == $verytotalamount){
                                                                $eachotherdeduction->paymenttype = 0;
                                                            }

                                                            // $eachotherdeduction->paymenttype = $deductinfo->paymenttype;
                                                            // $eachotherdeduction->amounttopay = $deductinfo->amountpaid;
                                                            $eachotherdeduction->amounttopay = $amounttopaidis;
                                                            // $eachotherdeduction->totalamount = $deductinfo->totalamount;
                                                            $eachotherdeduction->totalamount = $verytotalamount;

                                                            // return collect($deductinfo);
                                                            // if ($eachotherdeduction->startdate >= $payrollperiod->datefrom && $eachotherdeduction->enddate <= $payrollperiod->dateto) {
                                                            // if ($eachotherdeduction->enddate >= $payrollperiod->datefrom || $eachotherdeduction->startdate <= $payrollperiod->dateto) {
                                                            // if ($eachotherdeduction->startdate == $payrollperiod->datefrom || $eachotherdeduction->startdate < $payrollperiod->datefrom) {
                                                            if ($formattedStartDate < $payrollperiod->datefrom || $formattedStartDate == $payrollperiod->datefrom || $formattedStartDate == $payrollperiod->dateto) {
                                                            //if ($eachotherdeduction->startdate <= $payrollperiod->datefrom || $eachotherdeduction->startdate <= $payrollperiod->dateto) {
                                                                // $eachotherdeduction->paidstatus = $deductinfo->paidstatus;
                                                                $eachotherdeduction->paidstatus = 1;
                                                            } else {
                                                                $eachotherdeduction->paidstatus = 0;
                                                        ...
                                                        if(date('Y-m', strtotime($monthlypayroll[0]->datefrom)) == date('Y-m', strtotime($payrollperiod->datefrom)))
                                                        {
                                                            ...
                                                            //     ->where('particularid',$eachotherdeduction->id)
                                                            //     ->first();
                                                            $deductinfo = collect($deductinfo_all)->where('headerid', $monthlypayroll[0]->id)->where('particulartype', 2)->where('particularid',$eachotherdeduction->id)->first();
                                            ...
                                            {                   
                                                ...
                                                try{
                                                    ...
                                                    $deductinfo = collect($deductinfo_all)->where('headerid', collect($monthlypayroll)->where('payrollid', $payrollperiod->id)->first()->id)->where('particulartype',2)->where('particularid',$eachotherdeduction->id)->first();
                                                ...
                                                if($deductinfo)
                                                {
                                                    ...
                                                    if($payrollinfo)
                                                    {
                                                        ...
                                                        $deductinfo = collect($deductinfo_all)->where('headerid', collect($monthlypayroll)->where('payrollid', '!=',$payrollinfo->id)->first()->id)->where('particulartype',4)->where('particularid',$eachotherdeduction->id)->first();
                                            ...
                                            if($eachotherdeduction->amount)
                                            {
                                                    ...
                                                    if($payrollinfo)
                                                    {
                                                        ...
                                                        if ($basicsalaryinfo) {
                                                            if ($basicsalaryinfo->salarybasistype == 9) {
                                                                ...
                                                                $deductinfo = collect($deductinfo_all)->where('headerid', $monthlypayroll[0]->id)->where('particulartype',2)->where('particularid',$eachotherdeduction->id)->where('hr_payrollv2historydetail.payrollid', $payrollperiod->id)->first();
                                                ...
                                                {       
                                                    ...
                                                    $deductinfo = collect($deductinfo_all)->where('headerid', collect($monthlypayroll)->where('payrollid', $payrollperiod->id)->first()->id ?? 0)->where('particulartype',2)->where('particularid',$eachotherdeduction->id)->first();
                                                    ...
                                                    if($deductinfo)
                                                    {
                                                        ...
                                                        if(isset($payrollinfo->id))
                                                        {
                                                            ...
                                                            //     ->where('particularid',$eachotherdeduction->id)
                                                            //     ->first();
                                                            $deductinfo = collect($deductinfo_all)->where('headerid', collect($monthlypayroll)->where('payrollid', '!=',$payrollinfo->id)->first()->id)->where('particulartype',2)->where('particularid',$eachotherdeduction->id)->first();
                                                            if($deductinfo)
                                                            {
                                                                if($deductinfo->paymenttype == 0)
                                                                {
                            ...
                            if (count($otherdeductions)>0) {
                                foreach ($otherdeductions as $otherdeduction) {
                                    if (isset($otherdeduction->paidstatus) == 1) {
                            ...
                            // return $otheraddeddeductions;
                            $particulars = array_merge($standardallowances->toArray(), $standarddeductions, $otherdeductions->toArray(), $otheraddedearnings->toArray(), $otheraddeddeductions->toArray());
                            ...
                            // return $otheraddeddeductionsamount;
                            ...
                            // return collect($checkhistoryifexists);
                            // $checkhistoryifexists = DB::table('hr_payrollv2history')
                            //     ->where('payrollid', $payrollinseredid)
                            //     ->where('employeeid', $employeeinfo->id)
                            //     ->where('deleted','0')
                            ...
                            // }
                                            ...
                                            if (property_exists($particular, 'paidstatus') && $particular->paidstatus == 1) {
                                                ...
                                                if(collect($allparts)->where('particulartype', $particular->particulartype)->where('particularid', $particular->particularid)->count()>0)
                                                {
                                                    ...
                                                    // return collect($allparts)->where('particulartype', $particular->particulartype)->where('particularid', $particular->particularid)->first()->id;
                                                        ...
                                                        $balance = $particular->totalamount - $amountpaid;
                                ...
                                //     if(count($allparts) == 0)
                                //     {
                                //         foreach($otheraddedearnings as $addedparticular)
                                //         {
                                //             $detailid = DB::table('hr_payrollv2addparticular')
                                //                 ->insertGetId([
                                //                     'payrollid'             => $checkifexistsid,
        ...
        try{
            ...
            foreach ($employees as $employeeinfo) {
                ...
                // return collect($basicsalaryinfo);
                    ...
                    // return $otheraddedearningsamount;
    ...
    public function exportpayslip(Request $request)
    {
        ...
        if($request->get('exporttype') == 1)
        {
            ...
            if(strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'hccsi')
            {
                ...
                $pdf = PDF::loadview('hr/payroll/v3/pdf_singlev4',compact('schoolinfo','header','particulars','payrollinfo','addedparticulars','payrolldetail','leavedetails','otherdeductions', 'holidays','employeeinfo','preparedby'));
            }
    ...
    public static function exportpayslipbydepartment(Request $request){
        ...
        if($request->get('exporttype') == 2)
        {
            ...
            $payrolldetails = DB::table('hr_payrollv2historydetail')
                // ->leftJoin('hr_payrollv2history', 'hr_payrollv2historydetail.payrollid', '=', 'hr_payrollv2history.payrollid')
                ->where('hr_payrollv2historydetail.particulartype', 2)
                // ->where('hr_payrollv2historydetail.employeeid', 133)
            ...
            foreach ($otherdeductions as $otherdeduction) {
                foreach ($payrolldetails as $payrolldetail) {
                    if ($payrolldetail->deductionid == $otherdeduction->deductionotherid && $payrolldetail->employeeid == $otherdeduction->employeeid) {
    ...
    public function batchgenerateemployeepayroll(Request $request){
        ...
        $spreadsheet = $reader->load("PAYROLLSUMMARY/DCC/payrollgenerate.xlsx");
        ...
        foreach ($employeeidsrrays as $employeeidsrray) {
            ...
            foreach($generateddatahistorys as $generateddatahistory)
            {
                ...
                // return $mergedaddedparticularss;
                ...
                $finaldetails = $mergdata;
                // $finaldetails = array_merge($generateddatadetails->toArray());

                // foreach ($finaldetails as $detail) {
                //     // Check if both deductionunder and deductionunderdesc are not set or are null
                //     if ((!isset($detail->deductionunder) || $detail->deductionunder === null) && (!isset($detail->deductionunderdesc) || $detail->deductionunderdesc === null)) {
                //         $description = $detail->description;
                
                //         // Check if the description exists in $filterheader
                //         if (in_array($description, $filterheader)) {
                //             $columnIndex = array_search($description, $filterheader);
                
                //             // Check if the column index is found
                //             if ($columnIndex !== false) {
                //                 // Initialize $matchingAmounts as an array
                //                 $matchingAmounts = array(floatval(isset($detail->combinedamount) ? $detail->combinedamount : 0));
                
                //                 // Check if $matchingAmounts is not empty before calculating the sum
                //                 if (!empty($matchingAmounts)) {
                //                     $detail->combinedtotalamount = array_sum($matchingAmounts);
                //                 }
                //             }
                //         }
                //     }
