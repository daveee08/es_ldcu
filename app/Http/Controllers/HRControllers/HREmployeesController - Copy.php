...
class HREmployeesController extends Controller
{
    ...
    public function getprofile(Request $request)
    {
        ...
        $employeeinfo = DB::table('teacher')
            ->select(
                ...
                'teacher.title','teacher.lastname','teacher.firstname','teacher.middlename','teacher.suffix','teacher.licno','teacher.isactive','teacher.tid','teacher.picurl','teacher.rfid','teacher.usertypeid','teacher.employmentstatus as empstatus','usertype.utype as designation','hr_empstatus.description as employmentstatus')
            ->leftJoin('employee_personalinfo','teacher.id','=','employee_personalinfo.employeeid')
            ->leftJoin('usertype','teacher.usertypeid','=','usertype.id')
    ...
    public function addnewemployeeindex(Request $request)
    {
        ...
        return view('hr.employees.addnewemployee_v2')
        ...
        if(count($checkifexists) == 0){
            ...
            $newemployeeid = DB::table('teacher')
                ->insertGetId([
                    ...
                    'schooldeptid'                 =>  $request->get('departmentid'),
            ...
            DB::table('employee_personalinfo')
                ->insert([
                    ...
                    'numberofchildren'      =>  $request->get('noofchildren'),
                    'emercontactname'       =>  $request->get('emername'),
                    'emercontactrelation'   =>  $request->get('emerrelation'),
                    'emercontactnum'        =>  $request->get('emercontactno'),
            ...
            if($request->has('account-tin'))
            {
                if($request->get('account-tin') != null && $request->get('account-tin') != "" && $request->get('account-tin') != " ")
                {
                    DB::table('employee_accounts')
                        ->insert([
                            'employeeid'            => $newemployeeid,
                            'accountdescription'    => 'T.I.N',
                            'accountnum'            => $request->get('account-tin'),
                            'createdby'             => auth()->user()->id,
                            'createddatetime'       => date('Y-m-d H:i:s')
                        ]);
                }
            }
            if($request->has('educ-degree'))
            {
                if($request->get('educ-degree') != null && $request->get('educ-degree') != "" && $request->get('educ-degree') != " ")
                {
                    DB::table('employee_educationinfo')
                        ->insert([
                            'employeeid'            => $newemployeeid,
                            'coursetaken'    => $request->get('educ-degree'),
                            'major'            => $request->get('educ-major'),
                            'createdby'             => auth()->user()->id,
                            'createddatetime'       => date('Y-m-d H:i:s')
                        ]);
                }
