...
class ClgCollegeAttendanceController extends Controller
{
    ...
    public function getFilterSubject(Request $request){
        ...
        ->where('college_classsched.deleted', 0)
        ->where('college_classsched.teacherID', $teacher->id)
        ->where('college_classsched.syID', $request->syid)
        ->where('college_classsched.semesterID', $request->semid)
        ->get();
    ...
    public function clgattndcGeneratePDF($syid, $subjectid, $sectionid , $schedid ,$semid, $arraymonth){
    ...
    public function collegeattendancestore(Request $request)
    {
        // Decode the JSON string if it's coming as a JSON string
        ...
        $syid = $request->input('syid');
        $semid = $request->input('semid');
        $subjectID = $request->input('subjectID'); // Get subjectID
        $sectionID = $request->input('sectionID'); // Get sectionID
        ...
        foreach ($data as $entry) {
            ...
            DB::table('college_attendance')->updateOrInsert(
                [
                    ...
                    'syid' => $syid, 
                    'semid' => $semid,
                    'subjectid' => $subjectID, // Use subjectID from the request
                    'sectionid' => $sectionID, // Use sectionID from the request
                    'monthid' => $monthId,
                    'yearid' => $yearId
                ],
                $attendance
