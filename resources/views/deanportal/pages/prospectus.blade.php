...
@section('content')
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-primary">
                    <button class="btn btn-default " data-toggle="modal" data-target="#curriculumModalForm"
                        data-widget="chat-pane-toggle" id="createSubject">Create Curriculum</button>
    ...
    <div class="modal fade" id="subjectsTable" style="display: none;" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-primary">
                    <button class="btn btn-default " data-toggle="modal" data-target="#subjectModal"
                        data-widget="chat-pane-toggle" id="createSubject">CREATE SUBJECT</button>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true" class="text-white">×</span>
                    </button>
                </div>
                    ...
                    <div class="form-group">
                        <select id="select_curriculum_to_add" class="form-control form-control-sm float-right">
                            ...
                            @foreach (DB::table('college_curriculum')->where('courseID', $courseInfo->id)->where('deleted', 0)->get() as $item)
                                <option value="{{ $item->id }}" @if ($item->isactive == 1) selected @endif>
            ...
            <div class="col-md-3">
                <div class=" card">
                    ...
                    <div class="card-footer">
                        ...
                        <hr>
                        <button class="btn btn-block btn-primary btn-sm" data-value="1" id="updatepros">View
                            options</button>
        ...
        $(document).ready(function() {
            ...
            $(document).on('input', '#searchsubject', function() {
                ...
                if ($('#prerequisitesubjectlist').is(':visible')) {
                    ...
                    var url = '/collegesubjects?table=table&subjid=' + $(this).attr('data-id') +
                        '&currid=' + $('#curriculum').val() + '&yearid=' + selectedYear + '&semID=' +
                        selectedsem + '&course=' + course + '&search=' + $(this).val() + '&status=3'

                } else {

                    var url = '/collegesubjects?table=table&subjid=' + $(this).attr('data-id') +
                        '&currid=' + $('#curriculum').val() + '&yearid=' + selectedYear + '&semID=' +
                        selectedsem + '&course=' + course + '&search=' + $(this).val()
                }
            ...
            $(document).on('click', '.addsubjecttoprospectus', function() {
                ...
                $.ajax({
                    url: '/collegesubjects?insertoprospectus=insertoprospectus&subjid=' + $(this)
                        .attr('data-id') + '&currid=' + $('#curriculum').val() + '&yearid=' +
                        selectedYear + '&semID=' + selectedsem + '&course=' + course,
                    type: 'GET',
                    success: function(data) {
            ...
            $(document).on('click', '.removesubjectfromprospectus', function() {
                ...
                $.ajax({
                    url: '/collegesubjects?removefromprospectus=removefromprospectus&prosid=' + $(
                            this).attr('data-id') + '&currid=' + $('#curriculum').val() +
                        '&yearid=' + selectedYear + '&semID=' + selectedsem + '&course=' + course,
                    type: 'GET',
                    success: function(data) {
                        console.log(data)
            ...
            $(document).on('click', '.multipleaddsubjects', function() {
                ...
                $('#subjectmasterlist .modal-header h5')[0].innerText = 'Add subjects to ' + $(this).attr(
                    'data-string')
                ...
                $.ajax({
                    url: '/collegesubjects?table=table&course=' + course + '&currid=' + $(
                        '#curriculum').val(),
                    type: 'GET',
                    success: function(data) {
            ...
            $(document).on('click', '#add_subject_to_existing_curriculum_savebutton', function() {
                ...
                if ($('#select_curriculum_to_add').val() == '') {
                    ...
                    $('.editSubject').each(function() {
                        ...
                        $.ajax({
                            url: '/prospectus?transfer=transfer&prosid=' + $(this).attr(
                                    'data-id') + '&currid=' + $('#select_curriculum_to_add')
                                .val(),
                            type: 'GET',
                            success: function(data) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if ($('.editSubject').length == 0) {
                            ...
                            $('#nocurrsubj .card-footer').append(
                                '<button class="btn btn-primary" id="add_subject_to_existing_curriculum">Add subjects to existing curriculum</button>'
                            )
            ...
            $(document).on('click', '#savecur', function() {
                ...
                if (selectedCur == 0) {
                    ...
                    $.ajax({
                        url: '/curriculum?courseid=' + '{{ $courseInfo->id }}' +
                            '&insert=insert&curDesc=' + $('#curDesc').val(),
                        type: 'GET',
                        success: function(data) {
                    ...
                    $.ajax({
                        url: '/curriculum?courseid=' + '{{ $courseInfo->id }}' +
                            '&update=update&curDesc=' + $('#curDesc').val() + '&curid=' +
                            selectedCur,
                        type: 'GET',
                        success: function(data) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data, function(a, b) {
                            ...
                            $('#curriculum').append('<option value="' + b.id + '">' + b
            ...
            $(document).on('click', '.editcurriculum', function() {
                ...
                $.ajax({
                    url: '/curriculum?courseid=' + '{{ $courseInfo->id }}' + '&info=info&curid=' +
                        selectedCur,
                    type: 'GET',
                    success: function(data) {
            ...
            $(document).on('click', '.removecurriculum', function() {
                ...
                $.ajax({
                    url: '/curriculum?courseid=' + '{{ $courseInfo->id }}' +
                        '&remove=remove&curid=' + $(this).attr('data-id'),
                    type: 'GET',
                    success: function(data) {
            ...
            $(document).on('click', '.setasactive', function() {
                ...
                $.ajax({
                    url: '/curriculum?courseid=' + '{{ $courseInfo->id }}' +
                        '&setasactive=setasactive&activestatus=' + $(this).attr('data-value') +
            ...
            $(document).on('change', '#curriculum', function() {
                ...
                if ($(this).val() != '') {
                    ...
                    $.ajax({
                        url: '/prospectus?info=info&currid=' + $('#curriculum').val() + '&course=' +
                            course,
                        type: 'GET',
                        success: function(data) {

                            $.each(data, function(a, b) {
                                $("select[name='prereq[]']").append('<option value="' +
                                    b.id + '">' + b.subjDesc + '</option>')
                            })
            ...
            $(document).on('click', '.addprereq', function(a) {
                ...
                $('#prerequisitesubjectlist h5')[0].innerText = 'Add prerequisite to ' + $(this).attr(
                    'data-string')
                ...
                $.ajax({
                    url: '/collegesubjects?table=table&course=' + course + '&currid=' + $(
                        '#curriculum').val() + '&status=3&cursubjid=' + selectsubj,
                    type: 'GET',
                    success: function(data) {
            ...
            $(document).on('click', '.addtoprereq', function(a) {
                ...
                $.ajax({
                    url: '/collegesubjects?addtopreq=addtopreq&subjid=' + $(this).attr(
                        'data-subjid') + '&cursubjid=' + selectsubj,
                    type: 'GET',
                    success: function(data) {
            ...
            $(document).on('click', '.removetoprereq', function(a) {
                ...
                $.ajax({
                    url: '/collegesubjects?removetoprereq=removetoprereq&subjid=' + $(this).attr(
                            'data-subjid') + '&cursubjid=' + selectsubj + '&prereqid=' + $(this)
                        .attr('data-prereqid'),
                    type: 'GET',
                    success: function(data) {
            ...
            $(document).on('click', '.editSubject', function(a) {
                ...
                $.ajax({
                    url: '/course/' + course + '/prospectus/subject/' + $(this).attr('data-class') +
