            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-5">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        @else
                            <li class="breadcrumb-item active">Student List & Enrollment Summary</li>
                        @endif
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
            ...
            <div class="card">
                <div class="card-header d-flex p-0">
                    ...
                    <ul class="nav nav-pills ml-auto p-2">
                        <li class="nav-item"><a class="nav-link active text-sm" href="#tab_1" data-toggle="tab">Enrollment
                                Summary</a></li>
                        <li class="nav-item"><a class="nav-link text-sm" href="#tab_2" data-toggle="tab">Student List
                                (College)</a></li>
                        <li class="nav-item"><a class="nav-link text-sm" href="#tab_3" data-toggle="tab">NSTP Form
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="tab_1">
                            @if (strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'sic' ||
                                    strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'pcc')
                                <div class="row mb-2">
                                    <div class="col-md-2">
                                        ...
                                        <select class="form-control" id="es-syid">
                                            @foreach (DB::table('sy')->get() as $sy)
                                                <option value="{{ $sy->id }}"
                                                    @if ($sy->isactive == 1) selected @endif>{{ $sy->sydesc }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                        ...
                                        <select class="form-control" id="es-semid">
                                            @foreach (DB::table('semester')->get() as $semester)
                                                <option value="{{ $semester->id }}"
                                                    @if ($semester->isactive == 1) selected @endif>
                                    ...
                                    <div class="col-md-2">
                                        ...
                                        <select class="form-control" id="es-collegeid">
                                            ...
                                            @foreach (DB::table('college_colleges')->get() as $eachcollege)
                                                <option value="{{ $eachcollege->id }}">{{ $eachcollege->collegeDesc }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="col-md-9 text-right mt-2 align-self-end">
                                        <button type="button" class="btn btn-primary" id="btn-generate-es"><i
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select class="form-control" id="sl-semid">
                                        @foreach (DB::table('semester')->get() as $semester)
                                            <option value="{{ $semester->id }}"
                                                @if ($semester->isactive == 1) selected @endif>{{ $semester->semester }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6 text-right">
                                    <label>&nbsp;</label><br />
                                    <button type="button" class="btn btn-primary" id="btn-generate-sl"><i
                                            class="fa fa-sync"></i> Generate</button>
                                </div>
                            </div>
                        ...
                        <div class="tab-pane" id="tab_3">
                            <div class="row mb-2">
                                <div class="col-md-3">
                                    <label>Select School Year</label>
                                    <select class="form-control" id="nstpel-syid">
                                        @foreach (DB::table('sy')->get() as $sy)
                                            <option value="{{ $sy->id }}"
                                                @if ($sy->isactive == 1) selected @endif>{{ $sy->sydesc }}
                                            </option>
                                ...
                                @if (strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'dcc')
                                    <div class="col-md-3">
                                        <label>Select Semester</label>
                                        <select class="form-control" id="nstpel-semid">
                                            @foreach (DB::table('semester')->get() as $semester)
                                                <option value="{{ $semester->id }}"
                                                    @if ($semester->isactive == 1) selected @endif>
                                                    {{ $semester->semester }}</option>
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <select class="form-control" id="nstpel-reporttype">
                                    ...
                                    <div class="col-md-3 text-right">
                                        <label>&nbsp;</label><br />
                                        <button type="button" class="btn btn-primary" id="btn-generate-nstpel"><i
                                                class="fa fa-sync"></i> Generate</button>
                                    </div>
                                @else
    ...
    var selecteddate        = null;
        ...
        $(document).ready(function() {
                    ...
                    success: function(data) {
                        ...
                        if (data.length > 0) {
                            $.each(data, function(key, value) {
                                $('#es-courseid').append('<option value="' + value.id +
                                    '">' + value.courseDesc + '</option>')
                            })
                        }
            ...
            $(document).on('click', '#btn-export-es', function() {
                window.open('/registrar/studentlist?type=es&action=exportpdf&format=enrollmentsum&syid=' +
                    $('#es-syid').val() + '&semid=' + $('#es-semid').val() + '&department=' + $(
                        '#es-department').val()+'&selecteddate='+selecteddate, '_blank')
            })
            $(document).on('click', '#btn-export-excel', function() {
                window.open('/registrar/studentlist/excel?type=es&action=exportpdf&format=enrollmentsum&syid=' +
                    $('#es-syid').val() + '&semid=' + $('#es-semid').val() + '&department=' + $(
                        '#es-department').val()+'&selecteddate='+selecteddate, '_blank')
            })
            $(document).on('click', '#btn-export-estable', function() {
                window.open('/registrar/studentlist?type=es&action=exportpdf&format=enrollmenttable&syid=' +
                    $('#es-syid').val() + '&semid=' + $('#es-semid').val() + '&department=' + $(
                        '#es-department').val(), '_blank')
            })
            $(document).on('click', '.btn-export-es-ched', function() {
                ...
                window.open('/registrar/studentlist?type=es&action=exportexcel&reporttype=' + reporttype +
                    '&format=ched&syid=' + $('#es-syid').val() + '&semid=' + $('#es-semid').val() +
                    '&department=' + $('#es-department').val() + '&collegeid=' + collegeid +
                    '&courseid=' + courseid, '_blank')
            })
            $(document).on('click', '#btn-export-es-enrollment-stat', function() {
                window.open('/registrar/studentlist?type=es&action=exportpdf&format=enrollmentstats&syid=' +
                    $('#es-syid').val() + '&semid=' + $('#es-semid').val() + '&department=' + $(
                        '#es-department').val(), '_blank')
            })
            ...
            $(document).on('click', '#btn-export-sl', function() {
                window.open('/registrar/studentlist?type=sl&action=exportpdf&syid=' + $('#sl-syid').val() +
            ...
            $(document).on('click', '.btn-export-nstpel-sy', function() {
                ...
                window.open('/registrar/studentlist?type=nstpel&action=exportexcel&filetype=' + filetype +
                    '&reporttype=' + reporttype + '&syid=' + $('#nstpel-syid').val() + '&semid=' + $(
                        '#nstpel-semid').val() + '&nstpcomp=' + nstpcomp + '&exporttype=' + exporttype,
                    '_blank')
            })
