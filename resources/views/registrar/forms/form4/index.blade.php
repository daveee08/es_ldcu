...
@section('content')
                ...
                <div class="card-header">
                    <div class="row">
                        <div class="col-md-3">
                            ...
                            <select class="form-control" style="border: none; border-bottom: 1px solid #ddd"
                                id="selectedsy">
                        ...
                        <div class="col-md-3">
                            <label>Month</label>
                            <select id="selectedmonth" class="form-control form-control"
                                style="border: none; border-bottom: 1px solid #ddd">
                            </select>
                        </div>
                        <div class="col-md-3 text-right">
                            <label>&nbsp;</label><br />
                            <button type="button" class="btn btn-primary" id="btn-view-calendar"><i
                                    class="fa fa-calendar"></i> &nbsp;Pick dates</button>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer justify-content-between">
                    ...
                    <button type="button" id="btn-generate" class="btn btn-primary"><i class="fa fa-sync"></i>
                        Generate</button>
                </div>
            </div>
            <!-- /.modal-content -->
    ...
    <script>
        $(document).ready(function() {
            ...
            if ($('#selectedsy').val() != "" || $('#selectedsy').val() != null) {
                updateYearOptions();
            }

            $('#selectedsy').on('change', function() {
                updateYearOptions();
            });

            function populateMonths(startDate, endDate) {
                let start = new Date(startDate);
                let end = new Date(endDate);
                let $select = $("#selectedmonth");

                $select.empty();

                while (start <= end) {
                    let monthValue = ("0" + (start.getMonth() + 1)).slice(-2);
                    let monthName = start.toLocaleString("default", {
                        month: "long"
                    });

                    $select.append(new Option(monthName, monthValue));
            ...
            $.ajax({
                url: "/getActiveSchoolYear",
                type: "GET",
                dataType: "json",
                success: function(response) {
                    if (response) {
                        populateMonths(response.sdate, response.edate);
                    }
                },
                error: function() {
                    console.log("Error fetching school year data.");
                },
            });
        })
        ...
        function updateYearOptions() {
            var selectedSY = $('#selectedsy option:selected').text();
            var years = selectedSY.split('-');

            if (years.length === 2) {
                var startYear = $.trim(years[0]);
                var endYear = $.trim(years[1]);

                $('#selectedyear').empty();
                $('#selectedyear').append('<option value="' + startYear + '">' + startYear + '</option>');
                $('#selectedyear').append('<option value="' + endYear + '">' + endYear + '</option>');
            }
        }
    </script>
@endsection
