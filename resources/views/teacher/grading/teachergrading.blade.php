        ...
        <nav class="" aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a
                        href="/grades/getsubjects?selectedschoolyear={{ $schoolyearid }}&selectedsemester={{ $activeSem }}&selectedlevelid={{ $gradeLevelid }}&selectedsectionid={{ $sectionid }}">Subjects</a>
                </li>
            ...
            <div class="btn-actions-pane-lefy">
                <div role="group" class="btn-group-sm btn-group float-right">
                    ...
                    @if ($gradeLevelid == 14 || $gradeLevelid == 15)
                        @if ($activeSem == 1)
                            <button name="quarter" value="1" class="btn btn-success">1st Quarter <br> <span
                                    class="badge data_stat"
                                    value="1">{{ collect($grade_status)->where('quarter', 1)->first()->status }}</span></button>
                            <button name="quarter" value="2" class="btn btn-success">2nd Quarter <br> <span
                                    class="badge data_stat"
                                    value="2">{{ collect($grade_status)->where('quarter', 2)->first()->status }}</span></button>
                        @elseif($activeSem == 2)
                            <button name="quarter" value="3" class="btn btn-success">3rd Quarter <br> <span
                                    class="badge data_stat"
                                    value="3">{{ collect($grade_status)->where('quarter', 3)->first()->status }}</span></button>
                            <button name="quarter" value="4" class="btn btn-success">4th Quarter <br> <span
                                    class="badge data_stat"
                                    value="4">{{ collect($grade_status)->where('quarter', 4)->first()->status }}</span></button>
                        @endif
                    @else
                        @if (count($gradessetup) == 0)
                            <button name="quarter" value="1" class="btn btn-success" id="clickme">1st Quarter <br>
                                <span class="badge data_stat"
                                    value="1">{{ collect($grade_status)->where('quarter', 1)->first()->status }}</span></button>
                            <button name="quarter" value="2" class="btn btn-success">2nd Quarter <br> <span
                                    class="badge data_stat"
                                    value="2">{{ collect($grade_status)->where('quarter', 2)->first()->status }}</span></button>
                        ...
                        @else
                            @if ($gradessetup[0]->first == 1)
                                <button name="quarter" value="1" class="btn btn-success" id="clickme">1st Quarter
                                    <br> <span class="badge data_stat"
                                        value="1">{{ collect($grade_status)->where('quarter', 1)->first()->status }}</span></button>
                            @endif
    ...
    @if ($gradeLevelid == 14 || $gradeLevelid == 15)
        <div class="row mb-2">
            <div class="col-md-4">
                ...
                <p class="text-danger mb-0" style="font-size:.8rem !important"><i>The strands listed are from the subject
                        plot.</i></p>
            </div>
        </div>
    @endif
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.exportrecord', function() {
                ...
                if (quarter == 0) {
                    ...
                    window.open('/classrecord/pdf/' + sectionid + '/' + subjectid + '/' + quarter +
                        '?exporttype=' + exporttype + '&syid=' + syid + '&semid=' + semid, '_blank');
                        ...
                        if (result.value) {
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    $('#updateGrade').attr('disabled', 'disabled');
                                    $('#btnSubmit').attr('disabled', 'disabled');
                                    $('th[contenteditable="true"]').attr('contenteditable',
                                        'false')
                                    $('td[contenteditable="true"]').attr('contenteditable',
                                        'false')
                ...
                if (is_hps) {
                    try {
                        ...
                        $('.gradedetail').each(function() {
                            if ($(this)[0].cells[$('td[current="current"]')[0].cellIndex].innerText >
                                highest) {
                                highest = $(this)[0].cells[$('td[current="current"]')[0].cellIndex]
                                    .innerText;
                            }
                        })
