            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Grades / Pending Grades</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <div class="col-md-12">
                            <div class="card shadow">
                                <div class="card-body  p-2">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <button class="btn btn-info btn-block btn-sm mt-2" id="filter_button_1"><i
                                                    class="fas fa-filter"></i> View Pending Grades</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="col-md-9">
                    <div class="card shadow">
                        <div class="card-body p-2">
                                    ...
                                    <form action="/ecr/upload" id="upload_ecr" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="row">
                                            <div class="input-group input-group-sm col-md-12">
                                                <input disabled type="file" class="form-control" name="input_ecr"
                                                    id="input_ecr">
                            ...
                            <div class="row mt-4">
                                <div class="col-md-6">
                                    <button class="btn btn-success btn-sm" id="updateGrade" disabled> UPDATE
                                        GRADES</button>
                                    <p class="mb-0" id="failed_update"></p>
                                </div>
                                <div class="col-md-6 text-right ">
                                    <button class="btn btn-success  btn-sm" id="btnSubmit" disabled> SUBMIT
                                        GRADES</button>
                                </div>
                            </div>
                            {{-- <div class="row mt-4">
    ...
    <script>
        $(document).ready(function() {
            ...
            function getload() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        console.log('pending gradelist', data);
            ...
            $(document).on('click', '#filter_button_1', function() {
                ...
                // Filter schedule based on subject and section
                var selected = all_sched.filter(x => x.subjid == subjid && x.sectionid == sectionid);

                // Check if the 'selected' array is not empty
                if (selected.length > 0) {
                    // Check for levelid and proceed accordingly
                    if (selected[0].levelid == 14 || selected[0].levelid == 15) {
                        $.each(selected[0].strand, function(a, b) {
                            $('#strand_holder').append('<option value="' + b.strandid + '">' + b
                                .strandcode + '</option>');
                        });
                    ...
                    // Call function to load grades
                    load_grades();
                } else {
                    // Handle the case when no schedule matches
                    Toast.fire({
                        type: 'error',
                        title: 'No schedule found for the selected subject and section!'
                    });
                }
            });
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data == 'NYP') {
                            Toast.fire({
                                type: 'warning',
                                title: 'Quarter ' + (parseInt(curQuarter) - 1) +
                                    ' grades are not yet approved.'
                            })
                        } else if (data == 'NGS') {
                            Toast.fire({
                                type: 'warning',
                            ...
                            $('.isHPS[data-id="hps"]').each(function() {
                                ...
                                if (temp_id == undefined) {
                                    ...
                                    $(this).replaceWith('<th class="' + temp_class +
                                        ' text-center" data-field="' + data_field +
                                        '" data-id="hps" ' + data_hidden + '>' + $(this)
                                        .text() + '</th>');
                                }
                            });
                    ...
                    if ($('.edited[data-id="' + $(b).attr('data-value') + '"]').length > 0) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if (data[0].status == 1) {
                                    p_length += 1
                                    $('.edited[data-id="' + $(b).attr('data-value') + '"]')
                                        .removeClass('edited')
                                ...
                                if (data[0].status == 1) {
                                    ...
                                    if (not_checked == 0) {
                                        ...
                                        if (levelid == 14 || levelid == 15) {
                                            ...
                                            $.each(all_sched[temp_index].strand,
                                                function(a, b) {
                                            ...
                                            if (new_strand.length == 0) {
                                                var temp_index = all_sched.findIndex(
                                                    x => x.subjid == subjid && x
                                                    .sectionid == sectionid)
                                                all_sched[temp_index].pending_quarter =
                                                    all_sched[temp_index]
                                                    .pending_quarter.filter(x => x !=
                                                        quarter)
                                                if (all_sched[temp_index]
                                                    .pending_quarter == 0) {
                                                    all_sched[temp_index].with_pending =
                                                        false
                                                    all_sched = all_sched.filter(x => x
                                                        .subjid != subjid || x
                                                        .sectionid != sectionid)
                                                }
                                                load_subjects()
                                            ...
                                            if (all_sched[temp_index].pending_quarter ==
                                                0) {
                                                all_sched[temp_index].with_pending =
                                                    false
