...
<section class="content-header p-0" style="padding-top: 15px!important;">
    <div class="container-fluid">
        <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Deduction Printables</h1>
                </div>
                <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="/home">Home</a></li>
                    <li class="breadcrumb-item active">Deduction Printables</li>
                </ol>
                </div>
        </div>
    </div>
...
<section class="content">
    <div class="card border-0">
        <div class="card-body">
            <div class="row">
                <div class="col-md-12">
                    <table width="100%" class="table table-sm table-bordered table-head-fixed " id="deductions_datatables"  style="font-size: 16px">
                        <thead>
                              <tr>
                                    <th width="2%">&nbsp;&nbsp;No.</th>
...
<script>
    $(document).ready(function(){
        ...
        $(document).on('click', '#pdfdeduction', function(){
            var deductionid = $(this).attr('deductionid')
            var deductiondesc = $(this).attr('deductiondesc')
            
            var payrollid = $('#payrollid').val()

            window.open('/payrollclerk/setup/printdeduction?deductionid='+deductionid+'&payrollid='+payrollid+'&deductiondesc='+deductiondesc,'_blank')
        })
        // ========================================================== Functions =========================================================================
        function load_deductions(){
            $.ajax({
                type: "GET",
                url: "/payrollclerk/setup/deductionslist",
                success: function (data) {
                    deductions = data.filter(x=>x.description !== 'PHONE LOAN');
                    console.log(deductions);
                    load_deductionsdatatable()
                    payrollperiods()
                }
            });
        }
        ...
        function load_deductionsdatatable(){
            $('#deductions_datatables').DataTable({
                destroy: true,
                lengthChange: false,
                scrollX: false,
                autoWidth: false,
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        'createdCell':  function (td, cellData, rowData, row, col) {
                            ...
                            //                 '<a href="javascript:void(0)" id="" class="mb-0" style=""><i class="fas fa-file-pdf text-primary"></i></a>&nbsp;&nbsp;' +
                            //                 '<a href="javascript:void(0)" id="" class="mb-0" style=""><i class="fas fa-users-cog text-primary"></i></a>' +
                            //             '</div>';
                            var text = `<div class="text-center pdfs" style="display: flex; justify-content: center; align-items: center; font-size: 18px">
                                            <ul style="display: inline-flex; margin: 0; padding: 0;">
                                                <li class="icon" id="pdfdeduction" deductionid="${rowData.id}" deductiondesc="${rowData.description}">
            ...
            var label_text = $($('#deductions_datatables_wrapper')[0].children[0])[0].children[0]
