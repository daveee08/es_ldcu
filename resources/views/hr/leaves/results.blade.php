...
<table id="example" class="display table-bordered" style="width:100%; font-size: 12px;">
<thead>
    <tr class="text-center">
        ...
        <th>Purpose/Reason</th>
        <th>Status</th>
        <th class="none">#of leaves left</th>
...
<tbody>
    ...
    @if(collect($filedleave->approvals)->where('userid', auth()->user()->id)->count() > 0)
        <tr>
            <td class="p-0 text-center details-control">{{date('M d, Y', strtotime($filedleave->createddatetime))}}<br/>{{date(' h:i A', strtotime($filedleave->createddatetime))}}</td>
            <td>{{ucwords(strtolower($filedleave->lastname))}}, {{ucwords(strtolower($filedleave->firstname))}}</td>
            <td>{{$filedleave->leave_type}}</td>
            <td class="text-center">
                @if(count($filedleave->dates)>0)
                        ...
                        @if($approval->userid == auth()->user()->id)
                            @if($approval->appstatus == 0)
                                @php
                                    $pending += 1;
                                @endphp
                            <button type="button" class="btn btn-sm btn-warning btn-modalstatus" data-status="0" data-remarks="{{$approval->remarks}}" data-id="{{$filedleave->id}}">Pending</button>
                            @elseif($approval->appstatus == 1)
                                @php
                                    $approved += 1;
                                @endphp
                            <button type="button" class="btn btn-sm btn-success btn-modalstatus" data-status="1" data-remarks="{{$approval->remarks}}" data-id="{{$filedleave->id}}">Approved</button>
                            @elseif($approval->appstatus == 2)
                                @php
                                    $disapproved += 1;
                                @endphp
                            <button type="button" class="btn btn-sm btn-danger btn-modalstatus" data-status="2" data-remarks="{{$approval->remarks}}" data-id="{{$filedleave->id}}">Disapproved</button>
                            @endif
                        @endif
                    @endforeach
                @else
