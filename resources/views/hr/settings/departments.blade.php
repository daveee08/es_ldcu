...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-md-8">
            <button type="button" class="btn btn-sm btn-primary float-right" id="adddepartment"><i class="fa fa-plus"></i>
                Department</button>
        </div>
    </div>
    <div class="row d-flex align-items-stretch text-uppercase">
        ...
        @else
            @foreach ($departments as $department)
                <div class="card col-md-4 " style="border: none !important;box-shadow: none !important;"
                    data-string="{{ $department->department }}<">
                    <div class="card-body p-2" style="border: 1px solid#ddd;background: #e8e8e8;">
                        <div class="row">
                            ...
                            <div class="col-md-4 p-0 pr-2 text-right">
                                <button type="button" class="btn btn-sm btn-warning p-1 m-0 updatebutton"
                                    data-toggle="modal"
                                    data-target="#edit{{ $department->id }}"><small>Edit</small></button>
                                <button type="button" class="btn btn-sm btn-danger p-1 m-0 deletebutton"
                                    data-toggle="modal"
                                    data-target="#delete{{ $department->id }}"><small>Delete</small></button>
                            </div>
                        </div>
                        <small class="text-muted">
                            ...
                            @if (count($department->deptheads) > 0)
                                @foreach ($department->deptheads as $depthead)
                                    {{ $depthead->lastname }}, {{ $depthead->firstname }} {{ $depthead->middlename }}
                                    {{ $depthead->suffix }}<br />
                                @endforeach
                            @endif
                        </small>
                        <small class="text-muted">
                            Created by: {{ $department->lastname }}, {{ $department->firstname }}
                            {{ $department->middlename }} {{ $department->suffix }}
                        </small>
                        <div class="modal fade" id="edit{{ $department->id }}" style="display: none;" aria-hidden="true">
                            <div class="modal-dialog modal-md">
                                <form action="/hr/settings/departments/editdepartment" method="get" id=""
                                    name="changestatus">
                                    <div class="modal-content text-uppercase">
                                        ...
                                        <div class="modal-body">
                                            <label>Office</label>
                                            <input type="text" class="form-control text-uppercase" name="department"
                                            ...
                                            <select class="form-control select2" style="width: 100%;" name="employeeid">
                                                ...
                                                @if (count($employees) > 0)
                                                    @foreach ($employees as $employee)
                                                        <option
                                                            value="{{ $employee->id }}"@if (count($department->deptheads) > 0) @foreach ($department->deptheads as $depthead) {{ $employee->id == $depthead->id ? 'selected' : '' }} @endforeach @endif>
                                                            {{ $employee->lastname }}, {{ $employee->firstname }}
                                                            {{ $employee->middlename }} {{ $employee->suffix }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                        ...
                        <div class="modal fade" id="delete{{ $department->id }}" style="display: none;" aria-hidden="true">
                            <div class="modal-dialog modal-md">
                                <form action="/hr/settings/departments/deletedepartment" method="get" id=""
                                    name="changestatus">
                                    <div class="modal-content text-uppercase">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <input type="hidden" class="form-control" name="departmentid"
                                                value="{{ $department->id }}" />
                                            <input type="hidden" class="form-control text-uppercase" name="department"
                                                value="{{ $department->department }}" />
                                            <br>
                                            <br>
                            ...
                            @if (count($employees) > 0)
                                @foreach ($employees as $employee)
                                    <option value="{{ $employee->id }}">{{ $employee->lastname }},
                                        {{ $employee->firstname }} {{ $employee->middlename }} {{ $employee->suffix }}
                                    </option>
                                @endforeach
                            @endif
                        </select>
