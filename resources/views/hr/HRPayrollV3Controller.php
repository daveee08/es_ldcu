...
class HRPayrollV3Controller extends Controller
{
    ...
    public function getsalaryinfo(Request $request)
    {
        ...
        foreach ($otherdeductions as $otherdeduction) {
            ...
            $otherdeduction->totalotherdeductionpaid = $totalotherdeductionpaid;
        }
                    ...
                    if($eachotherdeduction->amount)
                    {
                        ...
                        elseif(count($monthlypayroll) == 1)
                        {
                            ...
                            if($payrollinfo)
                            {
                                ...
                                // return $eachotherdeduction->id;
                                // return  $monthlypayroll[0]->id;
                                $deductinfo = DB::table('hr_payrollv2historydetail')
                                    ->where('headerid', $monthlypayroll[0]->id)
                                    ->where('particulartype',2)
                                    ->where('deleted','0')
        ...
        $otherdeductions = array_filter($otherdeductions, function ($deduction) {
            return $deduction['totalotherdeductionpaid'] < $deduction['fullamount'];
        });
    ...
    public function exportpayslip(Request $request)
    {
        ...
        if($request->get('exporttype') == 1)
        {
            ...
            if(strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'hccsi')
            {
                ...
                $pdf = PDF::loadview('hr/payroll/v3/pdf_singlev3',compact('schoolinfo','header','particulars','payrollinfo','addedparticulars','payrolldetail','leavedetails','otherdeductions'));
            }
