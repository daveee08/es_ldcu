...
class HRAllowances extends Model
{
    ...
    {
        //THIS SETUP IS ONLY FOR 1-15 and 16-end of the month payroll period

        //get all the payroll history with the same MONTH and YEAR based on the payroll period 
        $monthlypayroll = DB::table('hr_payrollv2')
            ...
            ->get();
        //get first the details of the applied standard allowance of the employee to get the amount
        $allowanceinfo = DB::table('employee_allowancestandard')
            ->where('id', $employeeallowanceid)
            ->first();
        //get the employee's payroll history in the current payroll period
        ...
        $totalamount = 0;
        $amounttopay = 0;
        $paymenttype = 0; // 0 = full; 1 = half; 3 = null;
		
        if(count($monthlypayroll) == 0)
        {
            //if the monthlypayroll is 0 which means this is the employee's first pay of the MONTH and YEAR, then the amount is still the same as what he/she should be given
            if($allowanceinfo)
            {
        ...
        {            
            //if the monthlypayroll is 1 which means there is an existing payroll on the selected MONTH and YEAR based on the current payroll period, then the amount will be conditional
            if($historyinfo)
            {
                //if he/she has a CURRENT payroll information
                if($historyinfo->id == $monthlypayroll[0]->id)
                {
                    //if the CURRENT payroll information id is equal to the one that is existing with the selected MONTH and YEAR based on the current payroll period, get the details as shown below. This shows if he/she was paid during the current payroll period
                    ...
                    if($alloweinfo)
                    {  
                        //if he/she was paid, then fetch the amount, we cannot set the amount to default since maybe he/she was only paid half of the total amount
                        ...
                        {
                            //if he/she was paid full amount already, then we should lock the display to make it non-editable anymore
                            if($historyinfo->released == 1)
                            {
                                $lock = 1;
                            }
                        }
                        
                        if($alloweinfo->totalamount == $alloweinfo->amountpaid)
                        {
                            //another way to tell he/she was paid during the selected MONTH and YEAR. If the default amount is equals to the amount she was paid
                            if($historyinfo->released == 1)
                            {
                                $paidforthismonth = 1;
                            }
                        ...
                        //if he/she was not  yet paid, then fetch the default amount and set the payment type to 3 which is null, basically means the payroll clerk can still chose if he/she is gonna be paid in full or half amount
                        $paymenttype = 3;
                        $amounttopay = $allowanceinfo->amount;
                        $totalamount = $allowanceinfo->amount;
                    }
                    ...
                    if(date('Y-m', strtotime($monthlypayroll[0]->datefrom)) == date('Y-m', strtotime($payrollinfo->datefrom)))
                    {
                        ...
                        if($alloweinfo)
                        {
                            ...
                            {
                                if($monthlypayroll[0]->released == 1)
                                {
                                    $paidforthismonth = 1;
                                }
                            }else{
