...
<div width="100%" class="mt-1 col-md-12 table-responsive " style="overflow-x: scroll" >
    ...
    @endphp
    <table width="100%" class="table table-sm bordered" style="font-size: .6rem;max-height: 450px!important">
        <thead>
            <tr>
                <th rowspan="2" style="width: 500px!important" class="text-center align-middle">STUDENT</th>
                    @foreach($component as $item)
                        @php
            ...
            <tr>
                @foreach($component as $item)
                    @if(count($item['subgrading']) > 0)
                        @foreach($item['subgrading'] as $subitem)
                            <th colspan="{{$subitem['subcomponent_column']}}" class="text-center align-middle">{{$subitem['subcompname']}} <div class="subcomp_percentage">({{$subitem['subcomp_percentage']}}%)</div></th>
            ...
            <tr>
                <th rowspan="2" style="white-space: nowrap!important" class="align-middle">HIGHEST POSSIBLE SCORE</th>
                @foreach($component as $item)
                    @if(count($item['subgrading']) > 0)
                        @foreach($item['subgrading'] as $subitem)
                            @php
                            ...
                            @foreach(range(1, $columns) as $column)
                                @php
                                    $subsorts += 1
                                @endphp
                                <th  class="text-center align-middle date_time text-secondary keyinput" data-sort-id="{{$subsorts}}" data-comp-id="{{$item['componentid']}}" data-sub-id="{{$subitem['subcompid']}}" contenteditable  style="writing-mode: vertical-rl; padding: -20px; transform: rotate(180deg);" >MM/DD/YYYY</th>
                            @endforeach
                        @endforeach
                    @else
                        @php
                        ...
                        @foreach(range(1, $item['component_column']) as $column)
                            @php
                                $compsort += 1
                            @endphp
                            <th  class="text-center align-middle date_time text-secondary keyinput" data-sort-id="{{$compsort}}" data-comp-id="{{$item['componentid']}}" contenteditable  style="writing-mode: vertical-rl; padding: -20px; transform: rotate(180deg);" >MM/DD/YYYY</th>
                        @endforeach
                    @endif
                @endforeach
            </tr>
                        ...
                        @foreach($item['subgrading'] as $subitem)
                            ...
                            @foreach(range(1, $columns) as $column)
                                @php
                                    $subsorts += 1
                                @endphp
                                <th  class="text-center align-middle highest_score keyinput" data-sort-id="{{$subsorts}}" data-comp-id="{{$item['componentid']}}" data-sub-id="{{$subitem['subcompid']}}" contenteditable>0</th>
                            @endforeach
                        @endforeach
                    @else
                        @php
                        ...
                        @foreach(range(1, $item['component_column']) as $column)
                            @php
                                $compsort += 1
                            @endphp
                            <th class="text-center align-middle highest_score keyinput"  data-sort-id="{{$compsort}}" data-comp-id="{{$item['componentid']}}" contenteditable>0</th>
                        @endforeach 
                    @endif
                @endforeach
            </tr>
        ...
        <tbody>
            ...
            @foreach($students->where('gender', 'MALE') as $stud)
                <tr>
                    <td class="align-middle stud_row" data-stud-id = "{{$stud->studid}}">
                        <div class="d-flex flex-row justify-content-betweem">
                            <div class="mr-auto student" style="white-space: nowrap!important">{{$stud->studname}}</div>
                            <div class="ml-1"><input type="checkbox"  class="male_checkbox checkbox select ml-auto" data-stud-id="{{$stud->studid}}"></div>
                        </div>
                    </td>
                    @foreach($component as $item)
                        @if(count($item['subgrading']) > 0)
                            @foreach($item['subgrading'] as $subitem)
                                ...
                                @for($i = 1; $i <= $subitem['subcomponent_column']; $i++)
                                    @php
                                        $sort += 1
                                    @endphp
                                    <td class="text-center align-middle score scores keyinput"  data-sort-id="{{$sort}}" data-comp-id="{{$item['componentid']}}" data-stud-id="{{$stud->studid}}" data-sub-id="{{$subitem['subcompid']}}"  contenteditable >0</td>
                                @endfor
                        ...
                        @else
                            ...
                            @php
                                $sorted += 1
                            @endphp
                                <td class="text-center align-middle score scores keyinput" data-sort-id="{{$sorted}}" data-stud-id="{{$stud->studid}}" data-comp-id="{{$item['componentid']}}" contenteditable>0</td>
                            @endfor
            ...
            @foreach($students->where('gender', 'FEMALE') as $stud)
                <tr>
                    <td class="align-middle stud_row" data-stud-id = "{{$stud->studid}}">
                        <div class="d-flex flex-row justify-content-betweem">
                            <div class="mr-auto student" style="white-space: nowrap!important">{{$stud->studname}}</div>
                            <div class="ml-1"><input type="checkbox"  class="female_checkbox checkbox select ml-auto" data-stud-id="{{$stud->studid}}"></div>
                        </div>
                    </td>
                    @foreach($component as $item)
                        @if(count($item['subgrading']) > 0)
