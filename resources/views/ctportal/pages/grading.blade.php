                        ...
                        <div class="col-md-12 table-responsive tableFixHead" style="height: 420px;">
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="dropdown" style="float: right;">
                                    <button id="save_grades" class="btn btn-primary btn-sm" data-id="0"
                                        data-schedid="0" disabled hidden>Save
                                        Grades</button>

                                    <button class="btn btn-primary btn-sm submit_all_btn" id="grade_submit"
                                        data-value="5" data-term="5" disabled>
                                        Submit All
                                    </button>
                                </div>
        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-body" style="font-size:.9rem">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <button class="btn btn-primary btn-sm" id="print_grades" data-id = "">Print</button>
                        </div>
                    </div>
                </div>
            </div>
...
@section('footerscript')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.input_grades', function() {
                ...
                var prospectusid = $(this).attr('data-pid')
                ...
                check_sys_grades(studid, syid, semid, prospectusid, term)

            })

            function check_sys_grades(studid, syid, semid, prospectusid, term) {
                $.ajax({
                    url: '/ctportal/grading/check_sys_grades',
                    method: 'GET',
                    data: {
                        studid: studid,
                        syid: syid,
                        ...
                        if (data == 'true') {
                            Toast.fire({
                                type: 'warning',
                                title: 'System Grading for this Subject Exists.'
                            })
                            $('#save_grades').attr('disabled', true)
                                
                        }else{
                            $('#save_grades').attr('disabled', false)
                        }
                    }
                })
            }
            ...
            function checkKey(e) {
                ...
                // }
                    ...
                    // Fetch grades for each term if they exist in the setup
                    var grades = {};
                    var validTerms = 0; // Counter to track valid grades for active terms
                    if (gradesetupS.length > 0) {
                        gradesetupS.forEach(grade => {
                            if (grade.quarter == 1) {
                                grades.prelim = parseFloat($('.grade_td[data-studid="' + temp_studid +
                                    '"][data-term="1"]').text());
                                if (!isNaN(grades.prelim)) validTerms++;
                            }
                            if (grade.quarter == 2) {
                                grades.midterm = parseFloat($('.grade_td[data-studid="' + temp_studid +
                                    '"][data-term="2"]').text());
                                if (!isNaN(grades.midterm)) validTerms++;
                            }
                            if (grade.quarter == 3) {
                                grades.prefi = parseFloat($('.grade_td[data-studid="' + temp_studid +
                                    '"][data-term="3"]').text());
                                if (!isNaN(grades.prefi)) validTerms++;
                            }
                            if (grade.quarter == 4) {
                                grades.final = parseFloat($('.grade_td[data-studid="' + temp_studid +
                                    '"][data-term="4"]').text());
                                if (!isNaN(grades.final)) validTerms++;
                            }
                        });
                    }

                    // Calculate final grade only if all active terms have valid grades
                    if (gradesetupS.length > 0 && validTerms === gradesetupS.length) {
                        var total = 0;

                        // Sum up the grades for active terms
                        if (grades.prelim !== undefined && !isNaN(grades.prelim)) total += grades.prelim;
                        if (grades.midterm !== undefined && !isNaN(grades.midterm)) total += grades.midterm;
                        if (grades.prefi !== undefined && !isNaN(grades.prefi)) total += grades.prefi;
                        if (grades.final !== undefined && !isNaN(grades.final)) total += grades.final;

                        // Calculate the average
                        var fg = (total / gradesetupS.length).toFixed(2); // Adjust decimal points as needed
                        $('th[data-studid="' + temp_studid + '"][data-term="5"]').text(fg).addClass('updated');
                        $('th[data-studid="' + temp_studid + '"][data-term="6"]').addClass('updated');
                    } else {
                        // Clear the final grade fields if not all active terms have valid grades
                        $('th[data-studid="' + temp_studid + '"][data-term="5"]').text('').removeClass('updated');
                        $('th[data-studid="' + temp_studid + '"][data-term="6"]').text('').removeClass('updated');
                    }
    ...
    {{-- <script>
        $(document).ready(function() {
            ...
            let can_edit = true;
            let string = '';
            let gradesetupS = []; // Array to store grade setup data

            // Fetch grade setup data on page load
            getgradesetupS();

            $(document).on('click', '.input_grades', function() {
                const term = $(this).attr('data-term');
                const checkDateSetup = getTermDateSetup(term);

                if (checkDateSetup.length > 0 && !checkDateSetup[0].isended) {
                    Toast.fire({
                        type: 'warning',
                        title: 'Deadline Ended.'
                    });
                ...
                if (school.toUpperCase() === 'SPCT' && term === 'finalgrade') {
                    return false;
                }

                if (currentIndex !== undefined && !isvalidHPS && !can_edit) {
                    return;
                }
                ...
                $('#start').removeAttr('id');
                $(this).attr('id', 'start');
                $(currentIndex).removeClass('bg-danger bg-warning');
            ...
            $(document).on('click', '#download_ecr', function() {
                const url =
                    `/college/grade/ecr/download?syid=${$('#filter_sy').val()}&semid=${$('#filter_semester').val()}&schedid=${temp_id}&subjid=${$('#subjectDesc').data('id')}&levelid=${$('#collegeLevel').data('id')}&sectionid=${$('#section').data('id')}`;
                window.open(url, '_blank');
            });

            $('#input_ecr').on('change', function() {
                const fileInput = $(this)[0];
                const uploadButton = $('#upload_ecr_button');
            ...
            function dotheneedful(sibling) {
                const term = $(sibling).attr('data-term');
                const checkDateSetup = getTermDateSetup(term);
                ...
                if (sibling !== null) {
                    currentIndex = sibling;
                    $(sibling).removeClass('bg-danger bg-warning');
            ...
            function checkKey(e) {
                ...
                const keyActions = {
                    38: () => navigateCell(-1, 0), // Up arrow
                    40: () => navigateCell(1, 0), // Down arrow
                    37: () => navigateCell(0, -1), // Left arrow
                    39: () => navigateCell(0, 1), // Right arrow
                    73: () => setGrade('INC'), // 'I' key
                    68: () => setGrade('DROPPED'), // 'D' key
                    8: handleBackspace, // Backspace key
                };
            ...
            function navigateCell(rowOffset, colOffset) {
                const currentCell = $(currentIndex);
                const newRow = currentCell.parent()[rowOffset === -1 ? 'prevAll' : 'nextAll']('tr').first();
                const newCell = newRow.find('td').eq(currentCell.index() + colOffset);

                if (newCell.length && newCell.hasClass('input_grades')) {
                    string = newCell.text();
                    dotheneedful(newCell[0]);
                }
            }

            function setGrade(grade) {
                $(currentIndex).text(grade).addClass('updated');
                $('#save_grades').removeAttr('disabled');
                $('#grade_submit').attr('disabled', 'disabled');
                updateFinalGrade();
            }
            ...
            function handleBackspace() {
                if (currentIndex.innerText === 'INC' || currentIndex.innerText === 'DROPPED') {
                    ...
                    string = currentIndex.innerText.slice(0, -1);
                }

                $(currentIndex).text(string).addClass('updated');
                $('#save_grades').removeAttr('disabled');
                $('#grade_submit').attr('disabled', 'disabled');
                updateFinalGrade();
            }
            ...
            function handleNumericInput(key) {
                if (key === '.' && (gradesetup.decimalPoint === 0 || string.includes('.'))) {
                    return;
                }

                const checkString = string + key;
                const decimalCount = countDecimal(checkString);

                if (decimalCount <= gradesetup.decimalPoint) {
                    string += key;
                }

                if (gradesetup.isPointScaled === 0 && checkString > 100) {
                    string = '100';
                } else if (gradesetup.isPointScaled === 1 && checkString > 5) {
                    return;
                }
            ...
            function updateFinalGrade() {
                console.log('updateFinalGrade called'); // Debugging line
                const tempStudId = $(currentIndex).attr('data-studid');
                const prelim = parseFloat($(`.grade_td[data-studid="${tempStudId}"][data-term="1"]`).text());
                const midterm = parseFloat($(`.grade_td[data-studid="${tempStudId}"][data-term="2"]`).text());
                const prefi = parseFloat($(`.grade_td[data-studid="${tempStudId}"][data-term="3"]`).text());
                const final = parseFloat($(`.grade_td[data-studid="${tempStudId}"][data-term="4"]`).text());

                console.log('Prelim:', prelim, 'Midterm:', midterm, 'Prefi:', prefi, 'Final:',
                    final); // Debugging line

                const fg = gradesetupS.length > 0 ? gradesetupS.reduce((total, item) => total + item.finalGrade,
                    0) / gradesetupS.length : 0;
                console.log('FG:', fg); // Debugging line

                if (!isNaN(fg)) {
                    $(`th[data-studid="${tempStudId}"][data-term="5"]`).text(fg.toFixed(2)).addClass('updated');
                    $(`th[data-studid="${tempStudId}"][data-term="6"]`).addClass('updated');
                } else {
                    console.log('FG is NaN'); // Debugging line
                }
            }

            function countDecimal(num) {
                const converted = num.toString();
                return converted.includes('.') ? converted.split('.')[1].length : 0;
            }

            function getTermDateSetup(term) {
                const termMap = {
                    1: 'prelim',
                    2: 'midterm',
                    3: 'prefi',
                ...
                return inputperiod.filter(x => x.term === termMap[term]);
            }

            function getgradesetupS() {
                $.ajax({
                    type: 'GET',
                    url: '/semester-setup/getactive-setup/v4',
                    async: false,
                    data: {
                        syid: $('#filter_sy').val(),
                    ...
                    success: function(data) {
                        gradesetupS = data;
                        console.log('getgradesetup daw hays', gradesetupS);

                        if (gradesetupS.length === 0) {
                            console.log('No available grade setup.');
                            ...
                            gradesetupS.forEach(grade => {
                                if (grade.quarter === 1) termtext +=
                                    '<span class="badge badge-primary ml-1">Prelim</span>';
                                if (grade.quarter === 2) termtext +=
                                    '<span class="badge badge-primary ml-1">Midterm</span>';
                                if (grade.quarter === 3) termtext +=
                                    '<span class="badge badge-primary ml-1">Prefi</span>';
                                if (grade.quarter === 4) termtext +=
                                    '<span class="badge badge-primary ml-1">Final</span>';
                            });
    ...
    <script>
        ...
        function getgradesetupS(subjectid,sectionid) {
            $.ajax({
                type: 'GET',
                url: '/semester-setup/getactive-setup/v4',
                async: false,
                ...
                success: function(data) {
                    ...
                    if (gradesetupS.length == 0) {
                        ...
                        $('#grades_setup_holder').removeAttr('hidden', 'hidden');
                        var termtext = '';
                        $.each(gradesetupS, function(index, grade) {
                            if (grade.quarter == 1) {
                                console.log('Adding Prelim badge');
                                termtext += '<span class="badge badge-primary ml-1">Prelim</span>';
                            }
                            if (grade.quarter == 2) {
                                console.log('Adding Midterm badge');
                                termtext += '<span class="badge badge-primary ml-1">Midterm</span>';
                            }
                            if (grade.quarter == 3) {
                                console.log('Adding Prefi badge');
                                termtext += '<span class="badge badge-primary ml-1">Prefi</span>';
                            }
                            if (grade.quarter == 4) {
                                console.log('Adding Final badge');
                                termtext += '<span class="badge badge-primary ml-1">Final</span>';
                            }
                        });
                        $('#setup_term_holder')[0].innerHTML = termtext;

                        // if (gradesetup[0].isPointScaled == 1) {
                        //     console.log('Using Decimal Point Scale');
                        //     $('#setup_gs_holder').text('Decimal Point Scale ( 1 - 5 )');
                        // } else {
                        //     console.log('Using Numerical Point Scale');
                        //     $('#setup_gs_holder').text('Numerical Point Scale ( 60 - 100 )');
                        // }
                    }
                }
        ...
        $(document).ready(function() {
            ...
            function get_term(term) {
                if (term == 1) {
                    return "prelim_transmuted"
                } else if (term == 2) {
                    return "midterm_transmuted"
                } else if (term == 3) {
                    return "prefinal_transmuted"
                } else if (term == 4) {
                    return "final_transmuted"
                } else if (term == 5) {
                    return "submitall"
                }
            ...
            function submit_grade(clickedElement) {
                ...
                if(saved == 1){
                                    ...
                                    if (data.status === 0) {
                                        ...
                                        $.each(selected, function(a, b) {
                                            ...
                                            var temp_id = all_grades.findIndex(x => x.id ==
                                                b);
                                            if (dterm == 1) {
                                                all_grades[temp_id].prelim_status = 1;
                                            } else if (dterm == 2) {
                                                all_grades[temp_id].midterm_status = 1;
                                            } else if (dterm == 3) {
                                                all_grades[temp_id].prefinal_status = 1;
                                            } else if (dterm == 4) {
                                                all_grades[temp_id].final_status = 1;
                                            } else if (dterm == 5) {
                                                all_grades[temp_id].prelim_status = 1;
                                                all_grades[temp_id].midterm_status = 1;
                                                all_grades[temp_id].prefinal_status = 1;
                    ...
                    Toast.fire({
                        type: 'warning',
                        title: 'Please save your grades first!'
                    })
                }
            ...
            $(document).on('click', '#save_grades', function() {
                ...
                console.log('Saving Grades for Schedid: ' + schedid)
            ...
            function save_fg(schedid) {
                if ($('.updated[data-term="5"]').length == 0) {
                    save_fgremarks(schedid)
                }
            ...
            function save_fgremarks(schedid) {
            ...
            function get_subjects() {
                $.ajax({
                    type: 'GET',
                    url: '/college/teacher/schedule/get/v4',
                    data: {
                        syid: $('#filter_sy').val(),
                        semid: $('#filter_semester').val(),
                        teacherid: 73
                    ...
                    success: function(data) {
                        if (data.length == 0) {
                            ...
                            all_subject = data
                            console.log('all_subject daw 2025', all_subject)
                            // all_students = data
                ...
                $.each(data, function(a, persched) {
                    let request = $.ajax({
                        ...
                        url: `/college/teacher/student-list-for-all/${$('#filter_sy').val()}/${$('#filter_semester').val()}/` +
                            persched.subjectid + '/' + persched.sectionid,
            ...
            $(document).on('click', '.view_students', function() {
                $('#modal_1').modal()
                temp_id = $(this).attr('data-id')
                temp_section = $(this).attr('data-section-id')
                
                var students = all_subject.filter(x => x.subjectid == temp_id && x.sectionid == temp_section)
            ...
            $(document).on('click', '.view_grades', function() {
                ...
                getgradesetupS(subjectid,sectionid);
                ...
                $('#modal_2').modal()
                temp_id = $(this).attr('data-id')
                temp_section = $(this).attr('data-sectionid')
                grade_sched_id = $(this).attr('data-schedid')
                $('#print_grades').attr('data-id', grade_sched_id);
                console.log('Viewing Grades for ', temp_id)

                schedid = grade_sched_id

                $('#save_grades').attr('data-schedid', grade_sched_id)

                $('#save_grades').attr('data-id', temp_id)
                ...
                if (all_students.length != 0) {
                    var selected_students = all_students.filter(x => x.subjectid == temp_id   && x.sectionid == temp_section)
                }
                ...
                // var gradesetup = students[0].gradesetup

                
                
                if (gradesetupS.length > 0) {
                    gradesetupS.forEach(grade => {
                        if (grade.quarter == 1) disprelim = 1;
                        if (grade.quarter == 2) dismidterm = 1;
                        if (grade.quarter == 3) disprefi = 1;
                        if (grade.quarter == 4) disfinal = 1;
                    });
                }
                ...
                $.each(selected_students[0].students, function(a, b) {
                    ...
                    var text = '<tr data-gender="' + b.gender + '"><td class="text-center count">' +
                        ...
                        b.studid + '" data-id="' + b.id + '" style="float:right;"></td>';

                    if (disprelim == 1) {
                        text += '<td data-studid="' + b.studid + '" data-course="' + b.courseid +
                    ...
                    text += '<th data-studid="' + b.studid + '" data-course="' + b.courseid +
                        '" data-pid="' + pid + '" data-section="' + sectionid +
                    ...
                    $('#datatable_4').append(
                        ...
                        b.studid + '" data-id="' + b.id + '"></td><td>' + b.studid +
                        '</td><td>' +
                ...
                get_grades(schedid, true, students, sectionid, subjectid);
            ...
            $(document).on('click', '#print_grades', function() {
                var temp_sched_id = $(this).attr('data-id');
                // console.log('Printing grades for temp_id:', temp_id);
                print_grades(temp_sched_id)
                // print_grades()
            })

            function print_grades(temp_sched_id) {

                var pid = []
                var sectionid = []
                // var students = all_subject.filter(x => x.schedid == schedid)[0].students
                var students = all_subject.filter(x => x.schedid == temp_sched_id);
                // console.log(students);
                // alert('Printing grades for ' + students.length + ' students in subjectid ' + temp_id);
                // var temp_pid = [...new Map(students.map(item => [item['pid'], item])).values()]
                var temp_pid = [...new Map(students.map(item => [item['pid'], item])).values()]
                console.log('Printing grades for', students.length, 'students in subjectid', temp_id);
                ...
                window.open('/college/teacher/student/grades/print?&syid=' + syid + '&semid=' + semid + '&pid=' +
                    pid + '&sectionid=' + sectionid + '&schedid=' + temp_sched_id + '&subjid=' + temp_subjid +
                    '&dean=' + dean, '_blank');
            ...
            function get_grades(schedid, prompt = true, students, sectionid, subjectid) {
                saved == 0
                var pid = [];
                // var sectionid = [];
                var temp_pid = [...new Map(students.map(item => [item['pid'], item])).values()];
                ...
                $.ajax({
                    type: 'GET',
                    url: '/college/teacher/student/new/gradesv3/get',
                    data: {
                        syid: $('#filter_sy').val(),
                        semid: $('#filter_semester').val(),
                        pid: pid,
            ...
            //         ['prelim_transmuted', 'midterm_transmuted', 'prefinal_transmuted', 'final_transmuted']
            //         .forEach((term, idx) => {
            //             const termIndex = idx + 1;
            //             const grade = record[term];
            //             const status = record[term.replace('grade', 'status')];
            ...
            //             $('th[data-studid="' + record.studid + '"][data-term="5"]').text('---');
            //             $('th[data-studid="' + record.studid + '"][data-term="6"]').text('---');
            //             if (record.final_status === 8 || record.prelim_status === 8 || record
            //                 .midterm_status === 8 || record.prefinal_status === 8) {
            //                 finalStatusElement.text('INC').addClass('bg-warning');
            //                 finalStatusElement.removeClass('bg-success bg-danger');
            //                 $('th[data-studid="' + record.studid + '"][data-term="5"]').addClass(
            //                     'bg-warning').removeClass('bg-danger bg-success');
            ...
            //             } else if (record.final_status === 9 || record.prelim_status === 9 || record
            //                 .midterm_status === 9 || record.prefinal_status === 9) {
            //                 finalStatusElement.text('DROPPED').addClass('bg-danger');
            //                 finalStatusElement.removeClass('bg-success bg-warning');
            //                 $('th[data-studid="' + record.studid + '"][data-term="5"]').addClass(
            //                     'bg-danger').removeClass('bg-warning bg-success');
            ...
            //         const statusKeys = ['prelim_status', 'midterm_status', 'prefinal_status',
            //             'final_status'
            //         ];
            //         const gradeKeys = ['prelim_transmuted', 'midterm_transmuted', 'prefinal_transmuted',
            //             'final_transmuted'
            //         ];

            //         statusKeys.forEach((statusKey, idx) => {
            //             const termIndex = idx + 1;
            //             const grade = record[gradeKeys[idx]];
            ...
            //                 const finalTermElement = $('th[data-studid="' + record.studid +
            //                     '"][data-term="5"]');
            //                 finalTermElement.text('---').css('text-align', 'center');
            //                 if (!finalTermElement.hasClass('bg-primary') && !finalTermElement
            //                     .hasClass('bg-info')) {
            //                     finalTermElement.removeClass().addClass(statuses[status]);
            //                 }
            ...
            //         const finalStatusText = record.final_remarks || '';
            //         const finalStatusElement = $('th[data-studid="' + record.studid + '"][data-term="6"]');
            ...
            //             failedCount++;
            //         } else {
            //             if (record.final_grade_transmuted) {
            //                 $('th[data-studid="' + record.studid + '"][data-term="5"]').text(record
            ...
            //         if (hasFailedStatus || (gradeKeys.some(key => record[key] === 'INC'))) {
            //             $('th[data-studid="' + record.studid + '"][data-term="5"]').text('---');
            //             $('th[data-studid="' + record.studid + '"][data-term="6"]').text('---');
            //             if (record.final_status === 8 || record.prelim_status === 8 || record
            //                 .midterm_status === 8 || record.prefinal_status === 8 || gradeKeys.some(key =>
            //                     record[key] === 'INC')) {
            //                 finalStatusElement.text('INC').addClass('bg-warning');
            //                 finalStatusElement.removeClass('bg-success bg-danger');
            //                 $('th[data-studid="' + record.studid + '"][data-term="5"]').addClass(
            ...
            function plot_subject_grades(data) {
                ...
                $.each(data, function(index, record) {
                    ...
                    if (hasFailedStatus || (gradeKeys.some(key => record[key] === 'INC' || record[key] ===
                            'DROPPED'))) {
                        $('th[data-studid="' + record.studid + '"][data-term="5"]').text('---');
                        $('th[data-studid="' + record.studid + '"][data-term="6"]').text('---');
                        // if (gradeKeys.some(key => record[key] === 'INC')) {
                        //     finalStatusElement.text('INC').addClass('bg-warning');
                        //     finalStatusElement.removeClass('bg-success bg-danger');
                        //     $('th[data-studid="' + record.studid + '"][data-term="5"]').addClass(
                        //         'bg-warning').removeClass('bg-danger bg-success');
                        ...
                        //     $('td[data-studid="' + record.studid + '"][data-term="1"], td[data-studid="' +
                        //         record.studid + '"][data-term="2"], td[data-studid="' + record.studid +
                        //         '"][data-term="3"], td[data-studid="' + record.studid +
                        //         '"][data-term="4"]').each(function() {
                        //         $(this).addClass('bg-warning').removeClass('bg-danger bg-success');;
                        //     });
                        // } else if (gradeKeys.some(key => record[key] === 'DROPPED')) {
                        //     finalStatusElement.text('DROPPED').addClass('bg-danger');
                        //     finalStatusElement.removeClass('bg-warning bg-success');
                        //     $('th[data-studid="' + record.studid + '"][data-term="5"]').addClass(
                        //         'bg-danger').removeClass('bg-warning bg-success');
                        //     $('th[data-studid="' + record.studid + '"][data-term="5"]').text('---');
                        //     $('.student-checkbox[data-studid="' + record.studid + '"]').prop('disabled',
                        ...
                        //         '"][data-term="3"], td[data-studid="' + record.studid +
                        //         '"][data-term="4"]').each(function() {
                        //         $(this).addClass('bg-danger').removeClass('bg-warning bg-success');;
                        //     });
                        // }
                        if (gradeKeys.some(key => record[key] === 'DROPPED')) {
                            finalStatusElement.text('DROPPED').addClass('bg-danger');
                            finalStatusElement.removeClass('bg-warning bg-success');
                            $('th[data-studid="' + record.studid + '"][data-term="5"]').addClass(
                                'bg-danger').removeClass('bg-warning bg-success');
                        ...
                        } else if (gradeKeys.some(key => record[key] === 'INC')) {
                            finalStatusElement.text('INC').addClass('bg-warning');
                            finalStatusElement.removeClass('bg-success bg-danger');
                            $('th[data-studid="' + record.studid + '"][data-term="5"]').addClass(
                                'bg-warning').removeClass('bg-danger bg-success');
            ...
            // MEDYO FINAL WORKING CODE FOR PLOTTING
            // function plot_subject_grades(data) {
            //     let passedCount = 0;
            //     let failedCount = 0;
            //     let notSubmittedCount = 0;
                    ...
                    columnDefs: [{
                        ...
                        },
                        {
                            ...
                            'createdCell': function(td, cellData, rowData) {
                                ...
                                var studentCount = rowData.students.studentCount;
                                var text =
                                    '<a href="#" class="text-primary view_students" data-section-id="'+rowData.sectionid+'" data-id="' +
                                    rowData.subjectid +
                                    '" style="font-size: 20px; display: block; text-align: center; font-weight:bold">' +
                                    studentCount + '</a>';
                                $(td).html(text).addClass('align-middle');
                        ...
                        },
                        {
                            ...
                            'createdCell': function(td, cellData, rowData) {
                                var link =
                                    '<a href="#" style="color: #blue; text-decoration: underline;font-weight: bold;" class="view_grades" data-sectionid = "' + rowData.sectionid + '" data-id="' +
                                    rowData.subjectid + '" data-schedid="' +
                                    rowData.schedid +
                                    '"> Grading</a>';
                                $(td)[0].innerHTML = link;
                                $(td).addClass('text-center align-middle');
            ...
            $(document).on('click',
                '.grade_submissions_prelim, #grade_submit, .grade_submissions_midterm, .grade_submissions_semifinal, .grade_submissions_final',
                function() {
                    // submit_grade();
                    submit_grade(this); // Pass the clicked element to the function
                }
