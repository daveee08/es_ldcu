        ...
        $(document).ready(function(){
                ...
                function rendertablecategory(response) {
                                ...
                                if(response.length > 0) {
                                        ...
                                        for (var i = 0; i < response.length; i++) {
                                                const category = response[i].coursesubject;

                                                html += `<td>${response[i].score}/${response[i].sum} (${(response[i].score/response[i].sum * 100).toFixed(2)}%) <br>`;
                                                
                                                for (var j = 0; j < category.length; j++) {
                                                        if ((response[i].score / response[i].sum * 100).toFixed(2) >= category[j].passing) {
                                                        var status = "Passed";
                                                        } else {
                                                        var status = "Failed";
                                                        }
                                                        html += `<span style="font-size: 11px;"> <b>${category[j].courseDesc} </b> : ${category[j].passing}% (${status}) <br> </span>`;
                                                }
                                                html += '</td>';
                                        }
