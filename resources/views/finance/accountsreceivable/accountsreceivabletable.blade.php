<div class="card shadow" style="border: none;">
    <div class="card-header">
        ...
        <button type="button" class="btn btn-default btn-sm export" exporttype="pdf"><i class="fa fa-download"></i>
            PDF</button>
    </div>
                ...
                @else
                    @foreach ($students as $student)
                        <tr class="studid" id="{{ $student->id }}">
                            ...
                            <td style="text-align: left !important;" class="sname{{ $student->id }}">
                            ...
                            <td class="sta{{ $student->id }}">&#8369;
                                {{ number_format($student->totalassessment, 2, '.', ',') }}</td>
                            {{-- <td class="sd{{ $student->id }}">&#8369;
                                {{ number_format($student->discount, 2, '.', ',') }}</td> --}}
                            {{-- <td class="sna{{ $student->id }}">&#8369;
                                {{ number_format($student->netassessed, 2, '.', ',') }}</td> --}}
                            <td class="stp{{ $student->id }}">&#8369;
                                {{ number_format($student->totalpayment, 2, '.', ',') }}</td>
                            <td class="sb{{ $student->id }}">&#8369;
