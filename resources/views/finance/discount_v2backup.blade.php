                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row form-group">
                            ...
                            <div class="col-md-3">
                                <button class="btn btn-primary text-sm btn-block" id="discount_create">Add Student to Avail
                                    Discount</button>
                            </div>
                        </div>
                        <div class="row">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            function discount_charges() {
                ...
                // console.log('groups: ' + groups[0].classname)
                        ...
                        success: function(data) {
                            if ($('#discount_setup').val() != 0) {
                                ...
                                $('#discount_data tr').each(function() {
                                    if ($(this).find('.discount_include').prop(
                                            'checked') == true) {
                                        var chargeAmount = $(this).find(
                                            '.discount_balance').text().replace(',',
                                            '')
                                        ...
                                        $(this).find('.discount_discamount').text(
                                            calcDiscount(chargeAmount,
                                                discountAmount))

                                        discAmount = $(this).find(
                                ...
                                $('#discount_totaldiscamount').text(parseFloat(totaldiscAmount,
                                        10).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,")
                                    .toString())
                            } else {
            ...
            $(document).on('click', '#discount_body tr', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('#discount_students').val(data.studid).trigger('change')
                        $('#discount_setup').val(data.discountid).trigger('change')
                        $('#discount_sy').val(data.syid)
                        $('#discount_sem').val(data.semid)
                        $('#discount_totalamount').text(data.totalcharges)
                        $('#discount_totaldiscamount').text(data.totaldiscount)

                        $('#discount_data').empty()
                        $('#discount_data').html(data.discountlist)
