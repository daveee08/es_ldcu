...
<table class="table_xai" style="width: 100%;">
    {{-- <thead>
        <tr>
    ...
    <tbody style="font-size: 12px;">
        ...
        <tr>
            <td colspan="3" width="35%" style="text-align: center; color: #147a85; font-weight: bold;">WHOLE YEAR
                PAYABLES</td>
            <td colspan="3" width="25%" style="text-align: center; color: #147a85; font-weight: bold;">BACK
                ACCOUNT</td>
            <td colspan="3" width="30%" style="text-align: center; color: #147a85; font-weight: bold;">PAYMENT
                STATUS</td>
        ...
        <tr>
            ...
            <td width="10%" style="text-align: right;">
                {{ number_format(collect($ledger)->where('groupname', 'TUI')->sum('payment'), 2) }}</td>
            <td style="text-align: right;">
                {{ number_format(collect($ledger)->where('groupname', 'TUI')->sum('amount') - collect($ledger)->where('groupname', 'TUI')->sum('payment'), 2) }}
            </td>
        </tr>
        <tr>
            ...
            <td style="text-align: right;">
                {{ number_format(collect($ledger)->where('groupname', 'MISC')->sum('payment'), 2) }}</td>
            <td style="text-align: right;">
                {{ number_format(collect($ledger)->where('groupname', 'MISC')->sum('amount') - collect($ledger)->where('groupname', 'MISC')->sum('payment'), 2) }}
            </td>
        </tr>
        <tr>
            ...
            <td style="text-align: right;">
                {{ number_format(collect($ledger)->where('groupname', 'TUI')->sum('payment') + collect($ledger)->where('groupname', 'MISC')->sum('payment'), 2) }}
            </td>
            <td style="text-align: right;"></td>
        </tr>
        <tr>
            <td colspan="2" style="text-align: left; font-weight: bold; color:#cc5200;">BALANCE:</td>
            <td style="text-align: right; font-weight: bold; color:#cc5200;">
                {{ number_format(collect($ledger)->where('groupname', 'TUI')->sum('amount') - collect($ledger)->where('groupname', 'TUI')->sum('payment') + (collect($ledger)->where('groupname', 'MISC')->sum('amount') - collect($ledger)->where('groupname', 'MISC')->sum('payment')), 2) }}
            </td>
            <td style="text-align: center;"></td>
            <td colspan="2" style="text-align: center;"></td>
            <td colspan="3" style="text-align: right; font-weight: bold; color:#cc5200;">MONTHLY
                {{-- PAYMENT:({{ $monthname }})</td> --}}
            <td style="text-align: right;">
        ...
        <tr>
            <td colspan="10"
                style="text-align: center; background-color: #ffffb3; font-size: 13px; color:#cc5200; font-weight: bold;">
                PAYMENT UPDATE</td>
        </tr>
        <tr>
            <td colspan="2" width="20%" style="text-align: center;"></td>
            <td colspan="2" width="27%" style="text-align: center; font-weight: bold;">AMOUNT</td>
            <td rowspan="{{ collect($ledger)->where('groupname', '!=', 'TUI')->where('groupname', '!=', 'MISC')->count() + 4 }}"
                style="text-align: center; width: 1%;"></td>
        ...
        <tr style="font-size: 9px;">
            ...
            <td></td>
            <td style="text-align: right;">
                {{ number_format(collect($ledger)->where('groupname', '!=', 'TUI')->where('groupname', '!=', 'MISC')->sum('payment'), 2) }}
            </td>
            <td></td>
        </tr>
