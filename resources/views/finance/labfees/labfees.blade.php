@php
    ...
    else if(Session::get('currentPortal') == 4 || Session::get('currentPortal') == 15 || Session::get('currentPortal') == 41){
...
@section('modal')
        ...
        <div class="modal-dialog modal-md">
            <div class="modal-content text-sm">
                ...
                <div class="modal-body overflow-auto" style="height: 230px">
                    ...
                    <div class="row form-group">
                        <div class="col-md-6">
                            <select id="labfees_courseid" class="select2bs4 manageui" style="width: 100%">
                                <option value="0">COURSES - ALL</option>
                        ...
                        <div class="col-md-6">
                            <select id="labfees_level" class="select2bs4 manageui" style="width: 100%" disabled>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <select id="subj" class="select2bs4 form-control manageui" style="width: 100%">
                                {{-- <option value="0">SELECT SUBJECT</option>
                                <option value="add">ADD SUBJECT</option>
                                @foreach(DB::table('college_prospectus')->where('deleted', 0)->where('labunits', '!=',0.0)->groupBy('subjCode','subjDesc')->get() as $subj)
                                    <option value="{{$subj->id}}">{{$subj->subjCode . ' - ' . $subj->subjDesc}}</option>
                                @endforeach
                                @foreach(DB::table('labfee_subject')->join('college_prospectus', 'labfee_subject.prospectusid', '=', 'college_prospectus.id')->select('college_prospectus.*','labfee_subject.prospectusid as labfeeid')->get() as $labsubj)
                                   <option value="{{$labsubj->labfeeid}}">{{$labsubj->subjCode . ' - ' . $labsubj->subjDesc}}</option>
                                @endforeach --}}
                            </select>
                        </div>
                    </div>
    ...
    <div class="modal fade show" id="modal-addlabsubject" data-backdrop="static" aria-modal="true" style="display: none;">
        <div class="modal-dialog modal-md">
            <div class="modal-content text-sm">
                <div id="" class="modal-header bg-primary">
                    <h4 class="modal-title">Add Lab Subject</h4> 
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body overflow-auto" style="height: 100px">
                        <div class="row form-group">
                            <label for="subject">Subject</label>
                            <select id="subj_add" class="select2bs4 form-control manageui" style="width: 100%">
                                <option value="0">SELECT SUBJECT</option>
                                @foreach(DB::table('college_prospectus')->where('deleted', 0)->groupBy('subjCode','subjDesc')->get() as $subj)
                                    <option value="{{$subj->id}}">{{$subj->subjCode . ' - ' . $subj->subjDesc}}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="modal-body bg-light">
                    <div class="row">
                        ...
                        <div class="col-md-6 text-right">
                            <button id="savelabfeesubject" type="button" class="btn btn-primary"><i class="fas fa-save"></i> Save</button>  
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            ...
            function populate_subjects(){
                $.ajax({
                    url: '{{route('labfeesubject_get')}}',
                    type: 'GET',
                    success: function (data) {
                        $('#subj').empty();
                        $('#subj').append('<option value="0">Select Subject</option>');
                        $('#subj').append('<option value="add">Add Subject</option>');
                        $.each(data, function (key, value) {
                            $('#subj').append('<option value="' + value.id + '">' + value.subjDesc + '</option>');
                        })
                        select_subject()
                    }
                });
            ...
            {
                console.log('aaa');
                if($('#labfees_courseid').val() != 0)
                {
                    $('#labfees_level').prop('disabled', false)
                    $('#subj').val(0).trigger('change')
                    $('#subj').prop('disabled', true)
                }
                else{
                    $('#labfees_level').val(0).trigger('change')
                    $('#labfees_level').prop('disabled', true)
                    $('#subj').prop('disabled', false)
                }

                if($('#subj').val() != 0)
                {
                    $('#labfees_courseid').val(0).trigger('change')
                    $('#labfees_courseid').prop('disabled', true)

                    $('#labfees_level').val(0).trigger('change')
                    $('#labfees_level').prop('disabled', true)
                }
                else{
                    $('#labfees_courseid').prop('disabled', false)
                    $('#labfees_level').prop('disabled', false)
                }
                
            }

            $(document).on('select2:close', '.manageui', function(){
                // console.log('aaa');
                manageui();
            ...
            function select_subject(){
                $('#subj').select2({
                    templateResult: function (data) {
                        if (data.id === 'add') {
                            return $('<span style="color: black;"><i class="fa fa-plus text-success"></i> ' + data.text + '</span>');
                        }
                        return data.text;
                    },
            ...
            $(document).on('change', '#subj', function(){
                if($(this).val() === 'add'){
                    $('#subj').val(0).trigger('change');
                    $('#modal-addlabsubject').modal('show');
                }
            })

            $(document).on('click', '#savelabfeesubject', function(){

                $.ajax({
                    url: '{{route('labfeesubject_append')}}',
                    type: 'GET',
                    dataType: '',
                    data: {
                        subj:$('#subj_add').val(),
                    ...
                    success:function(data)
                    {
                        ...
                        $('#modal-addlabsubject').modal('hide')
                    }
                })
            })
