@php
    ...
    if (Session::get('currentPortal') == 14) {
        ...
        if (isset($check_refid->refid)) {
            ...
            } elseif ($check_refid->refid == 34) {
                $extend = 'library.layouts.backend';
...
@section('footerjavascript')
    ...
    <script>
        $(document).ready(function() {
            ...
            // $(document).on('change', '#studpic', function() {
            ...
            function get_profile() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#gender').text(data[0].gender != null ? data[0].gender.toString()
                            .toUpperCase() : '--')
                        ...
                        $.each(data[0].acadprog, function(a, b) {
                            acadprog += '<span class="badge badge-primary mr-2">' + b.progname +
                                '</span>'
                        })
                        ...
                        $.each(data[0].faspriv, function(a, b) {
                            privuser += '<span class="badge badge-primary mr-2">' + b.utype +
                                '</span>'
                        })
                        ...
                        // var picurl = data[0].picurl.replace('jpg', 'png') + "?random=" + new Date()
                        //     .getTime()
                        ...
                        $('#image_holder')[0].innerHTML = image;
                        // $('#demo')[0].innerHTML = image; // Ensure the initial src is set

                        // $('#studpic').change(function() {
                        //     var selectedFile = this.files[0];
                        //     if (selectedFile) {
                        //         var idxDot = selectedFile.name.lastIndexOf(".") + 1;
                        //         var extFile = selectedFile.name.substr(idxDot, selectedFile.name
                        //             .length).toLowerCase();
                        //         if (extFile === "jpg" || extFile === "jpeg" || extFile ===
                        //             "png") {
                        //             var reader = new FileReader();
                        //             reader.onload = function(e) {
                        //                 $('#demo')[0].innerHTML =
                        //                     '<img width="100%" src="' + e.target
                        //                     .result + '" onerror="this.src=\'' +
                        //                     onerror_url +
