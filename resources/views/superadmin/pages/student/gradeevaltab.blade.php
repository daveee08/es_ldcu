...
<script>
    ...
    function get_loadedsubjects(studID){
        $.ajax({
            type: 'GET',
            url: '/superadmin/student/grade/evaluation/get/loaded_subjects',
            data: {
                studid: studID,
            },
        ...
        $.each(schoolyear, function(index, sy) {
            $.each(semester, function(index, sem) {
                ...
                $.each(grades, function(index, grade) {
                    $('.div-' + grade.syid + '-' + grade.semid).removeClass('d-none')
                })
                
                let new_grades = grades.filter(grade => grade.syid == sy.id && grade.semid == sem.id)
                console.log(new_grades,'newgraes');
                ...
                $('#table-gradeeval-' + sy.id + '-' + sem.id).DataTable({
                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            ...
                            orderable: false,
                            createdCell: function(td, cellData, rowData) {
                                if (rowData.prelim_status == 5 && rowData.midterm_status == 5 && rowData.final_status == 5) {
    ...
    function student_grade_all(grades_eval){
        console.log(grades_eval,'grades_eval');
                    ...
                    $('#table_modal-' + level.id + '-' + sem.id).DataTable({
                        ...
                        columnDefs: [
                            ...
                            },
                            {
                                ...
                                orderable: false,
                                createdCell: function(td, cellData, rowData) {
                                    if (rowData.final_status != 5 && rowData.final_status != 3) {
                                        if(rowData.final_transmuted === 'DROPPED'){
                                            $(td).html(`<p class="p-1 mb-0 text-center" data-id="${rowData.id}">DROPPED</p>`)
                                        }else{
                                            $(td).html('')
                                            .addClass('text-center credited_subj')
                                        ...
                                        if (rowData.final_status == 8 || rowData.final_transmuted === 'DROPPED') {
                                            content = 'DROPPED';
                                        } else {
                                            content = rowData.grades.gpa ?? '';
                                        }

                                        $(td).html(`<p class="p-1 mb-0" data-id="${rowData.id}">${content}</p>`)
                                            .addClass('text-center');
                                    }
                                }
                            },
    ...
    $(document).on('hidden.bs.modal', '#studentLoadingModal', function() {
