...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="mb-2">
                <div class="">
                    <h1><i class="fa fa-cog"></i> Grading Equivalence Table</h1>
                </div>
                <div class="ml-4">
                    <ol class="breadcrumb">
        ...
        <div class="container-fluid">
            <ul class="nav nav-tabs" id="studentInfoTabs" role="tablist">
                ...
                <li class="nav-item mr-3" role="presentation">
                    <a class="nav-link" href="{{ url('tesda/gradeTransmutation') }}" style="color: black;">Grading
                        Transmutation
                        Table</a>
                </li>

                <li class="nav-item" role="presentation">
                    <a class="nav-link active" href="{{ url('tesda/setup/gradetransmutation/index') }}">Grade
                        Point Equivalency</a>
                </li>
            </ul>
        </div>
                ...
                <div class="col-md-12">
                    <div class="card shadow-sm">
                        <div class="card-header  bg-secondary">
                            <h3 class="card-title fw-1000">Grading Equivalence Table</h3>
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <div class="">
                                    <button class="btn btn-success btn-sm" id="addGradePointEquivalency"
                                        data-toggle="modal">Add Grading Equivalence Table</button>
                                </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addECRModalLabel"><strong>New Grading Equivalence Table</strong></h5>
                    ...
                    <form id="gradePointEquivalencyForm">
                        ...
                        <div class="row mb-3">
                            <label for="gradePointEquivalencyDescription" class="form-label mt-2 col-md-6"
                                style="font-size: 14px;">Grading Equivalence Description</label>

                            <input type="text" class="form-control form-control-sm col-md-9 ml-2"
                                id="gradePointEquivalencyDescription" placeholder="Enter Description" required>
                        ...
                        <div class="table-responsive">
                            <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                                <table class="table table-bordered" id="gradingPointsTable" style="font-size:12px;">
                                    <thead>
                                        <tr>
                                            <th>Grade Point System</th>
                                            <th>Letter Gading System</th>
                                            <th>Percentage Grading System</th>
    ...
    <script>
        $(document).ready(function() {
            ...
            function update_pointEquivalencyscale() {
                ...
                $("#editgradingPointsTable").find("tbody tr").each(function() {
                    if ($(this).is(":visible")) {
                        ...
                        if (gradePointEquivalency && letterGradeEquivalency && percentEquivalency &&
                            gradingRemarks) {
                            tableData.push({
                ...
                $.ajax({
                    type: "POST",
                    url: '/tesda/setup/gradepointequivalency/update',
                    data: {
            ...
            function create_pointEquivalencyscale() {
                ...
                $.ajax({
                    type: 'GET',
                    url: '/tesda/setup/gradepointequivalency/create',
                    data: {
                        gradePointDesc: gradePointEquivalencyDescription,
                        gradePointScaleData: tableData,
            ...
            function gradePointEquivalencyTable() {
                ...
                $("#gradePointEquivalencyTable").DataTable({
                    ...
                    // processing: true,
                    ajax: {
                        url: '/tesda/setup/gradepointequivalency/fetch',
                        type: 'GET',
                    ...
                    columnDefs: [
                        ...
                        // },
                        {
                            ...
                            'orderable': false,
                            'createdCell': function(td, cellData, rowData, row, col) {
                                $(td).html(rowData.grade_description).addClass('align-middle');
                            }
                        },
            ...
            $(document).on('click', '.view_gradepoint_equivalency', function() {
                ...
                $.ajax({
                    type: 'GET',
                    url: '/tesda/setup/selectedgradepointequivalency/fetch',
                    data: {
                        point_equivalency_id: point_equivalency_id
                    },
                    success: function(response) {
                        console.log(response);
                        var grade_point_equivalency = response.grade_point_equivalency;
                        var grade_point_scale = response.grade_point_scale;

                        $("#gradePointEquivalencyID").val(grade_point_equivalency
                            .grade_point_equivalency_id);
                        $("#edit_gradePointEquivalencyDescription").val(grade_point_equivalency
                            .grading_equivalence_desc);
        ...
        $(document).on('click', '.delete_gradepoint_equivalency', function() {
            var grade_point_equivalency_id = $(this).attr('data-id')
