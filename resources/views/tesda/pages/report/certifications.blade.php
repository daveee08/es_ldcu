...
@section('content')
    ...
    <section class="content-header">
        <div class="container-fluid">
            <h1><i class="fas fa-cog"></i>Certifications</h1>
            <ol class="breadcrumb float-sm-left">
                <li class="breadcrumb-item"><a href="/home">Home</a></li>
                <li class="breadcrumb-item active">{{ isset($page) ? $page : 'Certifications' }}</li>
            </ol>
        </div>
    </section><br>
        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title m-0"><i class="fas fa-list"></i> Student List</h3>
                    <div class="dropdown ml-auto">
                        <button class="btn btn-sm btn-primary dropdown-toggle" id="print_certifications"
                            data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
                            style="font-size: 13px;"><i class="far fa-file-pdf mr-1"></i>
                            Print Certifications
                        </button>
                        <ul class="dropdown-menu" aria-labelledby="print_certifications">
                            <li><a class="dropdown-item" href="/tesda/printCertCompletionAll" target="_blank">Certificate of Completion</a></li>
                            <li><a class="dropdown-item" href="/tesda/pdf/national_certificate" target="_blank">National Certificate</a></li>
                            <li><a class="dropdown-item" href="/tesda/pdf/honorable_dismissal_all" target="_blank">Certificate of Honorable Dismissal</a></li>
                        </ul>
                    </div>
                </div>
                <div class="card-body">
                                                    ...
                                                    <form style="font-size: 12px;">
                                                        ...
                                                        <div class="row mb-2">
                                                            ...
                                                            <div class="col-md-5">
                                                                <label for="gHAE">High Educational Attainment</label>
                                                                <input type="text"
                                                                    class="form-control form-control-sm" id="gHAE2"
                                                                    placeholder="Enter High Educational Attainment"
                                                                    readonly>
                                                            </div>
                                                            <div class="col-md-2">
    ...
    <script>
        ...
        $(document).ready(function() {
                ...
                for (const gender in groupedData) {
                    ...
                    students.forEach(student => {
                        ...
                        tableData.push({
                            ...
                            html: html,
                            sid: student.sid,
                            date_enrolled: student.dateenrolled,
                        ...
                        //     <div class="col-md-6">
                        //         <a href="#"><p class="text-primary print-cor" data-id="${student.id}">View Details</p></a>
                        //     </div>
                        // </div>`
                            options: `
                            <div class="row text-center">
                                <div class="col-md-12">
                                    <div class="dropdown">
                                        <a href="#" class="text-primary print-certeficates m-0" id="print-specific-certeficates" data-bs-toggle="dropdown" data-id="${student.id}" aria-haspopup="true" aria-expanded="false">View Details</a>
                                        <div class="dropdown-menu" aria-labelledby="print-specific-certeficates">
                                            <a class="dropdown-item" data-id="${student.id}" href="/tesda/printCertCompletionAll?id=${student.id}" target="_blank">Certificate of Completion</a>
                                            <a class="dropdown-item" data-id="${student.id}" href="/tesda/pdf/national_certificate?id=${student.id}" target="_blank">National Certificate</a>
                                            <a class="dropdown-item" data-id="${student.id}" href="/tesda/pdf/honorable_dismissal/${student.id}" target="_blank">Certificate of Honorable Dismissal</a>
                                        </div>
                                    </div>
                                </div>
                            </div>`
                ...
                $('#studentList').DataTable({
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            searchable: false,
                            render: function(data, type, row) {
                                return row.genderRow ? '' : row.date_enrolled ?? 'Not Specified';
                            }
                        },
                        {
                            data: null,
            ...
            function getStudents() {
                LoadingModal.show();
                $.ajax({
                    url: '/tesda/certifications/enrollment/get',
                    type: 'POST',
                    data: {
            ...
            function renderRegistrationContent(response, studentId) {
                if (response.success) {
                    ...
                    let registrationContent = `
                            <button class="btn btn-primary btn-sm mb-3 mt-3" id='printNationalCert' data-id="${studentId}">Print National Certificate</button>
                            <button class="btn btn-primary btn-sm mb-3 mt-3" id='printCertCompletion' data-id="${studentId}">Print Certificate of Completion</button>
                            <button class="btn btn-primary btn-sm mb-3 mt-3" id='printCertHonorable' data-id="${studentId}">Print Certificate of Honorable Dismissal</button>
                    ...
                    // Loop through schedules grouped by competency description
                    for (const competency in schedules) {
                        if (schedules.hasOwnProperty(competency)) {
                            const competencySchedules = schedules[competency];

                            competencySchedules.forEach((schedule, index) => {
                                registrationContent += `
                        <tr>
                            <td>${index === 0 ? competency : ''}</td> <!-- Show competency only once per group -->
                            <td>${schedule.hours ?? 'TBA'}</td>
                            <td>
                                ${schedule.date_from && schedule.date_to
                                    ? `${schedule.date_from} to ${schedule.date_to}`
                                    : 'TBA'} 
                                (${schedule.stime && schedule.etime
                                    ? `${schedule.stime} - ${schedule.etime}`
                                    : 'TBA'})
                            </td>
                            <td>${schedule.trainer ?? 'TBA'}</td>
                            <td>${schedule.roomname ?? 'TBA'}</td>
                        </tr>
                    `;
