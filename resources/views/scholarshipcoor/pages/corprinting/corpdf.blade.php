...
<body style="margin: 0.25in 0.25in 0.25in 0.25in;">
    <div class="">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td style="text-align: right; font-size:11px">
                                        Block Code: {{ $studentInfo->sectionDesc }}
                                        {{-- Block Code: 
        ...
        <table width="100%" class="table table-sm table-bordered table-smaller mb-0">
            ...
            <tbody>
                ...
                @foreach ($schedules as $item)
                    @php $count = 0; @endphp
                    <tr>
                        <td width="8%">{{ $item[0]->subjCode }}</td>
                        <td width="30%">{{ $item[0]->subjDesc }}</td>
                        <td class="text-center" width="5%">
                            {{ number_format($item[0]->lecunits + $item[0]->labunits, 1) }}
                        </td>
                        <td class="text-center" width="20%">
                            @foreach ($item as $timeitem)
                                @php $schedother = substr($timeitem->schedotherclass, 0, 3) . ':'; @endphp
                                @if ($timeitem->stime && $timeitem->etime)
                                    <p>{{ $schedother }}
                        ...
                        <td class="text-center" width="8%">
                            @foreach ($item as $descriptiontem)
                                @if ($descriptiontem->description)
                                    <p>{{ $descriptiontem->description }}</p>
                                @endif
                            @endforeach
                        </td>
                        <td class="text-center" width="14%">
                            @foreach ($item as $roomitem)
                                @if ($roomitem->roomname)
        ...
        <table class="table mb-0" width="100%" style="font-size:.5625rem !important; transform: translateY(-1.5em);">
            <tr>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-right p-1" style="font-size:.7525rem !important">
                                        <b>{{ number_format(collect($ledger)->sum('balance'), 2) }}</b>
                                    </td>
                                </tr>
                                <tr>
                                ...
                                @if($first_payment)
                                <tr>
                                    <td class="p-1">{{ $first_payment ? $first_payment->items : 'N/A' }} - {{ $first_payment ? $first_payment->transdate : 'N/A' }}
                                ...
                                <tr>
                                    ...
                                    <td class="p-1 bold text-right"
                                        ...
                                        {{ number_format(collect($ledger)->sum('balance'), 2) }}</td>
                                </tr>
                                <tr>
                                    @php
                                        $balance_raw = collect($ledger)->sum('balance') - (isset($first_payment) ? (is_numeric($first_payment->amountpaid) ? $first_payment->amountpaid : 0) : 0);
                                        $balance = number_format($balance_raw, 2);
                                        $exam_balance =  collect($ledger)->sum('balance');
                                    @endphp
                                    <td class="p-1 bold"
                                ...
                                <tr>
                                    ...
                                    <td class="text-right p-1"
                                        style="font-size:.9525rem !important; border-top: 1px solid black">
                                        <b>{{ number_format($exam_balance / 5, 2) }}</b>
                                    </td>
                                </tr>
                                @else
                                <tr>
                                ...
                                <tr>
                                    @php
                                        $balance_raw = collect($ledger)->sum('amount') - (isset($first_payment) ? (is_numeric($first_payment->amountpaid) ? $first_payment->amountpaid : 0) : 0);
                                        $balance = number_format($balance_raw, 2);
                                        $exam_balance =  collect($ledger)->sum('balance');
                                    @endphp
                                    <td class="p-1 bold"
                    ...
                    <table class="table" width="100%" style="font-size:10px !important">
                        ...
                        {{-- <tr>
                            @if (strtolower($schoolInfo->abbreviation) == 'stii')
                                ...
                                @if ($portal == 'finance')
                                    <td class="p-0 text-center" style="border-bottom:solid 1px black">Jonalyn P. Doruha</td>
                                @else
                        ...
                        {{-- <tr>
                            ...
                            @if ($portal == 'finance')
                                <td class="p-0 text-center"> ACCOUNTING CLERK</td>
                            @else
    ...
    <hr class="mb-2" style="border-top: 1px  black;">
                    ...
                    <td width="38%" class="p-0 align-top">
                        <table class="table " width="100%" style="font-size:.7625rem !important">
                            <tbody>
                                ...
                                @if($first_payment)
                                <tr>
                                    <td class="p-1">{{ $first_payment ? $first_payment->items : '0' }} - {{ $first_payment ? $first_payment->transdate : 'N/A' }}
                                        ORNO. {{ $first_payment ? $first_payment->ornum : 'N/A' }}</td>
                                    <td class="p-1 text-right" style="border-bottom: 1px solid black">
                                        {{ $first_payment ? $first_payment->amountpaid : 'N/A' }}</td>
                                </tr>
                                <tr>
                                    <td class="p-1 bold"
                                        style="border-bottom: 1px solid black; font-size:.7625rem !important">Sub-total
