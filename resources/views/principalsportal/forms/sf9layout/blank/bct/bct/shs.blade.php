...
<table class="table table-sm mb-0" width="100%">
      <tr>
        ...
        <td width="35%" class="border-bottom">{{$student[0]->levelname}} / {{$student[0]->ensectname}}</td>
    </tr>
</table>
<table class="table table-sm mb-1" width="100%">
      <tr>
        <td width="10%">TRACK:</td>
        <td width="30%" class="border-bottom">{{str_ireplace('TRACK', '' ,strtoupper($strandInfo[0]->trackname)) }}</td>
        <td width="8%">STRAND:</td>
        <td width="52%" class="border-bottom">{{$strandInfo[0]->strandname}}</td>
    </tr>
</table>
    ...
    <tbody>
        ...
        @foreach (collect($grades)->where('semid',1)->where('type',1) as $item)
                ...
                <tr class="table-bordered">
                    <td style="padding-left: 5px"><b>{{$item->sc}}</b></td>
                    <td class="p-1">{{$item->subjectcode!=null? $item->subjectcode : ''}}</td>
                    ...
                    <td class="text-center align-middle">{{$with_final_rating ? number_format($average) : ''}}</td>
                    <td class="text-center align-middle"><i>{{$with_final_rating ? number_format($average) >= 75 ? 'Passed' : 'Failed' : '' }}</i></td>
                </tr>
            @endforeach
        <tr style="background-color:#ffff66;">
            ...
            @foreach (collect($grades)->where('semid',1)->where('type',3) as $item)
                ...
                <tr class="table-bordered">
                    <td style="padding-left: 5px"><b>{{$item->sc}}</b></td>
                    <td class="p-1" style="text-align: left !important; padding-left: 5px; font-size:9px" >{{$item->subjectcode!=null? $item->subjectcode : ''}}</td>
                    <td class="text-center align-middle" >{{$item->quarter1 != null ? number_format($item->quarter1) : ''}}</td>
                    <td class="text-center align-middle" >{{$item->quarter2 != null ? number_format($item->quarter2) : ''}}</td>
                    <td class="text-center align-middle" >{{$with_final_rating ? number_format($average) : ''}}</td>
                    <td class="text-center align-middle" ><i>{{$with_final_rating ? number_format($average) >= 75 ? 'Passed' : 'Failed' : '' }}</i></td>
        ...
        <tr style="background-color:#538ed5">
            ...
            <!--<tr>-->
            <!--    <td style="font-size: 11px;background-color:#ed6c64;" width="10%"</td>-->
            <!--    <td style="font-size: 11px;background-color:#ed6c64;" width="60%">-->
            <!--        HOMEROOM GUIDANCE-->
            <!--    </td>-->
            <!--    <td width="10%" style="font-size: 11px;background-color:#ed6c64;"></td>-->
            <!--    <td width="10%" style="font-size: 11px;background-color:#ed6c64;"></td>-->
            <!--    <td width="10%" style="font-size: 11px;background-color:#ed6c64;"></td>-->
            <!--    <td width="10%" style="font-size: 11px;background-color:#ed6c64;"></td>-->
            <!--</tr>-->
            ...
            <!--    <td class="text-center">{{$finalhomeroom[0]->q1 != null && $finalhomeroom[0]->q2 != null ?  round( ( $finalhomeroom[0]->q1 + $finalhomeroom[0]->q2 ) / 2 ) : null}}</td>-->
            <!--    <td class="text-center"><i>-->
            <!--        @if($finalhomeroom[0]->q1 != null && $finalhomeroom[0]->q2 != null)-->
            <!--            @if(round( ( $finalhomeroom[0]->q1 + $finalhomeroom[0]->q2 ) / 2 ) == 0)-->
            <!--                NCO-->
            <!--            @elseif(round( ( $finalhomeroom[0]->q1 + $finalhomeroom[0]->q2 ) / 2 ) == 1)-->
            <!--                NI-->
            <!--            @elseif(round( ( $finalhomeroom[0]->q1 + $finalhomeroom[0]->q2 ) / 2 ) == 2)-->
            <!--                D-->
            <!--            @elseif(round( ( $finalhomeroom[0]->q1 + $finalhomeroom[0]->q2 ) / 2 ) == 3)-->
            <!--                SD-->
            <!--            @elseif(round( ( $finalhomeroom[0]->q1 + $finalhomeroom[0]->q2 ) / 2 ) == 4)-->
            <!--                D & C-->
            <!--            @endif-->
            <!--        @endif-->
            <!--    </i></td>-->
    ...
    <tbody>
        <tr style="background-color:#92d050;">
            ...
            @foreach (collect($grades)->where('semid',2)->where('type',1) as $item)
                ...
                <tr class="table-bordered">
                    ...
                    <td class="text-center align-middle" >{{$item->quarter1 != null ? number_format($item->quarter1) : ''}}</td>
                    <td class="text-center align-middle" >
                        {{$item->quarter2 != null ? number_format($item->quarter2) : ''}}
                    </td>
                    <td class="text-center align-middle" >
                        {{$with_final_rating ? number_format($average) : ''}}
                    </td>
                    <td class="text-center align-middle" >
                        <i>{{$with_final_rating ? number_format($average) >= 75 ? 'Passed' : 'Failed' : '' }}</i>
                    </td>
                </tr>
            @endforeach
        <tr style="background-color:#ffff66;">
        ...
        <tr style="background-color:#538ed5">
            ...
            @foreach (collect($grades)->where('semid',2)->where('type',2) as $item)
                @php
                    $with_final_rating = $item->quarter1 != null && $item->quarter2 != null ? true : false;
                    $average = $with_final_rating ? ($item->quarter1 + $item->quarter2 ) / 2 : 0;
                    $gen_ave_for_sem += $with_final_rating ? number_format($average) : 0;
                @endphp
                <tr class="table-bordered">
                    <td style="padding-left: 5px"><b>{{$item->sc}}</b></td>
                    ...
                    <td class="text-center align-middle" >
                        <i>{{$with_final_rating ? number_format($average) >= 75 ? 'Passed' : 'Failed' : '' }}
                        <!--</i>-->
                    </td>
                </tr>
            @endforeach
            ...
            <!--    <td class="text-center">{{$finalhomeroom[0]->q3 != null && $finalhomeroom[0]->q4 != null ?  round( ( $finalhomeroom[0]->q3 + $finalhomeroom[0]->q4 ) / 2 ) : null}}</td>-->
            <!--    <td class="text-center"><i>-->
            <!--        @if($finalhomeroom[0]->q3 != null && $finalhomeroom[0]->q4 != null)-->
            <!--            @if(round( ( $finalhomeroom[0]->q3 + $finalhomeroom[0]->q4 ) / 2 ) == 0)-->
            <!--                NCO-->
            <!--            @elseif(round( ( $finalhomeroom[0]->q3 + $finalhomeroom[0]->q4 ) / 2 ) == 1)-->
            <!--                NI-->
            <!--            @elseif(round( ( $finalhomeroom[0]->q3 + $finalhomeroom[0]->q4 ) / 2 ) == 2)-->
            <!--                D-->
            <!--            @elseif(round( ( $finalhomeroom[0]->q3 + $finalhomeroom[0]->q4 ) / 2 ) == 3)-->
            <!--                SD-->
            <!--            @elseif(round( ( $finalhomeroom[0]->q3 + $finalhomeroom[0]->q4 ) / 2 ) == 4)-->
            <!--                D & C-->
            <!--            @endif-->
            <!--        @endif-->
            <!--    </i></td>-->
            ...
            <tr>
                @php
                    $with_final = collect($grades)->where('semid',2)->where('quarter1','!=,',null)->count() == 0 && collect($grades)->where('semid',2)->where('quarter2','!=,',null)->count() == 0 ? true:false;
                @endphp
                <td colspan="2" style="text-align:right"><b>General Average for the Semester</b></td>
                <td colspan="2"></td>
 ...
 <table class="table table-sm table-bordered mb-1" width="100%" style="font-size:7px !important">
    ...
    <table class="table table-sm mb-1" width="100%"  style="margin-bottom:16px !important">
     ...
     <table class="table table-sm mb-0" width="100%">
        <tbody>
            <tr>
                <td width="50%"  class="text-center"><u>{{$adviser[0]->lastname}}, {{$adviser[0]->firstname}} {{$adviser[0]->middlename}}, LPT</u></td>
                <td width="50%"  class="text-center"><u>{{Session::get('prinInfo')->lastname}}, {{Session::get('prinInfo')->firstname}} {{Session::get('prinInfo')->middlename[0]}}. , LPT, MAST</u></td>
            </tr>
             <tr>
