...
<table class="table table-sm table-bordered mb-0" width="100%" style="margin-top:1rem !important">
    <tr>
        <td class="text-center">School Year {{$sy}}</td>
    </tr>
</table>
...
<table class="table table-sm" width="100%">
      <tr>
        ...
        <td width="43%" class="border-bottom">{{$student[0]->levelname}} / {{$student[0]->ensectname}}</td>
    </tr>
</table>
...
<table class="table table-sm table-bordered"  width="100%">
    ...
    <tbody>
        ...
        @foreach($grades as $item)
                ...
                <tr>
                    @if($item->mapeh == 1)
                      <td style="padding-left:1.5rem !important">{{$item->subjectcode}}</td>
                        @php
                            $fr = $item->quarter1 != null && $item->quarter2 != null && $item->quarter3 != null && $item->quarter4 != null ?  number_format( ( $item->quarter1 +  $item->quarter2 +  $item->quarter3 +  $item->quarter4 ) / 4) : '';
                    ...
                    <td class="text-center">{{$item->quarter1}}</td>
                    <td class="text-center">{{$item->quarter2}}</td>
                    <td class="text-center">{{$item->quarter3}}</td>
                    <td class="text-center">{{$item->quarter4}}</td>
                   
                    
                    <td class="text-center">{{$item->subjid != 26 && $item->subjid != 27 && $item->subjid != 28 && $item->subjid != 29 ? $fr : ''}}</td>
                    <td class="text-center">{{ ( $item->subjid != 26 && $item->subjid != 27 && $item->subjid != 28 && $item->subjid != 29 ) && $fr != '' ? $fr >= 75 ? 'PASSED' : 'FAILED' : ''}}</td>
                </tr>
          
        
        @endforeach
        <tr>
            <td colspan="5" style="text-align:right"><b>General Average</b></td>
            <td class="text-center">{{ number_format( $ge / collect($grades)->where('mapeh',0)->count() )}}</td>
            <td class="text-center">{{number_format( $ge / collect($grades)->where('mapeh',0)->count() ) >= 75 ? 'PASSED' : 'FAILED' }}</td>
    ...
    @php        
        $gradelevel = DB::table('gradelevel')->where('id',$levelid)->first();
    ...
    <table class="table table-sm" width="100%">
        <tbody>
            <tr>
                ...
                <td width="14%" class="border-bottom  text-center">{{$student[0]->ensectname}}</td>
    ...
    <br>
     <table class="table table-sm" width="100%">
        <tbody>
            <tr>
                <td width="50%"  class="text-center"><u>{{$adviser[0]->firstname}} {{$adviser[0]->middlename[0]}}. {{$adviser[0]->lastname}}</u></td>
