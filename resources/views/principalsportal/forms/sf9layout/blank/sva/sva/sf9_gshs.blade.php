        ...
        <table class="table table-bordered table-sm grades" width="100%">
            <thead>
                ...
                <table class="table table-bordered table-sm grades" width="100%">
                    ...
                    <tbody>
                        @foreach ($subjects as $item)
                            @if(isset(collect($studgrades)->where('subjid',$item->id)->first()->quarter1))
                                <tr>
                                    <td style="padding-left: {{$item->inTLE == 1 ? '1.5rem':'.5rem'}} !important;">{{$item->subjdesc}}</td>
                                    <td class="text-center {{collect($studgrades)->where('subjid',$item->id)->first()->quarter1 < 75 ? 'bg-red':''}}">{{collect($studgrades)->where('subjid',$item->id)->first()->quarter1 >= 60 ? collect($studgrades)->where('subjid',$item->id)->first()->quarter1 : ''}}</td>
                                    <td class="text-center {{collect($studgrades)->where('subjid',$item->id)->first()->quarter2 < 75 ? 'bg-red':''}}">{{collect($studgrades)->where('subjid',$item->id)->first()->quarter2 >= 60 ? collect($studgrades)->where('subjid',$item->id)->first()->quarter2 : ''}}</td>
                                    <td class="text-center {{collect($studgrades)->where('subjid',$item->id)->first()->quarter3 < 75 ? 'bg-red':''}}">{{collect($studgrades)->where('subjid',$item->id)->first()->quarter3 >= 60 ? collect($studgrades)->where('subjid',$item->id)->first()->quarter3 : ''}}</td>
                                    <td class="text-center {{collect($studgrades)->where('subjid',$item->id)->first()->quarter4 < 75 ? 'bg-red':''}}">{{collect($studgrades)->where('subjid',$item->id)->first()->quarter4 >= 60 ? collect($studgrades)->where('subjid',$item->id)->first()->quarter4 : ''}}</td>
                                    <td class="text-center {{collect($studgrades)->where('subjid',$item->id)->first()->finalrating < 75 ? 'bg-red':''}}"  width="10%">{{collect($studgrades)->where('subjid',$item->id)->first()->finalrating >= 60 ? collect($studgrades)->where('subjid',$item->id)->first()->finalrating : ''}}</td>
                        ...
                        <tr>
                            ...
                            <td class="text-center {{collect($finalgrade)->first()->quarter1 < 75 ? 'bg-red':''}}">{{collect($finalgrade)->first()->quarter2}}</td>
                            <td class="text-center {{collect($finalgrade)->first()->quarter1 < 75 ? 'bg-red':''}}">{{collect($finalgrade)->first()->quarter3}}</td>
                            <td class="text-center {{collect($finalgrade)->first()->quarter1 < 75 ? 'bg-red':''}}">{{collect($finalgrade)->first()->quarter4}}</td>
                ...
                <table class="table table-sm grades" width="100%" style="margin-top:40px !important">
                    <tbody>
                        <tr>
                            <td width="1000%" class="text-center align-middle"><u>{{Session::get('prinInfo')->firstname}} {{Session::get('prinInfo')->lastname}}</u></td>
                        </tr>
                        <tr>
                            <td class="text-center align-middle">Class Adviser</td>
                        </tr>
